# -*- coding: utf-8 -*-
# @Author: lshuns
# @Date:   2022-07-10 16:18:57
# @Last Modified by:   lshuns
# @Last Modified time: 2022-08-25 16:44:30

### check m difference between different PSF size 
...
# for saving results
outprefix = './outputs/dm_PSFsize_part'
...
cata_used = pd.DataFrame({
        ...
        'PSFsize': np.array(cata_used['PSFsize']).astype(float)})

# >>>>>>>>> split the whole catalogue based on the PSFsize
## split based on PSFsize
cata_used.loc[:, 'bin_PSFsize'] = pd.qcut(cata_used['PSFsize'].values, 3, 
                                    labels=False, retbins=False)
print('>>> mean PSFsize:\n  ', cata_used[['bin_PSFsize', 'PSFsize']].groupby('bin_PSFsize').mean())
...
for i_part in range(3):
    ...
    cata_used_tmp = cata_used[cata_used['bin_PSFsize'] == i_part].copy()
...
for i_bin in range(len(bin_edges)-1):
    ...
    for i_part in range(3):
        # select objects in these part
        cata_used_tmp = cata_selec[cata_selec['bin_PSFsize'] == i_part].copy()
...
# 1            1.961655
# 2            2.356111
# results saved as ./outputs/dm_PSFsize_part
# Elapsed:8:54.13,User=347.037,System=1159.921,CPU=282.1%.