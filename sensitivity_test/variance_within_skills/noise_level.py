# -*- coding: utf-8 -*-
# @Author: lshuns
# @Date:   2022-07-12 12:49:51
# @Last Modified by:   lshuns
# @Last Modified time: 2022-08-25 16:12:21

### check m difference between different noise levels
...
# noise info
in_file_noise = '../../noise_info/skills_fiducial/noise_selec_combined.csv'

# for saving results
outprefix = './outputs/dm_noise_part'
...
# >>>>>>>>> split the whole catalogue based on the noise level
## noise level info
noise_info = pd.read_csv(in_file_noise)
noise_info = noise_info[['label', 'rmsExpo_r']]
## split based on noise level
noise_info.loc[:, 'bin_noise'] = pd.qcut(noise_info['rmsExpo_r'].values, 3, 
                                    labels=False, retbins=False)
print('>>> mean noise:\n  ', noise_info.groupby('bin_noise').mean())
tile_labels_list = [noise_info.loc[noise_info['bin_noise']==i_binnoise, 'label'].values for i_binnoise in range(3)]
del noise_info
...
# 1           6.058390
# 2           6.847653
# results saved as ./outputs/dm_noise_part
# Elapsed:13:14.30,User=623.031,System=889.580,CPU=190.4%.
