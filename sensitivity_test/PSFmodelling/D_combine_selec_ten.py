# -*- coding: utf-8 -*-
# @Author: lshuns
# @Date:   2023-04-10 17:01:11
# @Last Modified by:   lshuns
# @Last Modified time: 2023-04-10 18:13:40

### a combined catalogue with selections
###### following ten-band catalogue selections
...
wanted_cols = './columns_photometry_ten.txt'
## wanted info
...
# out info
outpath = os.path.join(main_dir, f'skills_v07D7ten_dr5_LF_321_{combined_tag}_noSG_noWeiCut_newCut.feather')
...
for shear_tag in unique_shear_tags:
    ...
    for file in file_list:
        ...
        for part in parts:
            ...
            files_tmp = glob.glob(os.path.join(dir_fiducial_part, shear_tag, 'catalogues', 'tile*_rot0_combined_dr5_ten_kids_filters.feather'))
            ...
            if (tile_label in tile_labels_tmp):
                ...
                # load the catalogue
                file = os.path.join(dir_fiducial_part, shear_tag, 'catalogues', f'tile{tile_label}_rot{gal_rot:.0f}_combined_dr5_ten_kids_filters.feather')
        ...
        # >>>>>>>>>> photometry-related
        ## 1. ten-band photometry cut
        flag_gaap = np.zeros(len(cata)) 
        for band in ['u', 'g', 'r', 'i', 'i2', 'Z', 'Y', 'J', 'H', 'Ks']:
            flag_gaap += cata[f'FLAG_GAAP_{band}'].values
        mask_gaap = (flag_gaap==0)
        del flag_gaap
        ...
        mask_ast = ((gmr <= 1.5) | (imr <= 1.5) | (i2mr <= 1.5))
        del gmr, imr, i2mr
...
# ...
# Total number of source 13815729
# combined cata saved to /disks/shear16/ssli/ImSim/output/skills_v07D7p1_PSFmodelling/skills_v07D7ten_dr5_LF_321_combined_PSFmodelling41_321_noSG_noWeiCut_newCut.feather
# Elapsed:4:56.28,User=272.737,System=914.951,CPU=400.8%.
