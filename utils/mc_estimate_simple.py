...
########################
# usage: mc_estimate_simple.py [-h] [--in_file_sim IN_FILE_SIM]
#                              [--out_path OUT_PATH]
...
#                             Default ./test.txt.
#   --cols_g12_e12_tile COLS_G12_E12_TILE COLS_G12_E12_TILE COLS_G12_E12_TILE COLS_G12_E12_TILE COLS_G12_E12_TILE
#                         column names for g1_in, g2_in, e1, e2, tile_label.
#   --e_type {input,measured}
...
from mc_CalFunc import mcCalFunc_simple
...
parser.add_argument(
    ...
    Default ./test.txt.")
parser.add_argument(
    "--cols_g12_e12_tile", type=str, nargs=5, default=['g1_in', 'g2_in', 'e1', 'e2', 'tile_label'], 
    help="column names for g1_in, g2_in, e1, e2, tile_label.")
parser.add_argument(
...
parser.add_argument(
    "--nboot", type=int, default=0,
    help="Number of boots used for bootstrapping.")
parser.add_argument(
    "--rng_seed_boot", type=int, default=201294,
...
cata_used = pd.DataFrame({
    ...
    'e1_out': e1_out,
    'e2_out': e2_out,
    'tile_label': np.array(cata_sim[col_tile])})
## binning values
if col_binning is not None:
...
# whole sample
res = mcCalFunc_simple(cata_used, nboot=nboot, rng_seed_boot=rng_seed_boot)
N_s = len(cata_used)
...
if col_binning is not None:
    for i_bin in range(len(bin_edges)-1):
        ...
        res = mcCalFunc_simple(cata_selec, nboot=nboot, rng_seed_boot=rng_seed_boot)
        N_s = len(cata_selec)
