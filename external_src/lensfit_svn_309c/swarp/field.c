 ...
 ***/
fieldstruct *
inherit_field(char *filename, fieldstruct * reffield, int flags)
{
    fieldstruct *field;
 ...
 ***/
void
printinfo_field(fieldstruct * field, fieldstruct * wfield)
{
    wcsstruct *wcs;
    char stra[16], strd[16];
    static double pixpos[NAXIS], wcspos[NAXIS];
    double pixscale;
    int i;
...
/* Find field center */
    ...
    if (wcs->lat != wcs->lng) {
        ...
        QPRINTF(OUTPUT,
                ...
                wcs->wcsscale[0], wcs->wcsscale[1]);
    } else
        QPRINTF(OUTPUT, "    Center: %.3g   %.3g  Scale: %.3g /pixel\n",
                wcspos[0],
                wcs->naxisn[0] * wcs->wcsscale[0], wcs->wcsscale[0]);
...
/* Photometry */
    QPRINTF(OUTPUT, "    Gain: %.3g e-/ADU   Flux scaling (astrom/photom): "
            "%.4g X / %.4g X\n", field->gain, field->fascale, field->fscale);
 ...
 ***/
fieldstruct *
init_field(fieldstruct ** infield, int ninput, char *filename)
{
    ...
    tabstruct *tab;
    wcsstruct *wcs;
    double pixscale[NAXIS], val, epoch;
    float *scale;
    char *ctype[NAXIS], *pstr;
    int *axis,
...
/*-- Copy the units */
        for(n = 0; n < naxis; n++) {
            if (n == lng)
                strncpy(wcs->cunit[lng],
                        infield[0]->wcs->cunit[axis[0 + lng]], 8);
            else if (n == lat)
                strncpy(wcs->cunit[lat],
                        infield[0]->wcs->cunit[axis[0 + lat]], 8);
            free(ctype[n]);
        }
...
/*-------- Find (in a dumb way) the densest part of the coadd */
                ...
                for(j = 0; j < ninput; j++) {
                    ...
                    for(i = 0; i < ninput; i++) {
                        wcsmin2 =
                            infield[i]->wcs->wcsmin[axis[i * naxis + n]];
                        wcsmax2 =
                            infield[i]->wcs->wcsmax[axis[i * naxis + n]];
...
/*-------- Handled swapped ra, dec axes (e.g. SDSS) */
            ...
            default:
                error(EXIT_FAILURE,
                      "*Internal Error*: Unknown area type in ",
                      "init_field()");
                break;
            }
            ...
            case PIXSCALE_MIN:
                pixscale[n] = BIG;
                for(i = 0; i < ninput; i++)
                    if (infield[i]->wcs->wcsscale[axis[i * naxis + n]] <
                        pixscale[n])
                        pixscale[n] =
                            infield[i]->wcs->wcsscale[axis[i * naxis + n]];
                break;
            case PIXSCALE_MAX:
                pixscale[n] = -BIG;
                for(i = 0; i < ninput; i++)
            ...
            case PIXSCALE_FIT:
                ...
                pixscale[n] =
                    (wcsmax[n] - wcsmin[n]) / (double) prefs.image_size[n];
                break;
            ...
            default:
                error(EXIT_FAILURE,
                      "*Internal Error*: Unknown pixel scale type in ",
                      "init_field()");
                break;
            }
...
/*---- Following the tradition in astronomy, CDELTx decreases with longitude */
            wcs->cd[n * (naxis + 1)] = wcs->cdelt[n] = (n == lng
                                                        && lng !=
                                                        lat) ? -pixscale[n]
                : pixscale[n];
        }
...
/*-------- This is a special case where CRPIX is not NAXISN/2 */
                    ...
                    val =
                        wcs->cdelt[n] >=
                        0 ? wcs->crval[n] - wcsmin[n] : wcsmax[n] -
                        wcs->crval[n];
                    if ((n == lng || n == lat) && lat != lng) {
                        if (val < -180.0)
                            val += 360.0;
                        wcs->crpix[n] =
                            (int) (fmod(val, 180.0) / pixscale[n]) + 1.0;
/*-------- Add a 5% margin in field size */
                        tab->naxisn[n] = wcs->naxisn[n]
                            =
                            (int) (fmod(wcsmax[n] - wcsmin[n] + 360.0, 360.0)
                                   * 1.05 / pixscale[n])
                            + 1;
                    } else {
                        wcs->crpix[n] = (int) (val / pixscale[n]) + 1.0;
                        tab->naxisn[n] = wcs->naxisn[n]
                            = (int) (fabs(wcsmax[n] - wcsmin[n]) * 1.05 /
                                     pixscale[n]) + 1;
                    }
                } else {
/*-------- Add a 5% margin in field size */
        ...
        if (lat != lng) {
            ...
            if (!prefs.image_size[lng]) {
                if (prefs.center_type[lng] == CENTER_MANUAL) {
                    ...
                    tab->naxisn[lng] = wcs->naxisn[lng]
                        = (int) ((wcs->naxisn[lng] - 1) *
                                 cos(wcs->crval[lat] * DEG)) + 1;
                    wcs->crpix[lng] = (wcs->naxisn[lng] + 1.0) / 2.0;
                }
            } else if (prefs.pixscale_type[lng] == PIXSCALE_FIT)
                wcs->cd[lng * (naxis + 1)] = (wcs->cdelt[lng] *=
                                              cos(wcs->crval[lat] * DEG));
...
/*-------- Find (in a dumb way) the densest part of the coadd */
                ...
                for(j = 0; j < ninput; j++) {
                    ...
                    crpixmin1 =
                        infield[j]->wcs->crpix[n] -
                        infield[j]->wcs->naxisn[n] + 1;
                    countmin = countmax = 0;
                    for(i = 0; i < ninput; i++) {
...
/*---- Manual Image size */
            if (prefs.image_size[n]) {
                if (!center_flag)
                    wcs->crpix[n] -=
                        (wcs->naxisn[n] - prefs.image_size[n]) / 2;
                tab->naxisn[n] = wcs->naxisn[n] = prefs.image_size[n];
            }
        }
        if (center_flag) {
            ...
            for(n = 0; n < naxis; n++)
                wcs->crpix[n] +=
                    (int) (wcs->naxisn[n] / 2 - rawcenter[n] + 0.49);
        }
    }
 ...
 ***/
void
scale_field(fieldstruct * field, fieldstruct * reffield, int scaleflag)
{
    wcsstruct *wcs;
    static double raw[NAXIS], wcspos2[NAXIS];
    double *wcspos, *wcsscale, inscale, outscale;
    int i, naxis, lng, lat;
