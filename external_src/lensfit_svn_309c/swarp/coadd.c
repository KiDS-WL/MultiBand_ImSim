...
PIXTYPE *multibuf, *multiwbuf, *outbuf, *outwbuf, coadd_wthresh;
unsigned int *multinbuf;
int coadd_nomax, coadd_width;
char padbuf[FBSIZE];

#ifdef USE_THREADS
pthread_t *thread, movthread;
...
#ifdef USE_THREADS
static void *pthread_coadd_lines(void *arg), *pthread_move_lines(void *arg);
#endif
 ...
 ***/
int
coadd_fields(fieldstruct ** infield, fieldstruct ** inwfield, int ninput,
             fieldstruct * outfield, fieldstruct * outwfield,
             coaddenum coaddtype, PIXTYPE wthresh)
{
#ifdef USE_THREADS
    ...
    unsigned int *cflag, *array, d, n, n1, n2, flag;
...
/*---- Update input frame limits in output frame */
            wcs->outmin[d] =
                (int) floor(outfield->wcs->crpix[d] - wcs->crpix[d] + 1.0);
            if (wcs->outmin[d] < min)
...
/* Compute output saturation level (the minimum on all saturation) */
    ...
    for(n = 0; n < ninput; n++) {
        infield[n]->saturation =
            (prefs.subback_flag[n]) ? (infield[n]->saturation -
                                       infield[n]->backmean) : infield[n]->
            saturation;
...
/* Start the co-addition threads */
    for(p = 0; p < nproc; p++) {
        proc[p] = p;
        QPTHREAD_CREATE(&thread[p], &pthread_attr, &pthread_coadd_lines,
                        &proc[p]);
    }
...
/*-- Skip empty lines */
        for(d = naxis; --d;)
            rawpos2[d] = rawpos[d];
        nlinesmax = height - y;
        for(nbuflines = nlines = 0;
            nbuflines < nbuflinesmax && nlines < nlinesmax; nlines++) {
            for(d = naxis; --d;)
                    ...
                    if (!(cflag[n] & COADDFLAG_OPEN)) {
                        if (open_cat(infield[n]->cat, READ_ONLY) != RETURN_OK)
                            error(EXIT_FAILURE,
                                  ...
                                  infield[n]->filename);
                        if (inwfield[n]) {
                            if (open_cat(inwfield[n]->cat, READ_ONLY) !=
                                RETURN_OK)
                                error(EXIT_FAILURE,
                                      "*Error*: cannot open for reading ",
                                      inwfield[n]->filename);
...
/*-------- Refill the buffers with new data */
                    if (coadd_load(infield[n], inwfield[n],
                                   multibuf + dy * multiwidth,
                                   multiwbuf + dy * multiwidth,
                                   multinbuf + dy * outwidth, bufpos, bufmin,
                                   bufmax, nbuflines2, outwidth, omax)
                        != RETURN_OK)
...
/* FITS padding*/
    ...
    if (size)
        QFWRITE(padbuf, (size_t) size, outfield->cat->file,
                outfield->filename);
    size = PADEXTRA(outwfield->tab->tabsize);
    if (size)
        QFWRITE(padbuf, (size_t) size, outwfield->cat->file,
                outwfield->filename);
...
{
    int *all, *compsub, c, ncompsub, nmax;
...
/* Run the recursive part */
    max_clique_recur(array, nnode, all, 0, nnode, &compsub, &ncompsub, max,
                     &nmax);
 ...
 ***/
static void
max_clique_recur(unsigned int *array, int nnode, int *old,
                 int ne, int ce, int **compsub, int *ncompsub, int **best,
                 int *nbest)
{
    int *new, i, j, p, s, count, fixp, minnod, newce, newne, nod, pos, sel;
...
/*------ Found a max clique */
            ...
            max_clique_recur(array, nnode, new, newne, newce, compsub,
                             ncompsub, best, nbest);
/*-- Remove from "compsub" */
        (*ncompsub)--;
/*-- Add to "not" */
           ...
           int nbuflines, int outwidth, int multinmax)
{
    ...
    unsigned int *multinbuf2, d, y, nflag, naxis;
    int rawpos2[NAXIS], ival, inoffset, inbeg, muloffset, width;
#ifdef USE_THREADS
...
/*------ well now we are back within the frame; we might need to fseek() */
            ...
            coadd_movedata(line + inoffset,
                           multibuf + muloffset, multinbuf2 + inbeg, width,
                           multinmax);
#endif
...
/*-------- well now we are back withing the frame; we might need to fseek() */
                {
                    ...
                    QFSEEK(wfield->cat->file,
                           wfield->tab->bodypos +
                           offset * wfield->tab->bytepix, SEEK_SET,
                           wfield->filename);
                }
            ...
            coadd_movewdata(wfield ? (line + inoffset) : NULL,
                            multiwbuf + muloffset, multinbuf2 + inbeg, width,
                            multinmax);
#endif
...
pthread_move_lines(void *arg)
{
    ...
    while (!pthread_endflag) {
        if (pthread_wdataflag)
            coadd_movewdata(pthread_linebuf, pthread_multibuf,
                            pthread_multinbuf, pthread_npix, pthread_step);
        else
            coadd_movedata(pthread_linebuf, pthread_multibuf,
                           pthread_multinbuf, pthread_npix, pthread_step);
        threads_gate_sync(pthread_stopgate2);
 ...
 ***/
void
coadd_movedata(PIXTYPE * linebuf, PIXTYPE * multibuf,
               unsigned int *multinbuf, int npix, int step)
{
    PIXTYPE *pix, *multi;
    unsigned int *multin, x;
...
coadd_movewdata(PIXTYPE * linebuf, PIXTYPE * multiwbuf,
                unsigned int *multinbuf, int npix, int step)
{
    PIXTYPE *wpix, *multiw, val;
    unsigned int *multin, x, n;
