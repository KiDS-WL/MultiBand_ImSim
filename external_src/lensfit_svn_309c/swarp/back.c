...
*/
void
make_back(fieldstruct * field, fieldstruct * wfield)
{
    backstruct *backmesh, *wbackmesh, *bm, *wbm;
    tabstruct *tab, *wtab;
    PIXTYPE *buf, *wbuf, *buft, *wbuft;
    size_t bufsize, bufsize2, bufshift, size, meshsize, jumpsize;
...
/*---- Read and skip, read and skip, etc... */
            QFSEEK(tab->cat->file, bufshift * tab->bytepix, SEEK_CUR,
                   field->filename);
            buft = buf;
...
/*------ Read and skip, read and skip, etc... now on the weight-map */
                ...
                for(i = nlines; i--; wbuft += w) {
                    ...
                    if (i)
                        QFSEEK(wtab->cat->file, jumpsize * wtab->bytepix,
            ...
            for(m = nx; m--; bm++)
                if (bm->mean <= -BIG)
                    ...
                    QCALLOC(bm->histo, int, bm->nlevels);
            if (wfield) {
                QFSEEK(wtab->cat->file, wfcurpos2, SEEK_SET,
                       wfield->filename);
                wbm = wbackmesh;
                for(m = nx; m--; wbm++)
                    if (wbm->mean <= -BIG)
...
/*---- Build (progressively this time) the histograms */
            for(size = meshsize, bufsize2 = bufsize; size > 0;
                size -= bufsize2) {
                if (bufsize2 > size)
                    bufsize2 = size;
...
*/
void
backstat(backstruct * backmesh, backstruct * wbackmesh,
...
/*-- If not enough valid pixels, discard this mesh */
        ...
        if (wbackmesh) {
            wmean /= (double) npix;
            wsigma = (sig =
                      wsigma / npix - wmean * wmean) > 0.0 ? sqrt(sig) : 0.0;
        ...
        if (wbackmesh) {
            ...
            for(y = h; y--; buft += offset, wbuft += offset)
                for(x = bw; x--;) {
                    pix = *(buft++);
                    if ((wpix = *(wbuft++)) < wthresh && pix <= hcut
                        && pix >= lcut) {
                        mean += pix;
                        sigma += pix * pix;
                        npix++;
        ...
        bm->qscale =
            sigma > 0.0 ? 2 * QUANTIF_NSIGMA * sigma / bm->nlevels : 1.0;
        ...
        if (wbackmesh) {
            ...
            wbm->sigma = wsigma;
            if ((wbm->nlevels =
                 (int) (step * wnpix + 1)) > QUANTIF_NMAXLEVELS)
                wbm->nlevels = QUANTIF_NMAXLEVELS;
            wbm->qscale =
                wsigma >
                0.0 ? 2 * QUANTIF_NSIGMA * wsigma / wbm->nlevels : 1.0;
...
*/
void
backhisto(backstruct * backmesh, backstruct * wbackmesh,
          PIXTYPE * buf, PIXTYPE * wbuf, size_t bufsize,
          int n, int w, int bw, PIXTYPE wthresh)
{
    backstruct *bm, *wbm;
    PIXTYPE *buft, *wbuft, pix;
...
/* Leave here if the mesh is already classified as `bad' */
    ...
    for(n = 100; n-- && (sig >= 0.1) && (fabs(sig / sig1 - 1.0) > EPS);) {
        ...
        med = hihigh >= histo ?
            ((hihigh - histo) + 0.5 +
             ((double) highsum -
              lowsum) / (2.0 * (*hilow > *hihigh ? *hilow : *hihigh)))
            : 0.0;
        ...
        lcut = (ftemp =
                med - 3.0 * sig) > 0.0 ? (int) (ftemp >
                                                0.0 ? ftemp + 0.5 : ftemp -
                                                0.5) : 0;
        hcut = (ftemp =
                med + 3.0 * sig) < nlevelsm1 ? (int) (ftemp >
                                                      0.0 ? ftemp +
                                                      0.5 : ftemp - 0.5)
            : nlevelsm1;
    }
    ...
    *mean =
        fabs(sig) > 0.0 ? (fabs(bkg->sigma / (sig * bkg->qscale) - 1) <
                           0.0 ? bkg->qzero +
                           mea * bkg->qscale : (fabs((mea - med) / sig) <
                                                0.3 ? bkg->qzero +
                                                (2.5 * med -
                                                 1.5 * mea) *
                                                bkg->qscale : bkg->qzero +
                                                med * bkg->qscale))
...
*/
void
filter_back(fieldstruct * field)
{
    float *back, *sigma, *back2, *sigma2, *bmask, *smask, *sigmat,
        d2, d2min, fthresh, med, val, sval;
    int i, j, px, py, np, nx, ny, npxm, npxp, npym, npyp, dpx, dpy, x, y,
        nmin;
...
*/
float *
make_backspline(fieldstruct * field, float *map)
{
...
*/
void
backline(fieldstruct * field, int y, PIXTYPE * line)
{
 ...
 ***/
void
backrmsline(fieldstruct * field, int y, PIXTYPE * line)
{
    int i, j, x, yl, nbx, nbxm1, nby, nx, width, ystep, changepoint;
    float dx, dx0, dy, dy3, cdx, cdy, cdy3, temp, xstep,
        *node, *nodep, *dnode, *blo, *bhi, *dblo, *dbhi, *u;
