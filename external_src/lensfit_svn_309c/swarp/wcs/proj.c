...

{
    ...
    if (prj->w[1] == 0.0) {
        ...
        if (r2 < 1.0e-10) {
            ...
            *theta =
                90.0 -
                R2D * sqrt(r2 / (1.0 - x0 * prj->p[1] + y0 * prj->p[2]));
...

{
    ...
    *theta =
        wcs_atan2d(eta,
                   1.0) + wcs_asind(eta * prj->p[1] / sqrt(eta * eta + 1.0));
...

{
    ...
    if (prj->p[2] == 0.0) {
        ...
        prj->w[0] =
            prj->r0 * wcs_sind(prj->p[1]) * wcs_sind(prj->p[2]) / prj->p[2];
    }
    ...
    prj->w[1] = 1.0 / prj->w[0];
    prj->w[2] =
        prj->r0 * wcs_cosd(prj->p[2]) * wcs_cosd(prj->p[1]) / prj->w[0];
...

{
    ...
    prj->w[2] =
        prj->w[3] * pow(wcs_tand((90.0 - prj->p[1]) / 2.0), prj->w[0]);
    prj->w[4] = 1.0 / prj->w[3];
...

{
    if (prj->r0 == 0.0) {
        ...
        prj->w[1] = 1.0;
        prj->w[2] =
            prj->r0 * wcs_cosd(prj->p[1]) / wcs_sind(prj->p[1]) + prj->p[1];
    } else {
        prj->w[1] = prj->r0 * D2R;
        prj->w[2] =
            prj->r0 * (wcs_cosd(prj->p[1]) / wcs_sind(prj->p[1]) +
                       prj->p[1] * D2R);
    }
...

{
    ...
    a = z0 + yy * (z1 +
                   yy * (z2 + yy * (z3 + yy * (z4 + yy * (z5 + yy * z6)))));
    a = xf + xf * (1.0 - xx) * a;

    z0 = p00 + yy * (p10 +
                     yy * (p20 +
                           yy * (p30 +
                                 yy * (p40 + yy * (p50 + yy * (p60))))));
...

{
    ...
    if (xi == 0.0 && eta == 0.0) {
        ...
        yf = (xf / 15.0) * (wcs_atand(psi) -
                            wcs_asind(psi / sqrt(chi + chi)));
    } else if (xi >= fabs(eta)) {
        psi = eta / xi;
        chi = 1.0 + psi * psi;
        xf = sqrt(rhu / (1.0 - 1.0 / sqrt(1.0 + chi)));
...
raw_to_pv(struct prjprm *prj, double x, double y, double *xo, double *yo)
{
    ...
    if (!--k)
        goto poly_end;
    xp += *(a++) * x2 * y4;
    // the following line has a known bug and therefore
