...
read_tnxaxis(char *tnxstr)
{
    ...
    if ((pstr = strpbrk(tnxstr, "1234567890-+."))) {
        ...
        case TNX_XHALF:
            ...
            tnxaxis->ncoeff =
                tnxaxis->xorder * tnxaxis->yorder - order * (order - 1) / 2;
            break;
        case TNX_XFULL:
...
/*-- Now read the mapping coefficients */
        if (!(tnxaxis->coeff = malloc(tnxaxis->ncoeff * sizeof(double))))
            return NULL;
        for(i = 0; i < tnxaxis->ncoeff && (pstr = strtok_r(NULL, " ", &ptr));
            i++)
...
tnxaxisstruct *
copy_tnxaxis(tnxaxisstruct * axis)
{
    tnxaxisstruct *tnxaxis;
    int i;
...
void
free_tnxaxis(tnxaxisstruct * axis)
{
    if (axis) {
...
double
raw_to_tnxaxis(tnxaxisstruct * axis, double x, double y)
{
    double *xbasis, *ybasis, *coeff, norm, accum, val;
