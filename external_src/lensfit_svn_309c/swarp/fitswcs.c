 ...
 ***/
wcsstruct *
create_wcs(char **ctype, double *crval, double *crpix,
           double *cdelt, int *naxisn, int naxis)
{
    wcsstruct *wcs;
    int l;
...
/* Find the celestial system */
    for(s = 0;
        *celsysname[s][0] && strncmp(wcs->ctype[lng], celsysname[s][0], 4);
        s++);
...
#define	FITSREADS(buf, k, str, def) \
    ...
    double drota;
    int j, l, naxis;
    char name[16], *buf, *filename, *ptr;

    buf = tab->headbuf;
    filename =
        (tab->cat ? tab->cat->filename : strcpy(name, "internal header"));
    ...
    if (tab->naxis > NAXIS) {
        warning
            ("Maximum number of dimensions supported by this version of the ",
             "software exceeded\n");
        tab->naxis = 2;
    }
...
/*-- If CD keywords exist, use them for the linear mapping terms... */
        for(l = 0; l < naxis; l++)
            for(j = 0; j < naxis; j++) {
                ...
                FITSREADF(buf, str, wcs->cd[l * naxis + j],
                          l == j ? 1.0 : 0.0)
            }
...
/*------ Decode DATE-OBS format: DD/MM/YY or YYYY-MM-DD */
                for(l = 0;
                    l < 3 && (pstr = strtok_r(l ? NULL : str, "/- ", &ptr));
                    l++)
                    dpar[l] = atoi(pstr);
...
/*------ Well if really no date is found */
        ...
        FITSREADS(buf, "RADECSYS", str,
                  wcs->equinox >= 2000.0 ? "ICRS" : (wcs->equinox <
                                                     1984.0 ? "FK4" : "FK5"));
        if (!strcmp(str, "ICRS"))
            ...
            warning
                ("Using ICRS instead of unknown astrometric reference frame: ",
                 str);
            wcs->radecsys = RDSYS_ICRS;
        }
...
/*------ First we need to concatenate strings */
                ...
                for(j = 2;
                    fitsread(buf, str, pstr, H_STRINGS,
                             T_STRING) == RETURN_OK; j++) {
...
/*------ Read the remaining TNX parameters */
                ...
                if (!wcs->tnx_lngcor)
                    error(EXIT_FAILURE,
                          "*Error*: incorrect TNX parameters in ", filename);
...
write_wcs(tabstruct * tab, wcsstruct * wcs)
{
    ...
    for(l = 0; l < naxis; l++) {
        ...
        for(j = 0; j < naxis; j++) {
            ...
            fitswrite(tab->headbuf, str, &wcs->cd[l * naxis + j], H_EXPO,
                      T_DOUBLE);
        }
        for(j = 0; j < 100; j++)
            if (wcs->projp[j + 100 * l] != 0.0) {
                ...
                fitswrite(tab->headbuf, str, &wcs->projp[j + 100 * l], H_EXPO,
                          T_DOUBLE);
            }
    }
...
{
    char projcode[26][5] =
        { "AZP", "TAN", "SIN", "STG", "ARC", "ZPN", "ZEA", "AIR", "CYP",
"CAR",
...
invert_wcs(wcsstruct * wcs)
{
    ...
    double *outpos, *outpost, *lngpos, *lngpost,
        *latpos, *latpost, lngstep, latstep, rawsize, epsilon;
    int group[] = { 1, 1 };
    /*
    ...
    for(j = WCS_NGRIDPOINTS; j--; raw[lat] += latstep) {
        ...
        for(i = WCS_NGRIDPOINTS; i--; raw[lng] += lngstep) {
            if (linrev(raw, wcs->lin, pixin))
                error(EXIT_FAILURE,
                      "*Error*: incorrect linear conversion in ",
                      wcs->wcsprm->pcode);
            *(lngpost++) = pixin[lng];
            *(latpost++) = pixin[lat];
            if (tnxflag) {
...
range_wcs(wcsstruct * wcs)
{
    ...
    double *worldmin, *worldmax, *scale, *worldc, rad, radmax, lc;
    int linecount[NAXIS];
...
frame_wcs(wcsstruct * wcsin, wcsstruct * wcsout)
{
    ...
    int linecount[NAXIS];
    double worldc;
    int *min, *max, i, j, naxis, npoints, out, swapflag;
    ...
    for(j = npoints; j--;) {
        ...
        for(i = 0; i < naxis; i++) {
            rawin[i] = 0.5 + 0.5 * wcsin->naxisn[i]
                * (1 -
                   cos(PI * (linecount[i] + 1.0) / (WCS_NRANGEPOINTS - 1)));
            if (++linecount[i] < WCS_NRANGEPOINTS)
                break;
            else {
                linecount[i] = 0;       /* No need to initialize it to 0! */
 ...
 ***/
int
celsys_to_eq(wcsstruct * wcs, double *wcspos)
{
    double *mat, a2, d2, sd2, cd2cp, sd, x, y;
    int lng, lat;
...
{
    double *mat, a, d, sd2, cdcp, sd, x, y;
    int lng, lat;
 ...
 ***/
int
raw_to_wcs(wcsstruct * wcs, double *pixpos, double *wcspos)
{
    double imgcrd[NAXIS], phi, theta;
    int i;

    if (wcsrev((const char (*)[9]) wcs->ctype, wcs->wcsprm, pixpos,
               wcs->lin, imgcrd, wcs->prj, &phi, &theta, wcs->crval, wcs->cel,
               wcspos)) {
        for(i = 0; i < wcs->naxis; i++)
 ...
 ***/
int
wcs_to_raw(wcsstruct * wcs, double *wcspos, double *pixpos)
{
    double imgcrd[NAXIS], phi, theta;
    int i;
...
/* If needed, convert to a coordinate system different from equatorial */
    ...
    if (wcsfwd((const char (*)[9]) wcs->ctype, wcs->wcsprm, wcspos,
               wcs->crval, wcs->cel, &phi, &theta, wcs->prj, imgcrd, wcs->lin,
               pixpos)) {
        for(i = 0; i < wcs->naxis; i++)
 ...
 ***/
int
red_to_raw(wcsstruct * wcs, double *redpos, double *pixpos)
{
 ...
 ***/
int
raw_to_red(wcsstruct * wcs, double *pixpos, double *redpos)
{
    struct wcsprm *wcsprm;
    double offset;
    int face;
...
/* Compute pixel scale */
    ...
    if (wcs->lng != wcs->lat) {
        ...
        return fabs((dpos1 * (wcspos2[lat] - wcspos[lat])
                     - (wcspos1[lat] -
                        wcspos[lat]) * dpos2) * cos(wcspos[lat] * DEG));
    } else
        return fabs((dpos1 * (wcspos2[lat] - wcspos[lat])
 ...
 ***/
double
wcs_jacobian(wcsstruct * wcs, double *pixpos, double *jacob)
{
    double pixpos0[NAXIS], wcspos0[NAXIS], wcspos[NAXIS], dpos;
 ...
 ***/
void
precess_wcs(wcsstruct * wcs, double yearin, double yearout)
{
    double crval[NAXIS], a[NAXIS * NAXIS], b[NAXIS * NAXIS],
        *c, *at, val, cas, sas, angle, dalpha;
...
*/
void
b2j(double yearobs, double alphain, double deltain,
    double *alphaout, double *deltaout)
{
    int i, j;
    double a[3] = { -1.62557e-6, -0.31919e-6, -0.13843e-6 }, ap[3] = {
    1.245e-3, -1.580e-3, -0.659e-3}, m[6][6] = {
        {
    ...
    double *alphaout, double *deltaout)
{
    ...
    1.245e-3, -1.580e-3, -0.659e-3}, m[6][6] = {
        ...
        0.9999256794678425, 0.01118148281196562, 0.004859003848996022,
                -2.423898417033081e-06, -2.710547600126671e-08,
                -1.177738063266745e-08}, {
        -0.01118148272969232, 0.9999374849247641, -2.717708936468247e-05,
                2.710547578707874e-08, -2.423927042585208e-06,
                6.588254898401055e-11}, {
        -0.00485900399622881, -2.715579322970546e-05, 0.999988194643078,
                1.177738102358923e-08, 6.582788892816657e-11,
                -2.424049920613325e-06}, {
        -0.0005508458576414713, 0.2384844384742432, -0.4356144527773499,
                0.9999043171308133, 0.01118145410120206,
                0.004858518651645554}, {
        -0.2385354433560954, -0.002664266996872802, 0.01225282765749546,
                -0.01118145417187502, 0.9999161290795875,
                -2.717034576263522e-05}, {
    0.4357269351676567, -0.008536768476441086, 0.002113420799663768,
                -0.004858518477064975, -2.715994547222661e-05,
                0.9999668385070383}}, a1[3], r[3], ro[3], r1[3], r2[3], v1[3],
        v[3];
    double cai, sai, cdi, sdi, dotp, rmod, alpha, delta, t1;

/* Convert Julian years from J2000.0 to tropic centuries from B1950.0 */
    t1 = ((yearobs - 2000.0) +
          (MJD2000 - MJD1950) / 365.25) * JU2TROP / 100.0;
    alphain *= DEG;
    deltain *= DEG;
    cai = cos(alphain);
    sai = sin(alphain);
...
*/
char *
degtosexal(double alpha, char *str)
{
    int hh, mm;
    double ss;
