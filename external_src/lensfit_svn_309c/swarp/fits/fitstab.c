...
about_tab(catstruct * cat, char *tabname, FILE * stream)
{
    ...
    if ((tab = name_to_tab(cat, tabname, 0))) {
        ...
        for(i = 0, key = tab->key; i < tab->nkey; i++, key = key->nextkey) {
            ...
            fprintf(stream, "	Key name:...............%s\n", key->name);
            fprintf(stream, "	Key comment:............%s\n", key->comment);
            fprintf(stream, "	Key type:...............");
            switch (key->ttype) {
            case T_BYTE:
                fprintf(stream, "Byte");
                break;
            ...
            fprintf(stream, "\n");
            fprintf(stream, "	Key dimension:..........%d ", key->naxis);
            if (key->naxis)
                fprintf(stream, "(");
...
/*--update status for each table segment*/
    {
        ...
        for(nexttab = tab->nexttab, i = 2; !nexttab->nseg;
            nexttab = nexttab->nexttab, i++);
        nexttab->seg = i;
        tab->nseg = i;
        tab->seg = 1;
    }
 ...
 ***/
int
tab_row_len(char *file, char *tabname)
{
    catstruct *tcat;
    tabstruct *tab;
    int retcode = -1;
