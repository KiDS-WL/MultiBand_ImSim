 ...
 ***/
int
copy_key(tabstruct * tabin, char *keyname, tabstruct * tabout, int pos)
{
 ...
 ***/
void
read_keys(tabstruct * tab, char **keynames, keystruct ** keys, int nkeys,
          BYTE * mask)
{
    catstruct *cat;
    keystruct *key, **ckeys;
 ...
 ***/
void
show_keys(tabstruct * tab, char **keynames, keystruct ** keys, int nkeys,
          BYTE * mask, FILE * stream,
...
/* Allocate memory for the arrays */
    ...
    if (keynames)
        for(i = nkeys; i--;) {
            if ((key = name_to_key(tab, *(keynames++)))) {
                for(map = objectmap; map->oldname[0] && o_type == SHOW_SKYCAT;
                    map++) {
