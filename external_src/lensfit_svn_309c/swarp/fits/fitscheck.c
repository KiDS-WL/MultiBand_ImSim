...
{
    int ch[4], i, j, k, byte, check;

    for(i = 0; i < 4; i++) {
/*-- Each byte becomes four */
        byte = (sum << 8 * i) >> 24;
        ch[0] = (ch[1] = ch[2] = ch[3] =
                 byte / 4 + ENCODE_OFFSET) + (byte % 4);
...
/* Remove the permuted FITS byte alignment and the ASCII 0 offset */
    ...
    if (bswapflag)
        for(i = 4; i--;) {
            ...
            los = *(sbuf++);
            hi +=
                (*((unsigned char *) &his) << 8) + *((unsigned char *) &his +
                                                     1);
            lo +=
                (*((unsigned char *) &los) << 8) + *((unsigned char *) &los +
                                                     1);
    } else
        for(i = 4; i--;) {
            hi += *(sbuf++);
...
/* Keep only what's necessary */
    tab->headnblock =
        ((fitsfind(tab->headbuf, "END     ") + 36) * 80) / FBSIZE;
