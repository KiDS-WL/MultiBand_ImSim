 ...
 ***/
tabstruct *
asc2bin_tab(catstruct * catin, char *tabinname, catstruct * catout,
            char *taboutname)
{
    catstruct *tabcat;
    keystruct *key;
...
/*Interprete the next FITS line */
        if (fitspick(lptr, keyword, ptr, &htype, &ttype, comment) !=
            RETURN_OK) {
            char line[81];
            int qflag = 1;
...
/*----Allocate memory and copy data in the same time*/
            ...
            if (add_key(key, tabout, 0) == RETURN_ERROR) {
                sprintf(comment, "%s keyword found twice in ", keyword);
...
void
ttypeconv(void *ptrin, void *ptrout, t_type ttypein, t_type ttypeout)
{
    union {
        char tbyte;
        short tshort;
