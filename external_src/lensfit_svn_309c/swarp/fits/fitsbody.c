...
size_t body_maxram = BODY_DEFRAM,
    body_maxvram = BODY_DEFVRAM, body_ramleft, body_vramleft, body_ramflag;
 ...
 ***/
PIXTYPE *
alloc_body(tabstruct * tab, void (*func) (PIXTYPE * ptr, int npix))
{
    FILE *file;
    PIXTYPE *buffer;
...
/*-- There should be enough RAM left: try to do a malloc() */
        if ((tab->bodybuf = malloc(size))) {
            QFSEEK(tab->cat->file, tab->bodypos, SEEK_SET,
                   tab->cat->filename);
...
/*-- Convert and copy the data to a swap file, and mmap() it */
        ...
        if (!(file = fopen(tab->swapname, "wb+")))
            error(EXIT_FAILURE, "*Error*: cannot create swap-file ",
                  tab->swapname);
...
/*-- Apply pixel processing */
        ...
        for(sizeleft = size; sizeleft -= spoonful;) {
            read_body(tab, buffer,
                      (spoonful = DATA_BUFSIZE) / sizeof(PIXTYPE));
/*--- Apply pixel processing */
            if (func)
                (*func) (buffer, spoonful / sizeof(PIXTYPE));
            QFWRITE(buffer, spoonful, file, tab->swapname);
        ...
        free(buffer);
        tab->bodybuf =
            mmap(NULL, size, PROT_READ, MAP_SHARED, fileno(file), (off_t) 0);
        fclose(file);
        tab->swapflag = 1;
        body_vramleft -= size;
...
/*-- Uncompressed image */
    case COMPRESS_NONE:
        ...
        for(; size > 0; size -= spoonful) {
            ...
            QFREAD(bufdata, spoonful * tab->bytepix, cat->file,
                   cat->filename);
            switch (tab->bitpix) {
            case BP_BYTE:
                if (blankflag) {
                    ...
                    if (tab->bitsgn) {
                        ...
                        for(i = spoonful; i--; bufdata += sizeof(short))
                            *(ptr++) =
                                ((sval =
                                  *((short *) bufdata)) ==
                                 sblank) ? -BIG : sval * bs + bz;
                    } else {
                        sublank = (unsigned short) tab->blank;
                        for(i = spoonful; i--;
                            bufdata += sizeof(unsigned short))
                            *(ptr++) =
                                ((suval =
                                  *((unsigned short *) bufdata)) ==
                                 sublank) ? -BIG : suval * bs + bz;
                    }
                } else {
                    if (tab->bitsgn)
                        for(i = spoonful; i--; bufdata += sizeof(short))
                    ...
                    if (tab->bitsgn) {
                        ...
                        for(i = spoonful; i--; bufdata += sizeof(int))
                            *(ptr++) =
                                ((ival =
                                  *((int *) bufdata)) ==
                                 iblank) ? -BIG : ival * bs + bz;
                    } else {
                        iublank = (unsigned int) tab->blank;
                        for(i = spoonful; i--;
                            bufdata += sizeof(unsigned int))
                            *(ptr++) =
                                ((iuval =
                                  *((unsigned int *) bufdata)) ==
                                 iublank) ? -BIG : iuval * bs + bz;
                    }
                } else {
                    if (tab->bitsgn)
                        for(i = spoonful; i--; bufdata += sizeof(int))
            ...
            case BP_FLOAT:
                ...
                for(i = spoonful; i--; bufdata += sizeof(float))
                    *(ptr++) =
                        ((0x7f800000 & *(unsigned int *) bufdata) ==
                         0x7f800000) ? -BIG : *((float *) bufdata) * bs + bz;
                break;
            ...
            case BP_DOUBLE:
                if (bswapflag) {
                    ...
                    for(i = spoonful; i--; bufdata += sizeof(double))
                        *(ptr++) =
                            ((0x7ff00000 & *(unsigned int *) (bufdata + 4))
                             == 0x7ff00000) ? -BIG : *((double *) bufdata) *
                            bs + bz;
                } else {
                    for(i = spoonful; i--; bufdata += sizeof(double))
        ...
        while (size--) {
            if (!(npix--)) {
                if (curval != tab->compress_checkval)
                    error(EXIT_FAILURE,
                          "*Error*: invalid BASEBYTE checksum in ",
                          cat->filename);
    ...
    case COMPRESS_PREVPIX:
        ...
        while (size--) {
            if (!(npix--)) {
                if (curval != tab->compress_checkval)
                    error(EXIT_FAILURE,
                          "*Error*: invalid PREV_PIX checksum in ",
                          tab->cat->filename);
                bufdata = tab->compress_buf;
                QFREAD(bufdata, FBSIZE, cat->file, cat->filename);
                if (bswapflag)
...
write_body(tabstruct * tab, PIXTYPE * ptr, size_t size)
{
    ...
    if (!cat)
        error(EXIT_FAILURE,
              "*Internal Error*: no parent cat structure for table ",
              tab->extname);
        ...
        for(; size > 0; size -= spoonful) {
            ...
            case BP_BYTE:
                if (tab->bitsgn) {
                    ...
                    for(i = spoonful; i--;)
                        *(bufdata++) =
                            (char) ((*(ptr++) - bz) / bs + 0.49999);
                } else {
                    unsigned char *bufdata = (unsigned char *) cbufdata0;
                    for(i = spoonful; i--;)
                        *(bufdata++) =
                            (unsigned char) ((*(ptr++) - bz) / bs + 0.49999);;
                }
                break;
            ...
            case BP_SHORT:
                if (tab->bitsgn) {
                    ...
                    for(i = spoonful; i--;)
                        *(bufdata++) =
                            (short) ((*(ptr++) - bz) / bs + 0.49999);
                } else {
                    unsigned short *bufdata = (unsigned short *) cbufdata0;
                    for(i = spoonful; i--;)
                        *(bufdata++) =
                            (unsigned short) ((*(ptr++) - bz) / bs + 0.49999);
                }
                if (bswapflag)
            ...
            case BP_LONG:
                if (tab->bitsgn) {
                    ...
                    for(i = spoonful; i--;)
                        *(bufdata++) =
                            (unsigned int) ((*(ptr++) - bz) / bs + 0.49999);
                }
                if (bswapflag)
            ...
            QFWRITE(cbufdata0, spoonful * tab->bytepix, cat->file,
                    cat->filename);
        }
        break;
