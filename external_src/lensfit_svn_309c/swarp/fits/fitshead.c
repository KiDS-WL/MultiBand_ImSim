...
/*in case of a non-primary header*/
    ...
    tab->tabsize =
        tab->bytepix * tab->gcount * ((size_t) tab->pcount + tabsize);
...
/* Scaling parameters for basic FITS integer arrays */
    ...
    tab->blankflag =
        (fitsread(tab->headbuf, "BLANK   ", &tab->blank, H_INT, T_LONG) ==
         RETURN_OK) ? 1 : 0;
...
/* Custom basic FITS parameters */
    ...
    if (fitsread(tab->headbuf, "IMAGECOD", str, H_STRING, T_STRING) ==
        RETURN_OK) {
        if (!strcmp(str, "NONE"))
...
/* Checksum */
    if (fitsread(tab->headbuf, "DATASUM ", str, H_STRING, T_STRING) ==
        RETURN_OK)
...
/*--map binary-table fields*/
        ...
        if (fitsread(tab->headbuf, strk, key->name, H_STRING, T_STRING)
            != RETURN_OK) {
            error(EXIT_FAILURE,
                  "*Error*: Incorrect FITS binary-table header in ",
                  cat->filename);
        }
        ...
        if (fitsread(tab->headbuf, strk, strf, H_STRING, T_STRING) !=
            RETURN_OK) {
            error(EXIT_FAILURE,
                  "*Error*: Incorrect FITS binary-table header in ",
                  cat->filename);
...
/*--handle the special case of multimensional arrays*/
        if ((naxisn[0] = key->nbytes / t_size[key->ttype]) > 1) {
            ...
            if (fitsread(tab->headbuf, strk, strf, H_STRING, T_STRING) ==
                RETURN_OK) {
                str = strf;
                for(j = 0; (naxisn[j] = (int) strtol(str + 1, &str, 10));
                    j++);
                key->naxis = j;
            } else
                key->naxis = 1;
    ...
    if (pos != larray)
        error(EXIT_FAILURE,
              "*Error*: Malformed FITS binary-table header in ",
              cat->filename);
...
addkeywordto_head(tabstruct * tab, char *keyword, char *comment)
{
    ...
    if ((fitsfind(tab->headbuf, keyword) == RETURN_ERROR
        ...
        && (fitsfind(tab->headbuf, "END     ") + 1) * 80 >=
        tab->headnblock * FBSIZE) {
        tab->headnblock++;
...
removekeywordfrom_head(tabstruct * tab, char *keyword)
{
    ...
    if (fitsremove(tab->headbuf, keyword) == RETURN_OK) {
        if ((nb =
             fitsfind(tab->headbuf,
                      "END     ") / (FBSIZE / 80) + 1) < tab->headnblock) {
            tab->headnblock = nb;
            QREALLOC(tab->headbuf, char, tab->headnblock * FBSIZE);
        }
        return RETURN_OK;
 ...
 ***/
char *
tdisptoprintf(char *tdisp, char *str)
{
    char control[4];
    int w, d, n;
