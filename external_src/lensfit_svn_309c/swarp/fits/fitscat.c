 ...
 ***/
int
about_cat(catstruct * cat, FILE * stream)
{
    tabstruct *tab;
    int i;
...
/*General info about the catalog itself*/
    ...
    fprintf(stream, "Number of segments:....%d\n", cat->ntab);
    fprintf(stream, "\n");

/*Now for each table*/
    tab = cat->tab;
    for(i = 0; i < cat->ntab; i++) {
        fprintf(stream, "******	Table #%d\n", i + 1);
        fprintf(stream,
                "	Extension type:.........%s\n",
                tab->xtension[0] ? tab->xtension : "(Primary HDU)");
        fprintf(stream, "	Extension name:.........%s\n", tab->extname);
        if (tab->naxis) {
            fprintf(stream, "	Number of dimensions:...%d\n", tab->naxis);
            fprintf(stream,
...
/*copy only one table: well it could be simpler, but let's stay general!*/
    ...
    for(j = tabin->nseg; j--;) {
        ...
        *tabout = *tabin;
        if (tabin->naxis)
            QMEMCPY(tabin->naxisn, tabout->naxisn, int,
                    (size_t) tabin->naxis);
        if (tabin->headbuf)
            QMEMCPY(tabin->headbuf, tabout->headbuf, char,
                    tabin->headnblock * FBSIZE);
        if (tabin->bodybuf)
            QMEMCPY(tabin->bodybuf, tabout->bodybuf, char,
                    (size_t) tabin->tabsize);
        if (prevtabout) {
...
/*scan through the file until we reach the end*/
    ...
    for(ntab = 0; !get_head(tab); ntab++) {
        ...
        tab->nseg = tab->seg = 1;
        if (tab->tabsize)
            QFSEEK(cat->file, PADTOTAL(tab->tabsize), SEEK_CUR,
                   cat->filename);
        if (prevtab) {
            tab->prevtab = prevtab;
            prevtab->nexttab = tab;
...
open_cat(catstruct * cat, access_type at)
{
    ...
    if (!cat->file) {
        if ((cat->file =
             fopen(cat->filename, at == WRITE_ONLY ? "wb" : "rb")) == NULL)
            return RETURN_ERROR;
        cat->access_type = at;
    }
