...
/* Scan keys to find the reference tab and other things*/
    ...
    if (!keytab)
        error(EXIT_FAILURE,
              "*Error*: no original table found among keys in table ",
              tab->extname);
 ...
 ***/
int
read_obj_at(tabstruct * keytab, tabstruct * tab, char *buf, long pos)
{
    keystruct *key;
    char *pin, *pout;
    size_t n;
    ...
    if ((n = keytab->naxisn[0] * pos) >= keytab->tabsize)
        return RETURN_ERROR;
    QFSEEK(keytab->cat->file, keytab->bodypos + n, SEEK_SET,
           keytab->cat->filename);
    QFREAD(buf, keytab->naxisn[0], keytab->cat->file, keytab->cat->filename);
    key = tab->key;
    for(k = tab->nkey; k--; key = key->nextkey)
