...
/*---- If table contains some keys with no ptrs, we have to access a file */
            if (keytab) {
                ...
                if (open_cat(tabcat, READ_ONLY) != RETURN_OK)
                    error(EXIT_FAILURE, "*Error*: Cannot access ",
                          tabcat->filename);
                QFSEEK(tabcat->file, keytab->bodypos, SEEK_SET,
                       tabcat->filename);
            }
            nobj = tab->naxisn[1];
            for(o = 0; o < nobj; o++) {
                ...
                for(k = nkey; k--; key = key->nextkey) {
                    nbytes = key->nbytes;
                    ptr =
                        key->ptr ? (char *) key->ptr + nbytes * o : inbuf +
                        key->pos;
                    for(b = nbytes; b--;)
                        *(fptr++) = *(ptr++);
                    if (bswapflag)
...
/*------ A body is present in memory and needs to be written */
                if (bswapflag)
                    swapbytes(tab->bodybuf, tab->bytepix,
                              tabsize / tab->bytepix);
                QFWRITE(tab->bodybuf, (size_t) tabsize, cat->file,
                        cat->filename);
                if (bswapflag)
                    swapbytes(tab->bodybuf, tab->bytepix,
                              tabsize / tab->bytepix);
...
/*------ The body should be copied from the source tab */
            {
                ...
                if (open_cat(tabcat, READ_ONLY) != RETURN_OK)
                    error(EXIT_FAILURE, "*Error*: Cannot access ",
                          tabcat->filename);
                QFSEEK(tabcat->file, tab->bodypos, SEEK_SET,
                       tabcat->filename);
...
/* Scan keys to check that memory has been allocated */
    ...
    for(k = tab->nkey; k--; key = key->nextkey)
        if (!key->ptr)
            error(EXIT_FAILURE, "*Error*: no memory allocated for ",
                  key->name);
...
print_obj(FILE * stream, tabstruct * tab)
{
    ...
    if (!(key = tab->key))
        error(EXIT_FAILURE, "*Error*: no key to print in table ",
              tab->extname);
