 ...
 ***/
int
read_aschead(char *filename, int frameno, tabstruct * tab)
{
...
/*---- Always keep a one-line margin */
            if ((pos + 1) * 80 >= tab->headnblock * FBSIZE) {
                ...
                memset(tab->headbuf + (tab->headnblock - 1) * FBSIZE, ' ',
                       FBSIZE);
            }
 ...
 ***/
void
readfitsinfo_field(fieldstruct * field, tabstruct * tab)
{
 ...
 ***/
void
writefitsinfo_outfield(fieldstruct * field, fieldstruct * infield)
{
    struct tm *tm;
    time_t thetime;
    tabstruct *tab, *intab;
...
/* COMBINE_TYPE */
    ...
    fitswrite(tab->headbuf, "COMBINET", key[findkeys("COMBINE_TYPE", keylist,
                                                     FIND_STRICT)].
              keylist[prefs.coadd_type], H_STRING, T_STRING);
    ...
    if (infield && (intab = infield->tab) && prefs.ncopy_keywords) {
        ...
        for(k = 0; k < prefs.ncopy_keywords; k++) {
            if ((l =
                 fitsfind(intab->headbuf,
                          prefs.copy_keywords[k])) != RETURN_ERROR) {
                fitspick(intab->headbuf + l * 80, prefs.copy_keywords[k],
                         gstr, &htype, &ttype, comment);
                addkeywordto_head(tab, prefs.copy_keywords[k], comment);
                fitswrite(tab->headbuf, prefs.copy_keywords[k], gstr, htype,
                          ttype);
            }
        }
    }
...
/*-- RESAMPLING_TYPE */
        ...
        fitswrite(tab->headbuf, gstr, key[findkeys("RESAMPLING_TYPE", keylist,
                                                   FIND_STRICT)].
                  keylist[prefs.resamp_type[d]], H_STRING, T_STRING);
/*-- CENTER_TYPE */
        ...
        fitswrite(tab->headbuf, gstr, key[findkeys("CENTER_TYPE", keylist,
                                                   FIND_STRICT)].
                  keylist[prefs.center_type[d]], H_STRING, T_STRING);
/*-- PIXELSCALE_TYPE */
        ...
        fitswrite(tab->headbuf, gstr, key[findkeys("PIXELSCALE_TYPE", keylist,
                                                   FIND_STRICT)].
                  keylist[prefs.pixscale_type[d]], H_STRING, T_STRING);
    }
...
/* Image-dependent config parameters */
    if (prefs.writefileinfo_flag) {
        addkeywordto_head(tab, "COMMENT ", "");
        addkeywordto_head(tab, "COMMENT ",
                          "File-dependent config parameters");
...
/*------ WEIGHT_TYPE */
                ...
                fitswrite(tab->headbuf, gstr,
                          key[findkeys("WEIGHT_TYPE", keylist, FIND_STRICT)].
                          keylist[prefs.weight_type[n]], H_STRING, T_STRING);
            }
/*---- Interpolation flag */
            sprintf(gstr, "INTF%04d", n + 1);
            addkeywordto_head(tab, gstr, "INTERPOLATE config flag");
            fitswrite(tab->headbuf, gstr, &prefs.interp_flag[n], H_BOOL,
                      T_LONG);
/*---- Background-subtraction flag */
            sprintf(gstr, "SUBF%04d", n + 1);
            addkeywordto_head(tab, gstr, "SUBTRACT_BACK config flag");
            fitswrite(tab->headbuf, gstr, &prefs.subback_flag[n], H_BOOL,
                      T_LONG);
/*---- BACK_TYPE */
...
/*---- BACK_FILTERSIZE */
            ...
            fitswrite(tab->headbuf, gstr, &prefs.back_fsize[n], H_INT,
                      T_LONG);
        }
    }
 ...
 ***/
void
writefitsinfo_field(fieldstruct * field, fieldstruct * infield)
{
    struct tm *tm;
    time_t thetime;
    tabstruct *tab, *intab;
...
/* Flux scales */
    ...
    fitswrite(tab->headbuf, prefs.fscale_keyword, &field->fscale, H_EXPO,
              T_DOUBLE);
...
/* Gain (conversion factor) */
    ...
    fitswrite(tab->headbuf, prefs.gain_keyword, &field->gain, H_EXPO,
              T_DOUBLE);

/* Saturation level */
    addkeywordto_head(tab, prefs.sat_keyword, "Saturation level");
    fitswrite(tab->headbuf, prefs.sat_keyword, &field->saturation, H_EXPO,
              T_DOUBLE);
