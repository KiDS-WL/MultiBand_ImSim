...
interpolate_pix(fieldstruct * field, fieldstruct * wfield,
                ...
                PIXTYPE * woutpix)
{
    PIXTYPE *pixin, *pixout, pixval;
    double dpos[INTERP_MAXDIM], kernel_vector[INTERP_MAXKERNELWIDTH];
    double *kvector, max, val;
    long step[INTERP_MAXDIM];
    long start, fac;
    int linecount[INTERP_MAXDIM];
    int *naxisn, i, j, n, ival, naxis, nlines, kwidth, width, badpixflag;
 ...
 ***/
ikernelstruct *
init_ikernel(interpenum * interptype, int naxis)
{
    ...
    ikernel->nlines = 1;
    for(n = 0; n < naxis; n++) {
        ikernel->nlines *= (ikernel->width[n] =
                            interp_kernwidth[(int) interptype[n]]);
 ...
 ***/
void
free_ikernel(ikernelstruct * ikernel)
{
    free(ikernel->buffer);
