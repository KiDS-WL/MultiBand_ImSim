            ...
            int frameno, int fieldno, weightenum wtype)
{
    ...
    default:
        error(EXIT_FAILURE,
              "*Internal Error*: Unknown weight-map type in ",
              "load_weight()");
        break;
    }
...
/*-- Check the image exists and read important info (image size, etc...) */
        if (frameno >= cat->ntab)
            error(EXIT_FAILURE,
                  "*Internal Error*: FITS extension unavailable in ",
                  wfield->filename);
...
/*-- Set field width and field height (the latter can be "virtual") */
        ...
        if ((wfield->width != reffield->width)
            || (wfield->height != reffield->height))
            error(EXIT_FAILURE,
 ...
 ***/
fieldstruct *
init_weight(char *filename, fieldstruct * reffield)
{
    fieldstruct *wfield;

    wfield =
        inherit_field(filename, reffield, WEIGHT_FIELD | reffield->flags);
 ...
 ***/
void
read_weight(fieldstruct * wfield)
{
    set_weightconv(wfield);
...
{
    weight_reffield =
        (wfield->flags & BACKRMS_FIELD) ? wfield->reffield : NULL;
    weight_pixcount = 0;
    weight_y = 0;
    weight_width = wfield->width;
    weight_type =
        wfield->
        flags & (BACKRMS_FIELD | RMS_FIELD | VAR_FIELD | WEIGHT_FIELD);
...
/*------ New line */
            if (!(n++ % weight_width)) {
                backrmsline(weight_reffield, weight_y++,
                            weight_reffield->backline);
