...
/*Scan the configuration file*/
    ...
    for(warn = 0;;) {
        ...
        if (keyword && keyword[0] != 0 && keyword[0] != (char) '#') {
            if (warn >= 10)
                error(EXIT_FAILURE, "*Error*: No valid keyword found in ",
                      filename);
...
/*------ Expansion of environment variables (preceded by '$') */
                ...
                case P_KEY:
                    ...
                    if (*value == '@')
                        value = listbuf = list_to_str(value + 1);
                    if ((ival =
                         findkeys(value, key[nkey].keylist, FIND_STRICT))
                        != RETURN_ERROR)
                        *(int *) (key[nkey].ptr) = ival;
                    else
                        error(EXIT_FAILURE, keyword,
                              " set to an unknown keyword");
                    break;

                case P_BOOLLIST:
                    if (value && *value == '@')
                        value = strtok(listbuf =
                                       list_to_str(value + 1), notokstr);
                    for(i = 0; i < MAXLIST && value && value[0] != (char) '#';
                        i++) {
    ...
    for(i = 0; key[i].name[0]; i++)
        if (!key[i].flag)
            error(EXIT_FAILURE, key[i].name,
                  " configuration keyword missing");
    if (!flage)
        fclose(infile);
...
*/
int
findkeys(char *str, char keyw[][32], int mode)
{
    int i;
...
*/
int
cistrcmp(char *cs, char *ct, int mode)
{
    int i, diff;
 ...
 ***/
char *
list_to_str(char *listname)
{
    FILE *fp;
    char liststr[MAXCHAR], *listbuf, *str;
    ...
    for(bufpos = 0; fgets(liststr, MAXCHAR, fp);)
        for(str = NULL; (str = strtok(str ? NULL : liststr, "\n\r\t "));) {
            if (bufpos > MAXLISTSIZE)
                error(EXIT_FAILURE, "*Error*: Too many parameters in ",
                      listname);
            l = strlen(str) + 1;
            if (bufpos + l > bufsize) {
                bufsize += 8 * MAXCHAR;
...
/*---- Now check that we haven't gone too far!! */
            if (prefs.ninwfield > prefs.nweight_type)
                error(EXIT_FAILURE,
                      "*Error*: the number of WEIGHT_TYPEs and ",
...
/*---- Weight-maps given through the WEIGHT_IMAGE keyword */
            if (prefs.ninwfield == 1) {
                warning
                    ("Several input images and a single weight-map found: ",
                ...
                error(EXIT_FAILURE,
                      "*Error*: the number of input images and ",
                      "weight-maps do not match");
        }
/*-- Weight types */
...
/* Flux scale default value */
    for(i = prefs.nfscale_default; i < prefs.ninfield; i++)
        prefs.fscale_default[i] =
            prefs.fscale_default[prefs.nfscale_default - 1];
...
/* Default weight-threshold values */
    if (!prefs.nweight_thresh) {
        prefs.weight_thresh[0] =
            (prefs.weight_type[0] == WEIGHT_FROMWEIGHTMAP) ? 0.0 : BIG;
        prefs.nweight_thresh = 1;
    } else
        for(i = prefs.nweight_thresh; i < prefs.ninfield; i++)
            prefs.weight_thresh[i] =
                prefs.back_default[prefs.nweight_thresh - 1];
...
/* Center */
    for(i = prefs.nimage_center; i < INTERP_MAXDIM; i++) {
        ...
        strcpy(prefs.image_center[i],
               prefs.image_center[prefs.nimage_center - 1]);
    }
    prefs.nimage_center = INTERP_MAXDIM;
/* Pixel scale type */
    for(i = prefs.npixscale_type; i < INTERP_MAXDIM; i++)
        prefs.pixscale_type[i] =
            prefs.pixscale_type[prefs.npixscale_type - 1];
