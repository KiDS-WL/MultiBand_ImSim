...
23 Jan 2014 corrected error in rfunc prior definition

SVN Revision $Rev: 220 $ 
last changed by $Author: miller $
on $LastChangedDate: 2014-01-23 22:04:55 +0000 (Thu, 23 Jan 2014) $

--------------------------------------------------------------------------- */

#define DATE_STRING "$LastChangedDate: 2014-01-23 22:04:55 +0000 (Thu, 23 Jan 2014) $"
#define REV_STRING "$Rev: 220 $"
...
int numgoodpsf[NUM_THREAD], numbadpsf[NUM_THREAD],
    numcosmicreject[NUM_THREAD];
...
short int *wanted[NUM_THREAD], *new_wanted[NUM_THREAD];
...
// work arrays for varylpthresholdf3
//int *iwork[NUM_THREAD][6];
//unsigned short int *swork[NUM_THREAD][4];
//int *Istart[NUM_THREAD], *Ilast[NUM_THREAD];
//int imax_intensity[NUM_THREAD];
//float intensity_limit[NUM_THREAD];
//float fmax_intensity[NUM_THREAD], scale[NUM_THREAD];
...
double **u[NUM_THREAD], **v[NUM_THREAD];
...
double mshift[NUM_THREAD][2];
double sumdata[NUM_THREAD];
double *scalefactor, ***edist;
double modelflux[NUM_THREAD][2];
...
/*  table of posterior probability for each galaxy  */
    ...
    char extname[] = "Posterior probability surface";   /*  extension name  */
    char *ttype[] = {"ID", "ra", "dec", "mag", "exposures", "chips", "PSFe1", "PSFe2", "meanPSFe", "Strehl", "class", "size", "bfrac", "flux", "SNratio", "chisqprob", "sampling", "sensitivity", "evalues", "logP" }; /*  names of columns  */
	...
	if (VERBOSE==1)
	  {
        ...
        if (forder < 0) {
            ...
            for(nt = 0; nt < NUM_THREAD; nt++) {
                u[nt] =
                    (double **) calloc((1 + maxncoeffs), sizeof(double *));
                if (u[nt] == NULL) {
                    fprintf(stderr, " error allocating memory for u[nt]\n");
                    exit(EXIT_FAILURE);
                }
                meme += (float) (1 + maxncoeffs) * sizeof(double *);
                ...
                meme += (float) (1 + maxncoeffs) * sizeof(double *);
                if (v[nt] == NULL) {
                    fprintf(stderr, " error allocating memory for v[nt]\n");
                    exit(EXIT_FAILURE);
                }
                for(i = 0; i <= maxncoeffs; i++) {
                    v[nt][i] =
...
/* read the input catalogue */
    ...
    // conversion for exponent 4/3
    aval = exp(afit[0] + afit[1]*(catminmag-refmag))/1.134;
    // value of r at which distribution has a maximum for exponent 4/3
    ...
    */
    ...
    for (nRo=1; nRo<numR; nRo++)
	  ...
	  // galaxy models
	  // linear sampling
	  // Ro[nRo] = Rmin + (double)(nRo-1)*(Rmax-Rmin)/(double)(numR-2);
	  // logarithmic sampling between Rmin and Rmax 
	Ro[nRo] = Rmin * exp((double)(nRo-1)*log(Rmax/Rmin)/(double)(numR-2));
      }
    */
    ...
    for(nt = 0; nt < NUM_THREAD; nt++) {
        ...
        chisq[nt] = (double *) calloc(pwidth * pheight, sizeof(double));
        xfit[nt] = (double *) calloc(pwidth * pheight, sizeof(double));
        yfit[nt] = (double *) calloc(pwidth * pheight, sizeof(double));
        zfit[nt] = (double *) calloc(pwidth * pheight, sizeof(double));
        wfit[nt] = (double *) calloc(pwidth * pheight, sizeof(double));
        meme += (float) 5. *pwidth * pheight * sizeof(double);
        if (chisq[nt] == NULL || xfit[nt] == NULL || yfit[nt] == NULL
            || zfit[nt] == NULL || wfit[nt] == NULL) {
            fprintf(stderr, " failed to allocate memory for fit arrays \n");
            exit(EXIT_FAILURE);
        }
...
/*  figure out how many ellipticity values there will be in the grid and allocate
    ...
    for(nt = 0; nt < NUM_THREAD; nt++) {
        if (NITER > 1) {
            cumulative[nt] = (double *) calloc(nume, sizeof(double));
            if (cumulative[nt] == NULL) {
                fprintf(stderr,
                        " memory allocation error for cumulative array \n");
                exit(EXIT_FAILURE);
            }
            meme += (float) nume *sizeof(double);
        }
...
/* table to identify each object */
    ...
    if (nn > 10) {
        ...
        // choose cutoff as being 10-percentile of magnitude for this range of fprob
        fmag = smag[nn / 10];
    } else {
        ...
        if (fitclass[i] == 0) {
            ...
            if (fprob[i] < fthresh) {
                ...
                fits_write_col(ofptr[0], TINT, 11, row, 1, 1, &fitclass[i],
                               &status);
                fits_write_col(ofptr[0], TDOUBLE, 18, row, 1, 2, de, &status);
                tgoodfit--;
                tstarfit++;
                tswap++;
            }
...
objectloop(void *threadargs)
{
    ...
    int ie1, ie2, iie1, iie2, nRo, nRmin, nRmax, fitOK;
    int close, pixel, modelnum, bmodelnum, ichip, xchip, ychip, nchips;
    int bestne, bestnRo, ne;
    int sampling, oldsampling, nlist, numwanted, oldnumwanted, ine, ndf;
    int nimcheck, thisnim, iter, magbin;
    int abovethreshold;
    int peakpos, ymaxpos;
    int ic, len;
    int step;
    long row, lnmaximages;

    nchips = (int) NCHIPS;

    /*
     * set model galaxy position angle: iter=0, use actual data; iter=1
     * simulated galaxy same ellipticity; iter=2 simulated galaxy rotated
     * by 90 degrees; iter=4 simulated galaxy, same ellipticity but random PA 
     */
    float mphi[4];
    mphi[0] = 0.;
    mphi[1] = 0.;
    mphi[2] = pi / 2.;
    mphi[3] = 0.;               /* fourth iteration this value will be replaced by random angle */

    double fval, radius;
    double maxmarP, dist, distsq, xval, yval, normvalue, meane1, meane2;
    double mmeane1 = 0., mmeane2 = 0.;
    double mbulgeflux = 0., mdiskflux = 0., mbestsize = 0.;
    double chisquared, reducedchisq, bulgeflux, diskflux, bestsize,
        bulgefraction, totalflux;
    double randval, rval, angle, sumsqval, meanval, rmar;
    double psfe[2], centroid[2], meanpsfe[2], moments[3], meanstrehl, psfemod;
    double de[2], sens[4][2];   /*  surface sensitivity vector for an individual galaxy  */
    ...
    double emod, stepemod, sump, sumprior, beta, mval, pval, pvalmax, ene, posterior;
    double maxlikele, s1, s2, value, eeprior, stepeprior;
    double stepvalue, ee1, ee2;
    double stepnorm[4], e1step[4], e2step[4], stepe1[4], stepe2[4];
    double minimumvalue, maximumvalue, localmaxposterior;
    double rndf, rchisq, imchisq, imndf, rprob;
    double cross0, cross1, mcross0, mcross1, mcross2, rechisq, denom, sumysq,
        sumy0, sumy1, amp0, amp1;
    double tsumysq, tsumy0, tsumy1, trechisq, tbestchisq;
    double xmval, ymval, ymax, xmin, xmax;
    double Rarcsec;
    double sumw, meanscalefactor, tscalefactor;
    // double costheta, sintheta, ap, bp;

    fftw_complex shear, shearstar, rotation;
    ...
    for(i = (nmin + nt); i < nmax; i+= NUM_THREAD) {
        /*
         * initialise the noiseperpixel for this object 
         */
        noiseperpixel[i] = 0.;
	...
	sampling = 0;
	meane1 = meane2 = 0.;
	shear = shearstar = rotation = 0.+0.*I;
	fitOK = 0;
        ...
        /*
         * just to test the sensitivity dependence, iterate over some
         * synthesised models (for normal operation NITER is set to 1
         * so this step is missed)
         */

        for(iter = 0; iter < NITER; iter++) {

            sens[iter][0] = sens[iter][1] = 0.;

            /*
             * replace data with simulated data for iter>0 
             */

            if (iter > 0) {
                /*
                 * only replace data for valid galaxies, otherwise
                 * repeat with current data (wastes a bit of time but
                 * it's the easiest way)
                 */
                if (fitclass[i] == 0) {

                    /*
                     * on final iteration set random galaxy PA 
                     */
                    if (iter == 3) {
                        mphi[iter] = 2. * pi * drand48();

                    }
                    make2Dgalaxymodel(nt, testmodel[nt], 
                                           mmeane1, mmeane2, mbestsize);
                    for(j = 0; j < nim[i]; j++) {
                        convolvemodel(pinv[nt], B[nt][j], testmodel[nt],
                                      pheight, pwidth, C[nt]);
                        meanval = 0.;
                        sumsqval = 0.;
                        for(y = 0; y < pheight; y++) {
                            for(x = 0; x < pwidth; x++) {
                                 ...
                                 */
                                mval =
                                    mbulgeflux * c[nt][1][pixel] +
                                    mdiskflux * c[nt][0][pixel];
                                /*
                                 * add gaussian noise (draw random
                                 * numbers from 2D gaussian distn)
                                 */
                                randval = 10.;  // arbitrary number > 1 to start the loop
                                while (randval >= 1.) {
                                    randval = drand48();
                                }
                                rval = sqrt(-2. * log(1. - randval));
                                angle = 2. * pi * drand48();
                                dpix[nt][pixel] = mval + rval * cos(angle);
                                meanval += dpix[nt][pixel];
                                sumsqval += dpix[nt][pixel] * dpix[nt][pixel];
                            }
                        }
                        meanval = 0.;   // dont subtract mean
                        datasq[i][j] =
                            (sumsqval -
                             meanval * meanval / (pwidth * pheight)) / fsize;
                        numgoodpix[i][j] = pwidth * pheight;
                        fftw_execute(q[nt]);    // overwrite the current data FT
                        for(jj = 0; jj < (halfpwidth * pheight); jj++) {
                            D[i][j][jj] = DFT[nt][jj];
                        }
                    }
                }
	    ...
	    //if (i==91025) printf(" starting object %d nim %d \n",i,nim[i]);
            ...
            if (nim[i] > 0) {
			    ...
			    globalreconstruct(xval, yval, sorder,
                        ...
                        psfmoments(psf[nt], pheight, pwidth,
                                   psfevals[nt][jim], centroid, moments);
		...
		*/
		 ...
		 * and if it has enough exposures
		    ...
		    nmarfail[nt] = 0;
                    ...
                    bestne = numzero;
                    bestnRo = 0;
                    bulgeflux = modelflux[nt][1];
                    diskflux = modelflux[nt][0];
                    starflux = modelflux[nt][1];
                    bestsize = 0.;      // best-fitting scalelength
                    rmar = 0.;  // marginalised scalelength
                    starbestpos[0] = bestpos[0] = mshift[nt][0];
                    starbestpos[1] = bestpos[1] = mshift[nt][1];
                    maxlikel = -1.e10;
		       ...
		       * no calculations have yet been made for this ne value.  A value of -2.e10
		       * indicates that this parameter has not been investigated yet. A value
		       * of -1.e10 indicates that calculations have been made but no satisfactory likelihood
		       * maximum was found.  Any other value indicates some success  
		       */
		      modelflux[nt][0] = modelflux[nt][1] = 0.;
		      for (ne=0;ne<(nume*numR);ne++) 
			{
			  likel[nt][ne]=-2.e10;
			}
		      for (ne=0; ne<nume; ne++) marP[nt][ne]= -2.e10;
		      maxlikel = -1.e10;
			 ...
			 must be a power of two factor of basic grid */
		      ...
		      if (sampling < 1) sampling = 1;
		        ...
		        /*
                        ...
                        for(ie1 = -(edim / sampling);
                            ie1 <= (edim / sampling); ie1++) {
                            for(ie2 = -(edim / sampling);
                                ie2 <= (edim / sampling); ie2++) {
                                ii = ie1 * sampling;
                                jj = ie2 * sampling;
			...
			*/
                        ...
                        /*
                         * search through the coarse grid in size and e 
                         */

                        maxposterior = maxlikel = -1.e10;
                            ...
                            for(ie1 = -edim; ie1 <= edim; ie1++) {
                                for(ie2 = -edim; ie2 <= edim; ie2++) {
                                    ...
                                    if (ne >= 0) {
                                        modelnum = nRo * nume + ne;
					bmodelnum = modelnum; 
                                        likel[nt][modelnum] = -2.e10;
                                        if (wanted[nt][ne] == 1) {
                                            likel[nt][modelnum] =
					      likelihood(nt, nimcheck, nRo, ne, modelft[modelnum][0], modelft[bmodelnum][1]);
                                            ...
                                            if (posterior > maxposterior) {
                                                ...
                                                bestne = ne;
                                                //printf("loop %f %f %f %d\n",bulgeflux,diskflux,bestsize,bestne);
                                                bestpos[0] = mshift[nt][0];
                                                bestpos[1] = mshift[nt][1];
                                                tbestchisq = likel[nt][modelnum];
                                            }
                                            /*
                                             * find maximum likelihood 
                                             */
                                            if (likel[nt][modelnum] > maxlikel) {
                                                maxlikel = likel[nt][modelnum];
                                                for(ic = 0; ic < (pwidth * pheight); ic++) {
                                                    if (chisq[nt][ic] > maxchisq) {
                                                        maxchisq = chisq[nt][ic];
                                                        maxchisqpos = ic;
                            ...
                            nRo += nRo < 4 ? 1 : 4;     // choose the finest r step initially but coarser later
                        }

                        /*
                         * test whether any better fits than stellar have been found.  If
                         * not, don't bother proceeding.  We will retest star/galaxy models later 
                         * 
                         * if (maxlikel <= zerolikel)
                         * {
                         * starfit[nt]++;
                         * fitclass[i] = 1;
                         * for (ii=0; ii<nume; ii++) wanted[nt][ii]=0;
                         * }
                         * else
                         * {
                         */
                        ...
                        /*
                         * remove any grid points whose maximum is below the threshold 
                         */
                        /*
                         * for this purpose, include any points whose posterior is above
                         * some threshold and also any points whose likelihood is above
                         * some threshold (to ensure both are measured accurately) 
                         */

                        Lthreshold = maxlikel - 3.9;
                        Pthreshold = maxposterior - 3.9;

                        /*
                         * reset the posterior and likelihood thresholds if either needs to be lower 
                         */
                            ...
                            for(ie1 = -edim; ie1 <= edim; ie1++) {
                                for(ie2 = -edim; ie2 <= edim; ie2++) {
                                    ...
                                    if (ne >= 0) {
                                        ...
                                        if (wanted[nt][ne] == 1) {
                                            posterior = likel[nt][modelnum] + rprior[nt][nRo] + eprior[nt][ne];
                                            if (likel[nt][modelnum] > Lthreshold
                                                || posterior > Pthreshold) {
                                                if (likel[nt][modelnum] < Lthreshold) {
                                                    Lthreshold = likel[nt][modelnum];
                                                }
                                                if (posterior < Pthreshold) {
                                                    Pthreshold = posterior;
                                                }
                                            }
                                        }
                                    }
                        ...
                        /*
                         * remove any points below the joint likelihood/posterior threshold 
                         */
                        numwanted = oldnumwanted = 0;
                        for(ie1 = -edim; ie1 <= edim; ie1++) {
                            for(ie2 = -edim; ie2 <= edim; ie2++) {
                                ne = elookup[edim + ie1][edim + ie2];
                                if (ne >= 0) {
                                    if (wanted[nt][ne] == 1) {
                                        ...
                                        while (nRo < numR) {
                                            ...
                                            if (likel[nt][modelnum] >= Lthreshold
                                                || posterior >= Pthreshold) {
                                                abovethreshold = 1; // set if above threshold at any nRo
                                            }
                                            nRo += nRo < 4 ? 1 : 4; // choose the finest r step initially but coarser later
                                        }
                                        if (abovethreshold == 0)
			...
			*/  
                        ...
                        // printf(" %f %f %d %d \n",Lthreshold,Pthreshold,numwanted,oldnumwanted); fflush(stdout);

                        if (numwanted <= 0) {
			  fflush(stdout);
			  fprintf(stderr," all points rejected before marginalising, object %d \n",i);
			  fprintf(stderr," %lf %lf %lf %lf \n", maxlikel,maxposterior, Lthreshold, Pthreshold);
			  exit(EXIT_FAILURE);
                        }

                        /*
                         * go through the grid, with full sampling in R, select points whose maxima are above
                         * the threshold, and marginalise over R 
                         */

                        /*
                         * this is also the re-entry point if finer grid sampling is required 
                         */

                        oldsampling = 1024;     //dummy value to make sure loop is entered

                        while (oldsampling > sampling) {
                            maxmarP = -1.e10;

                            exception = no_exception = 0;
                            for(ie1 = -(edim / sampling);
                                ie1 <= (edim / sampling); ie1++) {
                                for(ie2 = -(edim / sampling);
                                    ie2 <= (edim / sampling); ie2++) {
                                    ...
                                    if (ne >= 0) {
                                        ...
                                        sump = 0.;
                                        if (wanted[nt][ne] == 1) {
                                            bestnRo = 3;        //arbitrary but valid default maximum
                                            localmaxposterior = -1.e10;
                                            // first quickly check values already computed and find the maximum posterior r
                                            for(nRo = 1; nRo < numR; nRo++) {
                                                modelnum = nRo * nume + ne;
                                                priorval = rprior[nt][nRo] + eprior[nt][ne];
                                                if (likel[nt][modelnum] > -1.e10) {
                                                    posterior = likel[nt][modelnum] + priorval;
                                                    if (posterior > localmaxposterior) {
                                                        localmaxposterior = posterior;
                                                        bestnRo = nRo;  // current best maximum position
                                                    }
                                                }
                                            }
					    // search existing values to find range
					    nRmax = bestnRo+2;
					    posterior = localmaxposterior;
					    while (nRmax<numR && posterior>localmaxposterior-4.6)
					      {
						modelnum = nRmax*nume + ne;
						priorval = rprior[nt][nRmax]+eprior[nt][ne];
						if (likel[nt][modelnum] > -1.e10)
						  {
						    posterior = likel[nt][modelnum] + priorval;
						  }
					    ...
					    nRmin = bestnRo-2;
					    posterior = localmaxposterior;
					    while (nRmin>0 && posterior>localmaxposterior-4.6)
					      {
						modelnum = nRmin*nume + ne;
						priorval = rprior[nt][nRmin] + eprior[nt][ne];
						if (likel[nt][modelnum] > -1.e10)
						  {
						    posterior = likel[nt][modelnum] + priorval;
						  }
					    ...
					    if (nRmin<0) nRmin=0;
					    for (nRo=nRmin; nRo<=nRmax; nRo++)
					      { // accumulate new values to marginalise over r
						/*
						 * now using full sampling in r but stop if we've hit the
						 * posterior threshold set for this ellipticity 
						 */ 
						modelnum = nRo * nume + ne;
						priorval = rprior[nt][nRo] + eprior[nt][ne];
						if (likel[nt][modelnum] <= -1.5e10) {
						  likel[nt][modelnum] =
						...
						/*
						 * if a valid likelihood value has been returned, set the other thresholds 
						 */
						if (likel[nt][modelnum] > -1.e10) {
						  likelval = likel[nt][modelnum];
						  posterior = likelval + priorval;
						  /*
						   * set thresholds for this loop 
						   */
						  if (posterior > localmaxposterior)
						    {
						      localmaxposterior = posterior;
						  ...
						  if (likelval > maxlikele) {
						    maxlikele = likelval;
                                            ...
                                            // range such that they can be summed).
                                            if (localmaxposterior > -1.e10) {
					      // adjust the localmaxposterior so that we only consider the r prior component
					      localmaxposterior -= eprior[nt][ne];
                                                // find first valid measurement
                                                nRo = 1;
                                                modelnum = nRo * nume + ne;
						priorval = rprior[nt][nRo];
						likelval = likel[nt][modelnum];
						posterior = exp(likelval + priorval - localmaxposterior);
                                                while (nRo < numR && ( likel[nt][modelnum] <= -1.e10 || posterior<=0. ) ) 
						  {
                                                    nRo++;
                                                    modelnum = nRo * nume + ne;
						    priorval = rprior[nt][nRo];
						    ...
						    priorval = rprior[nt][nRo]
						      - rfunc(Rarcsec, catmag[i]);  // adjust for varying Ro
                                                    xmval = Ro[nRo] / 10.;      // set start point
                                                    while (xmval < Ro[nRo]) {
                                                        // linear interpolate log(likelihood) between r=0 value and Ro[nRo] value
                                                        ymval =  zerolikel +
                                                            (xmval/Ro[nRo]) * (likel[nt][modelnum]-zerolikel);
                                                        xmarvals[nt][num_marvals[nt]] = xmval;
							Rarcsec = xmval*meanscalefactor;
                                                        ymarvals[nt][num_marvals[nt]] = ymval + priorval 
							  + rfunc(Rarcsec, catmag[i])
                                                          - localmaxposterior;
                                                        num_marvals[nt]++;
                                                        xmval += Ro[nRo] / 5.;
                                                    }
								...
								//printf("   %f %f %f %f \n",Ro[nRo],log(posterior),ee1,ee2);
                                                    ...
                                                    for(ii = 1;ii < num_marvals[nt];ii++) {
                                                        if (xmarvals[nt][ii] <= xmarvals[nt][ii-1]) {
                                                            fflush(stdout);
                                                            fprintf(stderr,
                                                                    " interp array out of order at object %d ne %d \n",
                                                                    i, ne);
							...
							if (ymarvals[nt][ii] > -70. && ymarvals[nt][ii] > EXPMIN)
							  {
							    ...
							    if (xmarvals[nt][ii] > xmax) xmax = xmarvals[nt][ii];
                                                    ...
                                                    if (num_marvals[nt] < 5 || xmax <= xmin) {
                                                        fflush(stdout);
                                                        fprintf(stderr,
                                                                " error in function, object %d %d %d \n",
                                                                i, sampling, ne);
                                                        fprintf(stderr,
                                                                " num = %d xmin,max = %f %f \n",
                                                    ...
                                                    /*
                                                     * printf(" integration %d %f %f %f %f %d \n",ne,
                                                    ...
                                                    gsl_status = gsl_spline_init(fspline[nt],
                                                         xmarvals[nt],ymarvals[nt],num_marvals[nt]);
                                                    if (gsl_status) {
                                                        fflush(stdout);
							...
							*/
							  ...
							  if (VERBOSE == 1)
								...
								fflush(stdout);
                                                        ...
                                                        marP[nt][ne] = log(sump) + localmaxposterior;
                                                        if (marP[nt][ne] > maxmarP) 
							  {
                                                            maxmarP = marP[nt][ne];
							  }
                                                    }
                            ...
                            if (exception > 0)
                                printf
                                    (" object %d had %d integration errors and %d successes \n",
                                     i + 1, exception, no_exception);
			    
			    /* uncomment this part to write out for each fitted galaxy
			       the total number of calls to the likelihood function 
			    if (nmarfail[nt]>0)
			      {
			    
			    printf("ncall %d %d %d %d %d \n",nt,i,sampling,ncall[nt],nmarfail[nt]);
			    fflush(stdout);
                            ...
                            for(ie1 = -(edim / sampling);
                                ...
                                for(ie2 = -(edim / sampling);
                                    ...
                                    if (ne >= 0) {
                                        if (wanted[nt][ne] != 1
                                            || marP[nt][ne] <= -1.e10) {
                                            marP[nt][ne] = rmarvals[nt][ne] = 0.;
                                            for(nRo = 1; nRo < numR; nRo++) {
					      marP[nt][ne] += exp(rprior[nt][nRo]);
					      rmarvals[nt][ne] += Ro[nRo] * exp(rprior[nt][nRo]);
                                            }
                                            if (marP[nt][ne] > 0.) {
					      rmarvals[nt][ne] /= marP[nt][ne];
					      marP[nt][ne] = log(marP[nt][ne]) - 0.5 * sumdata[nt];
					      if (marP[nt][ne] > maxmarP) {
						maxmarP = marP[nt][ne];
					      }
                                            } else {
                                                fflush(stdout);
                                                fprintf(stderr,
                                                        " error finishing marginalisation at object %d\n",
                                                        i + 1);
                                                exit(EXIT_FAILURE);
                                            }
                                        }
                            ...
                            Lthreshold = maxlikel - 3.9; /* set threshold to 5 percent of peak */
                            Pthreshold = maxmarP - 3.9;

                            /*
                             * reset marginalised posterior threshold so that the peak likelihood is always included 
                             */
                                    ...
                                    for(ie2 = -edim; ie2 <= edim; ie2++) {
                                        ...
                                        if (ne >= 0) {
                                            if (wanted[nt][ne] == 1) {
                                                modelnum = nRo * nume + ne;
                                                if (likel[nt][modelnum] > Lthreshold
                                                    && marP[nt][ne] < Pthreshold) {
                                                    Pthreshold = marP[nt][ne];
                                                }
                                            }
                                        }
                                    }
                            ...
                            /*
                             * select grid points that have maximum L
                             * & P values above the threshold, and
                             * create a new list of wanted grid points
                             */
			...
			for(ie1 = -edim; ie1 <= edim; ie1++) {
			  for(ie2 = -edim; ie2 <= edim; ie2++) {
			    ...
			    */
                            if (nlist <= 0) {
                                fprintf(stderr,
                                        " no grid points selected after rethresholding, object %d \n",
                                        i);
                                fprintf(stderr,
                                        " %d selected before rethresholding \n",
                                        numwanted);
                                fprintf(stderr, " sampling = %d \n",
                                        sampling);
                                fprintf(stderr, " %lf %lf %lf %lf \n",
                                        maxlikel, maxposterior, Lthreshold,
                                        Pthreshold);
                                exit(EXIT_FAILURE);
                            }

                            /*
                             * use number of grid points currently
                             * selected to decide if increased
                             * sampling is required, and if so, how
                             * much, aiming for around 12 sampled
                             * points altogether, and only allowing
                             * grid sampling that is a power of 2
                             */

			    oldsampling = sampling;
			    sampling = (int)(.001+log((float)sampling/sqrt((float)minsamplepoints/(float)nlist))/log(2.));
			    sampling = (int)(.001+pow(2.,(double)sampling));
			    if (sampling < 1) sampling = 1;
			    if (sampling > oldsampling) sampling = oldsampling;

                            if (sampling < oldsampling) {
                                /*
                                 * fill out the "wanted" grid by
                                 * expanding around each currently
                                 * selected grid point
                                 */
                                    ...
                                    for(ie2 = -edim; ie2 <= edim; ie2++) {
                                        ...
                                        if (ne >= 0) {
                                            if (wanted[nt][ne] == 1) {
                                                ie1 = e1lookup[ne];
                                                ie2 = e2lookup[ne];
                                                for(iie1 = ie1 - oldsampling;
                                                    iie1 <= ie1 + oldsampling;
                                                    iie1 += sampling) {
                                                    for(iie2 = ie2 - oldsampling;
                                                        iie2 <= ie2 + oldsampling;
                                                        iie2 += sampling) {
                                                        ii = edim + iie1;
                                                        jj = edim + iie2;
                                                        if (ii >= 0
			...
			for(ie1 = -edim; ie1 <= edim; ie1++) {
			  for(ie2 = -edim; ie2 <= edim; ie2++) {
				...
				*/
                            ...
                            /*
                             * go up to next sampling if required 
                             */
                        }
                        ...
                        fitOK = 0;      // initialise the "good fit" flag 

                        /*
                         * test whether the maximum in the
                         * best-fitting chi-squared surface is outside
                         * the nominal position error: if it is,
                         * disqualify this object as being
                         * contaminated
                         */
                        y = maxchisqpos / pwidth;
                        x = maxchisqpos - pwidth * y;
                        if (x >= pwidth / 2)
                            x -= pwidth;
                        if (y >= pheight / 2)
                            y -= pheight;
			  ...
			  if (fitclass[i] >= 0)
			    {
			      ...
			      badfit[nt]++;
                            ...
                            fval = 0.;
                            fprob[i] = 0.;
                            if (ndf > 7 && zerolikel > -1.e10
                                && maxlikel > -1.e10 && zerolikel != 0.) {
                                fval =
                                    maxlikel * (ndf - 3) / zerolikel / (ndf -
                                                                        7);
                                if (fval > 0.)
                                    fprob[i] =
                                        gsl_cdf_fdist_Q(fval,
                                                        (double) (ndf - 7),
                                                        (double) (ndf - 3));
                            }
                             ...
                             */
                            else if (maxlikel + 0.5 * sumdata[nt] > 7.035) {
                                /*
                                ...
                                for(ie1 = -(edim / sampling);
                                    ...
                                    for(ie2 = -(edim / sampling);
                                        ...
                                        if (ne >= 0) {
                                            rmar +=
                                                rmarvals[nt][ne] *
                                                exp(marP[nt][ne] - maxmarP + eprior[nt][ne]);
                                            sump +=
                                                exp(marP[nt][ne] - maxmarP + eprior[nt][ne]);
                                        }
                                    }
                                ...
                                /*
                                 * now mean posterior ellipticity 
                                 */
                                sump = 0.;
                                meane1 = 0.;
                                meane2 = 0.;
                                ...
                                for(ie1 = -(edim / sampling);
                                    ...
                                    for(ie2 = -(edim / sampling);
                                        ...
                                        if (ne >= 0) {
                                            for(nRo = 1; nRo < numR; nRo++) {
                                                priornorm += exp(rprior[nt][nRo]);
                                                modelnum = nRo * nume + ne;
                                                if (wanted[nt][ne] == 1
                                                    && likel[nt][modelnum] > -1.e10) {
                                                    value = exp(likel[nt][modelnum] +
								eprior[nt][ne] +
                                                                rprior[nt][nRo]
                                                                - maxposterior);
                                                } else {
						  /* CHECK THIS TERM */
                                                    value = exp(-0.5*sumdata[nt] +
								eprior[nt][ne] +
								rprior[nt][nRo]
								- maxposterior);
                                                }
                                ...
                                /*
                                 * find the means and create summed arrays, sensitivity arrays 
                                 */
                                if (sump > 0.) {
                                    meane1 = meane1 / sump;
                                    meane2 = meane2 / sump;

                                    sump = log(sump) + maxposterior;

                                    /*
                                     * find the sampling bin number 
                                     */
                                    is = 0;
                                    ts = 1;
                                    while (ts < sampling) {
                                    ...
                                    if (is >= nsampling) {
                                        fprintf(stderr,
                                                " error in sampling %d %d \n",
                                                is, nsampling);
                                        exit(EXIT_FAILURE);
                                    }

				    /* set up steps for sensitivity calculation */
				    for (step=0; step<4; step++)
				      {
					stepnorm[step] = stepe1[step] = stepe2[step] = 0.;
				      }
				    e1step[0] = e_interval;
				    e2step[0] = 0.;
				    e1step[1] = -e_interval;
				    ...
				    /* evaluate sensitivity using discrete steps in ellipticity */
                                    for(nRo = 1; nRo < numR; nRo++) {
                                        for(ie1 = -(edim / sampling); ie1 <= (edim / sampling); ie1++) {
                                            for(ie2 = -(edim / sampling); ie2 <= (edim / sampling); ie2++) {
                                                ii = ie1 * sampling;
                                                jj = ie2 * sampling;
                                                ne = elookup[edim + ii][edim + jj];
                                                if (ne >= 0) {
                                                    modelnum = nRo * nume + ne;
                                                    if (wanted[nt][ne] == 1 && likel[nt][modelnum] > -1.e10) {
                                                        value = exp(likel[nt][modelnum] +rprior[nt][nRo] +eprior[nt][ne] -sump);
                                                    } else {
                                                        value = exp(-0.5*sumdata[nt] +rprior[nt][nRo] +eprior[nt][ne] -sump);
                                                    }

                                                    if (WRITE_SUMMED_LIKELIHOODS == 1)
                                                        sumlikel[nt][magbin][is][modelnum] += value;

                                                    emod = sqrt(e1[ne] * e1[ne] + e2[ne] * e2[ne]);
						    
						    for (step=0; step<4; step++)
						      {
							ee1 = e1[ne] + e1step[step];
							ee2 = e2[ne] + e2step[step];
							stepemod = sqrt (ee1*ee1+ee2*ee2);
							if (emod < max_eta && stepemod < max_eta ) 
							  {
							    eeprior = 
							      (1.-bulgepopfrac)*exp(efunc(emod)) + 
							      bulgepopfrac*exp(bulgefunc(emod));
							    stepeprior = 
							      (1.-bulgepopfrac)*exp(efunc(stepemod)) + 
							      bulgepopfrac*exp(bulgefunc(stepemod));
							    stepvalue = value * stepeprior/eeprior;
							    stepe1[step] += stepvalue*ee1;
							    stepe2[step] += stepvalue*ee2;
							    stepnorm[step] += stepvalue;
							  }
						      }
						}
					    }
				    ...
				    if (stepnorm[0]>0. && stepnorm[1]>0.)
				      s1 = (stepe1[0]/stepnorm[0] - stepe1[1]/stepnorm[1])/2./e_interval;
				    else
				      s1 = 0.;

				    if (stepnorm[2]>0. && stepnorm[3]>0.)
				      s2 = (stepe2[2]/stepnorm[2] - stepe2[3]/stepnorm[3])/2./e_interval;
				    else
				      s2 = 0.;
                                     ...
                                     */
                                    for(ine = 0; ine < nume; ine++) {
                                        if (marP[nt][ine] > -1.e10) {
                                            marP[nt][ine] -= maxmarP;
					    fitOK = 1;
                                        }
                                    }
                                ...
                                if (fitOK == 1 && fitclass[i]>=-1) 
				  {
                                    fitclass[i] = 0;
                                    goodfit[nt]++;
				  }

				if (fitOK == 1 && (fitclass[i] >= 0 || fitclass[i] < -4) )
				  {
                                    /*
                                     * normalise the posterior probability surface  
                                     */
                                    /*
                                     * assumes probability is zero outside the calculated grid
                                     * and that the sampling in the grid is constant at "sampling" 
                                     */
                                    beta = 0.;
                                    for(ie1 = -(edim / sampling);
                                        ie1 <= (edim / sampling); ie1++) {
                                    ...
                                    /*
                                     * make models of the best fit galaxy and of one rotated by 90 degrees 
                                     * and replace the data array by this next model, so that on the next
                                     * iteration this model is used instead of real data 
                                     */

                                    if (iter == 0 && NITER > 1) {
                                        /*
                                         * sample the galaxy posterior distribution to get the distribution correct 
                                         */
                                        cumulative[nt][0] =
                                            linearposterior[nt][0] *
                                            e_interval * e_interval *
                                            sampling * sampling;
                                        for(ine = 1; ine < nume; ine++) {
                                            if (wanted[nt][ine] == 1) {
                                                cumulative[nt][ine] =
                                                    cumulative[nt][ine - 1] +
                                                    linearposterior[nt][ine] *
                                                    e_interval * e_interval *
                                                    sampling * sampling;
                                            } else {
                                                cumulative[nt][ine] =
                                        ...
                                        ene =
                                            drand48() * cumulative[nt][nume -
                                                                       1];
                                        ine = 0;
                                        while (cumulative[nt][ine] < ene
                                               && ine < nume - 1) {
                                            ine++;
                                        }
                                        /*
                                         * set the ellipticity for the simulated galaxy (next iter) 
                                         */
                                        mmeane1 = e1[ine];
                                        mmeane2 = e2[ine];
                                        /*
                                         * capure the size and bulge/disk fluxes also - NB these have not been
                                         * treated correctly with the bayesian formalism! 
                                         */
                                        mbestsize = bestsize;
                                    ...
                                    /*
                                     * keep the sensitivity for later output (now disabled) 
                                     */
                                    sens[iter][0] = de[0];
                                    sens[iter][1] = de[1];

                                    /*
                                     * disqualify the entire fit if the sensitivity comes
                                     * out negative 
                                     * if (de[0]<0. || de[1]<0.)
                                     * {
                                     * printf (" negative sensitivity %f %f \n",de[0],de[1]); 
                                     * goodfit[nt]--;
                                     * badfit[nt]++;
                                     * fitclass[i] = -3;
                                     * for (ii=0; ii<nume; ii++) wanted[nt][ii]=0;
                                    ...
                                    /*
                                     * if no marginalised values above -1.e10 are found, disqualify 
                                     */
                                } else {
				  if (fitclass[i]>=-1)
				    {
				  ...
				  if (fitclass[i]>=-1)
				    {
				      ...
				      fitclass[i] = -3;
                        ...
                        /*
                         * no valid fit was found for point source (zerolikel < -1.e10)  
                         */
		      if (fitclass[i]>=-1)
			{
			  badfit[nt]++;
			  fitclass[i] = -3;
                    ...
                    /*
                     * insufficient valid data extracted 
                     */
		      if (fitclass[i]>=-1)
			{
			  nodata[nt]++;
	    ...
	    if (nimcheck>0 && nimcheck<minexposurenumber) toofewexposures[nt]++;
            ...
            if (fitOK==1 && (fitclass[i] == 0 || fitclass[i] < -4)) {
                ...
                maxposterior = -2.e10;
                for(ie1 = -(edim / sampling); ie1 <= (edim / sampling); ie1++) {
                    for(ie2 = -(edim / sampling); ie2 <= (edim / sampling);
                        ie2++) {
                        ii = ie1 * sampling;
                        jj = ie2 * sampling;
                        ne = elookup[edim + ii][edim + jj];
                        if (ne >= 0) {
                            if (marP[nt][ne] > maxposterior)
                                maxposterior = marP[nt][ne];
                            if (marP[nt][ne] > maxlikele)
                                maxlikele = marP[nt][ne];
                        }
                    }
                }
                for(ie1 = -(edim / sampling); ie1 <= (edim / sampling); ie1++) {
                    for(ie2 = -(edim / sampling); ie2 <= (edim / sampling);
                        ...
                        if (ne >= 0) {
                            ...
                            if (sampling >= 8 ||
                                marP[nt][ne] > maxposterior - 11.51 ||
                                marP[nt][ne] > maxlikele - 11.51) {
                                etable[nt][nlist] = ne;
	      ...
	      de[1] = 0;
            ...
            // printf(" nlist %d %d %f %f %d %d \n",i,fitclass[i],de[0],de[1],sampling,nlist);

            /*
             * if (nlist<=0)
             * {
             * printf (" no prob values, nlist= %d \n",nlist);
             * fitclass[i] = -3;
             * }
             */

            /*
             * make sure bad fits have zero sensitivity
             * if (fitclass[i]<0)
             * {
             * de[0]=0.;
             * de[1]=0.;
	    ...
	    // make sure bad fits have zero scalelength
            if (fitOK==0 || (fitclass[i] != 0 && fitclass[i] > -4)) rmar = 0.;
		...
		/* recalculate best-fitting likelihood to see what the values were inside the function 
		if (nRo>0 && nt==0)
		  {
		    printlikel=1;
		    likelval = likelihood(nt,nimcheck,nRo,bestne,modelft[modelnum]);
		    printlikel=0;
		  }
		*/
	    ...
	    if (status) fits_report_error(stderr, status); /* print error message */
             ...
             */
            fits_write_col(ofptr[iter], TDOUBLE, 12, row, 1, 1, &rmar,
                           &status);
            ...
            fits_write_col(ofptr[iter], TDOUBLE, 14, row, 1, 1, &totalflux,
                           &status);
            /*
             * S/N ratio returned from detectobject 
             */
             ...
             * reduced chi-squared 
             */
            fits_write_col(ofptr[iter], TDOUBLE, 16, row, 1, 1, &rprob,
                           &status);
            /*
	     ...
	     * now replace it with fprob and dummy value
	     */
	    de[0] = fprob[i];
	    de[1] = 0.;
        ...
        /*
         * printf (" sensitivities %d %lf %lf %f %f %f %f %f %f \n",
         * i,wcsx[i],wcsy[i],sens[0][0],sens[1][0],sens[2][0],sens[0][1],sens[1][1],sens[2][1]);
         */
...
likelihood(int nt, int nim, int nRo, int ne, float *ftd, float *ftb)
{
    ...
    int newixmax, newiymax, ixmax, iymax, nfit, order, crossterm, ncoeffs;
     ...
     * the value of poserror.
     * 
     * If the maximum value is found outside this radius, but closer than 
     * five times poserror, then this code assumes that another object
     * is close to the one being fitted, and that the isophotes overlap
     * sufficiently to invalidate the fitting algorithm.  Hence in this
     * case the likelihood will be set to the default negative value
     * 
	...
	for (y=0; y<=halfpheight; y++)
	  {
		...
		expandedft[nt][mt][opixel] = ft[mt][ipixel];
    ...
    iymax = newiymax = 0;
    newmaximum = 1;
    dx = 2;                     // start with a search box of full width poserror pixels
    while (newmaximum == 1) {
        newmaximum = 0;
        ixmax = newixmax;
        iymax = newiymax;
    ...
    /*
     * if the radius of the peak from the centre is more than
     * the prescribed limit, then set likelihood to a big negative
     * number and return for the next model.  This guards against
     * contamination from neighbouring objects 
     */
     ...
     * 2D polynomial fit 
     */
    svdfit2dsqc(xfit[nt], yfit[nt], zfit[nt], wfit[nt], nfit, order,
                crossterm, avals, u[nt], v[nt], w);
    ...
    if (avals[6] < 0. && avals[3] < 0. && denom > 0.) {
        ...
        x0 = (avals[5] * avals[2] - 2. * avals[4] * avals[3]) / denom;
        y0 = (avals[5] * avals[4] - 2. * avals[2] * avals[6]) / denom;
        newmaxval = avals[1] + avals[2] * y0 + avals[3] * y0 * y0
            + avals[4] * x0 + avals[5] * x0 * y0 + avals[6] * x0 * x0;
	...
	if (flux>(totalflux/100.))
	  {
		...
		if (arg1>0.)
		  {
		    ...
		    /*
		    if (nt==0 && nRo>0 && printlikel==1 && flux==totalflux)
		      {
		    ...
		    */
			...
			if (erfval > 1.e-6)
		      ...
		      {
			//printf(" failed pedestal erf generation %f %f \n",erfarg1,erfarg2);
			pedval=-1.e10;
		      }

		    // marginalise over position
		    if (likel > -1.e10 && pedval > -1.e10)
		      {
			likel = likelfunc(likel-pedval);
			if (likel > -1.e10) likel += pedval;
		      }
		    else
		      {
			likel = pedval = -1.e10;
		...
		if (bulgelikel > -1.e10) bulgelikel += bulgepedval;
		...
		if (likel > -1.e10)
		  {
		    ...
		    if ( bulgelikel>-1.e10 && dlikel < EXPMAX/2)
		      {
		...
		/*
		if (printlikel==1 && nt==0 && nRo>0 && flux==totalflux) 
		...
		if (likel > -1.e10)
		  {
		    ...
		    // multiply likelihood by flux prior
		    fprior = -fluxpriorslope * log(flux);
		    // if (printlikel==1 && nt==0) printf("star likel %f %f %f %f \n",flux,fprior,likel,-sumdata[nt]/2.);
		    xmarvals[nt][num_marvals[nt]] = flux;
		    likel += fprior;
		    ymarvals[nt][num_marvals[nt]] = likel;
	...
	/* now marginalise over flux by numerical integration, both for the
	   likelihood at this best-fit position and also for the pedestal */
		...
		// printf(" failed likel marginalisation %d %d %d \n",nt,nRo,ne);
		likel = sqrt(2.*pi)*fluxerr;
	      }
	    // if (printlikel==1 && nt==0) printf(" mar vals %d %f %f \n",num_marvals[nt],fluxmar,fluxmarbt);
      }
    else
      {
    ...
    // if (printlikel==1 && nt==0) printf(" likels %f %f \n",maxlikel,likel);
    if (likel>0. && maxlikel>-1.e10)
      {
	// take log and apply maximum offset from above
	likel = log(likel)+maxlikel;
	/*
	if (printlikel==1 && nt==0)
	  printf(" marginalised likel %f \n",likel);
	*/
      }
    else
      {
	// printf(" nRo %d likels %f %f \n",nRo,maxlikel,likel);
	likel = -1.e10;
      }
    
    // if (printlikel==1 && nt==0) printf(" %f \n",likel);
    
    // set the modelfluxes to the marginalised values
    if (fluxmar <= 0.) fluxmar = totalflux;
    if (nRo==0) fluxmarbt = 1.;
    modelflux[nt][0] = (1.-fluxmarbt)*fluxmar;
    modelflux[nt][1] = fluxmarbt*fluxmar;
    ...
    )
{
    ...
    int n, n2, ncol, dx, dy, numlimit, target, neighbour, nfit, order,
        ncoeffs, crossterm;
    int useful, niter, idsnlimit;
    int nswap, ntcol, found, nkcol[9], kcol[9], nmaxcol, better_colour;
    int subtract_background=1; // define whether to fit and subtract background in this function (0=no)
    double radius, minradius, maxr, dsnlimit, ldsnlimit, stop_dsn, meanx, meany, sumw;
    double hsize, xd, yd, avals[4], w[4], sgauss[4][4], smooth;
	...
	// go through and crate objects of connected pixels starting with the brightest objects
	...
	if (subtract_background==1)
	  {
	    ...
	    for(k = 0; k < nimages; k++) {
		  ...
		  svdfit2dsqc(xfit[nt], yfit[nt], zfit[nt], wfit[nt], nfit,
			      order, crossterm, avals, u[nt], v[nt], w);
		  nfit = 0;
		  for(j = 0; j < size; j++) {
		    y = j / pwidth;
		    x = j - y * pwidth;
		    ...
		    if (Dpix[jj][pixel] != 0.) {
		      ...
		      yfit[nt][nfit] = yd / hsize;
		      zfit[nt][nfit] =
			avals[1] + avals[2] * yfit[nt][nfit] +
			avals[3] * xfit[nt][nfit];
