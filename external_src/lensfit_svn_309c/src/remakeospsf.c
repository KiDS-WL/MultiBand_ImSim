...
on $LastChangedDate: 2014-10-27 23:02:42 +0000 (Mon, 27 Oct 2014) $

---------------------------------------------------------------------------*/

#define DATE_STRING "$LastChangedDate: 2014-10-27 23:02:42 +0000 (Mon, 27 Oct 2014) $"
...
int getospsfsizes(char *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *,
                      int *, int *, int *, int *, int *, double *, double *, float *,
                      double *, int *, int *, int *, int *);
int readoscoeffs(char *, int *, double *, int, int, int,
		 double *, double *);
...
char *recreate_psf_name(const char *, char *, const char *);
char *recreate_ospsf_name(const char *, char *, const char *);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 6) {
        printf
            (" %s <imagelist> <global order> <chip order> <snratio> <solution file> \n",
             argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    */
    ...
    if (argc==6)
      {
	solfile = ml_calloc(PATH_MAX, sizeof(char), &memory, "solfile");
      }
    ...
    coeffname = recreate_psf_name(psfdir, firstname, dotdelimiter);
    // PSF coefficients file with oversampling, new format
    if ((int)GREAT3 < 1)
      strcpy(firstname, argv[1]);
    else
      strcpy(firstname, "starfield_image-000-0");
    ospsfname = recreate_ospsf_name(psfdir, firstname, dotdelimiter);

    if (argc==6)
      {
	strcpy(solfile, argv[5]);
      }
  ...
  nychip = 1;  // one chip on y-axis
    ...
    */
	...
	fheight = pheight*ofactor;
	fsize = fwidth*fheight;
	int psfoversampling, correctd;
	      getospsfsizes(solfile, &correctd, &fwidth, &fheight, &psfoversampling,
			    &order, &chipvariation, &chiporder,
			    &ncoeffs, &nfit, numstar, &nxchip, &nychip,
			    &xchipsampling, &ychipsampling, &big_gap,
			    &hxsize, &hysize, &psfposerror, scalefactor,
			    &sorder, &schipvariation, &schiporder,
			    &sncoeffs);
	      // fudge the returned values
	      psfoversampling = oversampling;
	      nxchip=8;
	      nychip=4;
	      xchipsampling=2110;
	      ychipsampling=4160;
	      ...
	      hysize=8320.;
	      psfposerror=6.;
	      if (psfoversampling<=0) psfoversampling = fwidth/48;
	      if (psfoversampling != oversampling)
		{
		  fflush(stdout);
		  fprintf(stderr," mismatch in user-requested PSF oversampling and oversampling of PSF files %d %d\n",oversampling,psfoversampling);
		  exit(EXIT_FAILURE);
		}
	readoscoeffs(solfile, pindex, soln, nfit, ncoeffs,
		     sncoeffs, xcoeffs, ycoeffs);
	if (nfit>fsize)
	  {
	    fprintf(stderr," error in number of fittable pixels returned by readoscoeffs %d\n",nfit);
	    fflush(stderr);
	    exit(EXIT_FAILURE);
	  }
	dsize = nfit*ncoeffs;
	...
	while (iter<niter && 
	       ...
	       )
	  {
	    ...
	    if (solfile==NULL)
	      {
		chisqmin = solvePSF();
		printf(" PSF model solution has been frozen\n");
	      }
	    deltachisq = pchisq-chisqmin;
    ...
    writeoscoeffs(ospsfname, correct_distortion, pindex, basesoln, nfit, ncoeffs, 
		  ...
		  fwidth, fheight, oversampling, 
		  nxchip, nychip, numstar, xchipsampling, ychipsampling,
...
char *
recreate_psf_name(const char *psfdir, char *firstname, const char *dotdelimiter)
{
    char *pstr;
    int len;
    char *coeffname;
    ...
    if (!(pstr = strrchr(firstname, *dotdelimiter)))
        pstr = firstname + strlen(firstname);
    sprintf(pstr, "%s", ".psfcoeffs.remake.fits");
...
char *
recreate_ospsf_name(const char *psfdir, char *firstname, const char *dotdelimiter)
{
    char *pstr;
    int len;
    char *oscoeffname;
    ...
    if (!(pstr = strrchr(firstname, *dotdelimiter)))
        pstr = firstname + strlen(firstname);
    sprintf(pstr, "%s", ".ospsfcoeffs.remake.fits");
