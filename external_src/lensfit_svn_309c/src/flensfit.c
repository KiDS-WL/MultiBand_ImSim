...
9/9/15 removed various suoerfluous print statements, added extra output columns for psf size and poserror test,
updated file string handling for kids r and i bands

SVN Revision $Rev: 309 $ 
last changed by $Author: miller $
on $LastChangedDate: 2017-09-11 19:55:58 +0100 (Mon, 11 Sep 2017) $

---------------------------------------------------------------------------

GREAT3 Revision: 21
last changed by Lance Miller
on 30.04.2014 17.30
SUMMARY OF previous CHANGE : Version Control (IFC)
SUMMARY OF previous CHANGE : Updated priors to lognormal size distribution
including cut on galaxy size and fits to ellipticity prior for G3.
Also changed command-line arguments and added function addnoise to 
optionally allow extra gaussian noise to be injected into postage stamps,
for measuring noise bias on the G3 training data. (LM)
SUMMARY OF previous CHANGE: Corrected error in implementation of addnoise function. (LM)
SUMMARY OF previous CHANGE: introduced option ALLOW_STAR_FIT to switch off star fitting (LM)
SUMMARY OF previous CHANGE: changed size, ellipticity and bulge fraction prior parameters values
to match results on ground  control Deep data with new PSF model (LM 9/4/2014 18.44).
SUMMARY OF LAST CHANGE: added extra power into ellipticity prior and modified prior parameter values (LM 10/4/2014 00:23).
SUMMARY: corrected wrong values of ellipticity prior parameters (LM 10/4/2014 08:07)
SUMMARY: updated size, bulge fraction and ellipticity prior parameters (LM 12/4/2014 21:25)
SUMMARY: correct logic of forcing all objects to be fitted as galaxies (LM 14/4/2014 14:36)
SUMMARY: modified tbswarpextract_nodistortion and the surrounding loop to allow galaxies to be near the image edge (LM 15/4/2014 20.08)
SUMMARY: implementation of self-calibration LM 23-24 April 2014 22.39pm
SUMMARY: simplified ellipticity measurement and applied posterior cut LM 25 April 09.40
SUMMARY: removed outer ellipticity cut inside function measure_ellipticity LM 25 April
SUMMARY: corrected errors in passing scalefactor to rfunc, tidied up arrays LM 26 April
SUMMARY: put cuts back in, on high likelihood values at extreme ellipticity LM 27 April
SUMMARY: now reads prior parameters from file LM 27 April 12.23
SUMMARY: now allows input of oversampled PSF coefficients file and does accurate FFT of model and PSF in the case of a G3 constant PSF.  LM 27/4/14 21.50
SUMMARY: improvements to creation of bulge models (no longer puts a huge spike in the middle!) and minor changes, but still no fix for the problem that using oversampled PSFs produces bad results...  LM 29 April
SUMMARY: removed oversampling option above because of realisation that it's implemented incorrectly - we need to store complex Fourier transformed models not purely real transformed models
SUMMARY: changed transform array to type complex float so that exact FFT convolution can be carried out, thus resurrect the oversmapling option.  LM 29/4/2014
SUMMARY: modified model generation so that pixel averaging is done in the isotropic space, to prevent effective broadening of the PSF in the data space. LM 30/4/2014

POST-GREAT3 REVISIONS:

updated star-galaxy discrimination and included switches for choice of real data or GREAT3 simulations.  LM 23/5/2014.
returned autocalibration to standard likelihood instead of sqrt. LM 24/6/2015.

--------------------------------------------------------------------------- */

#define DATE_STRING "$LastChangedDate: 2017-09-11 19:55:58 +0100 (Mon, 11 Sep 2017) $"
#define REV_STRING "$Rev: 309 $"
...
#include <unistd.h>
#include <fitsio.h>
// complex.h must be invoked BEFORE fftw3 to get complex types correct
#include <complex.h>
#include <fftw3.h>
#include <nrutil.h>
#include <lensutil.h>
...
/* define which size definition to use.  Setting this equal to 3 will choose
the GREAT3 convention.  Any other positive integer value selects GREAT10. 
Any other choice (e.g. 0) will select the original lensfit
definition.  In fact GREAT3 and original lensfit should be the same, but the code 
used in these two options differs.  For real data I recommend now using the GREAT3 code */
#define GREAT_SIZE_CONVENTION (3)
						      								    
/* define whether to use the prior functional forms and size cut for GREAT3. 
   If this is set at all, the GREAT3 priors and ellipticity-dependent
   size cut will be used.  Comment out the following line for use on real data. */
//#define GREAT_PRIORS (1)

/* For real data, we do not extract postage stamps that are not fully covered by a CCD.  If you
   wish to remove this constraint (e.g. for GREAT3) then comment out the following define statement */
#define EXCLUDE_EDGES (1)
...
/* for GREAT3 we want to switch off the star fitting (set the following to zero) */
#define ALLOW_STAR_FIT 1
...
// switch this off for distortion-free simulations (eg GREAT3)
...
#define systematic 0.0

/*--------------------------------------------------------------------*/

/* function prototypes */

int open3Dfits(char*, fitsfile**, int, int, int);
int write3Dfits(fitsfile*, int, double*, int, int);
void read_prior_parameters (char *);
double G3sizecut(int);
double loadmodel(int, int, int, int, double, double);
void measure_ellipticity (int, int, double, complex double *,  
                          double*, double*, double*, double*);
int measure_marginalised_likelihood (int, int, int, int, int, int, int, 
...
int getglobalcoeffs(char *, double **, int, int, int, int, int, int, double *,
                    double *);
int readoscoeffs(char *, int *, double *, 
		 int, int, int,
		 double *, double *);
int getpsfsize_v5(char *, int *, int *, int *, int *, int *, int *, int *);
int getglobalpsfsizes(char *, int *, int *, int *, int *, int *, int *, int *, int *, int *,
                      int *, int *, int *, int *, int *, double *, double *, float *,
                      double *, int *, int *, int *, int *);
int getospsfsizes(char *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *,
                      int *, int *, int *, int *, int *, double *, double *, float *,
                      double *, int *, int *, int *, int *);
...
double likelihood(int, int, int, int, complex float *, complex float *);
...
int getimagesize(char *, char *, int *, float *, float *, float *, float *,
                 int *, int *, float);
void make2Dgalaxymodel(int, complex float**, double, double, double);
void make2Dgalaxymodelconv(int, complex float**, double, double, double);
void make3Dgalaxymodel (int, complex float**, double, double, double);
void complexconvolvemodel(fftw_plan *, fftw_complex *, complex float **, int, int,
                   fftw_complex **);
...
void globalreconstruct(double, double, int, int, int, int, int, int, double *,
                       double *);
void reconstructosPSF(int, double, double, int, int, int, int, double *);
int smatrix(int, double*, int*);
void downsample(int, double*);
void psfmoments(double *, int, int, double *, double *, double *);
//int varylpthresholdf3(int, int);
int detectobject(int, int, double **, int**, int*, float*);
double rfunc (double, float, double);
double efunc(double);
...
// set ellipticity posterior threshold at 0.02
// this is used when selecting samples in the final ellipticity estimate
float Pecut;

// set ellipticity posterior threshold at 0.02
// this is used when marginalising over size
float Prcut;

// set likelihood threshold at 0.05 in probability (i.e. enclosing 95% of a 2D gaussian)
// this is used when sampling the likelihood surface
float Lcut;
...
// the following variables can be set to test the code or to run in "fitclass update" mode
int skip_fitting;  // if this is set then all data postage stamps are emptied and no models are fitted
int skip_models;   // if this is set then no models are made
...
int nmaximages, chipvariation, chiporder, nfit, ncoeffs, forder, crossterm;
int *npix[NUM_THREAD], **spix[NUM_THREAD];
double **sval[NUM_THREAD], *psfindex[NUM_THREAD];
int numzero, nume, numR, edim, halfpwidth, halfpheight;
int expandedwidth, expandedheight;
int nsampling, npsfchip, xchipsampling, ychipsampling, big_gap, *xchip, *ychip;
int *chipnumber;
...
int *ochipnum, *catid;
int maximagefilelen;
int ***pnobj, **nnobj;  // arrays holding relative positions of other objects in postage stamps
int nnobjlim; // maximum number of object positions that can be stored per postage stamp
int nfoundexposures; // number of unique exposure names
...
FILE *shiftsfile, *asciifile;
char **exposurename, **uniqexpname;
char *badpixdir;
...
double efunc_a, efunc_emax, efunc_sigma, efunc_power;
// bulge ellipticity prior parameters
double bulgefunc_b, bulgefunc_c;
double priormoment;

double r0;   // characteristic scalelength of lognormal size distribution (PIXELS)
double rgamma; // sigma width of lognormal size distribution (PIXELS)
double rcut0; // lower bound on size distribution at zero ellipticity (PIXELS)

// parameters for nominal fit to log(median) from V606 Simard photometry
double afit[2];  
float refmag={23.0};
int iband, rband;

// camera identifiers
int kids, megacam, megaprime, suprime, single;
...
double *rawpos[NUM_THREAD], *wcspos[NUM_THREAD], *wtest[NUM_THREAD];
...
complex double **expandedft[NUM_THREAD];
complex double ***interpft[NUM_THREAD];
...
pthread_mutex_t fitsfilelock, shiftsfilelock, noiserefvallock,
  nimlock, memdlock, testlock; // , threadlock;
pthread_attr_t attr;
...
{
  // get random number from unix hardware stream, or otherwise from clock
 unsigned int seed;
 struct timeval tv;
 FILE *devrandom;
...
void addnoise(int nt, double *image, int width, int height, double nfactor, double nval)
{
  // add gaussian noise to a postage stamp within thread nt
  int i, isize;
  double sigma;

  if (noiserefval <= 0. || nval <= 0.)
    {
      fflush(stdout);
      fprintf(stderr," error in noise level inside addnoise %g %g \n",noiserefval, nval);
      exit(EXIT_FAILURE);
    }

  // the data are assumed normalised by noiserefval
  // the actual noise in the data is nval, which has not been normalised by noiserefval
  // scale the required noise by nfactor and
  // subtract unity so that we add on enough noise to make the
  // total noise what we want
  sigma = nfactor*nval/noiserefval - 1.;

  if (sigma > 0. && nfactor > 0.)
    {
      isize = width*height;
      for (i=0; i<isize; i++)
	{
	  // assume data is already normalised by the true rms noise
	  // and add additional noise required to make total noise
	  // nfactor larger
	  image[i] += gsl_ran_gaussian(gauss[nt],sigma);
	  // then renormalise data so that it is normalised to the
	  // new noise level
	  image[i] /= nfactor;
	}
    }
...
// find min, max allowed exponent 
    ...
    int gsl_status;
    unsigned int tot;
    unsigned long int seedval, oldseedval;
    ...
    char *prefsname, *pstr;
    char *starcatname, *shiftsfilename, *asciifilename;
    char *prior_parameters_file, *psf_type;
    char *Lcutenv, *Prcutenv, *Pecutenv;
    char *skipenv, *cameraenv, *wavebandenv;
    char *rawfile;
...
/*  read command-line and test the input arguments  */
    ...
    if (argc != 6 && argc != 8 && argc != 9) {
        printf
            (" %s <input file name> <output name> <postage stamp size> <start exposure> <end exposure> [<bright mag limit> <faint mag limit> <extra noise factor>]\n",
             argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    if (argc != 6 && argc != 7) {
        printf
            (" %s <input file name> <output name> <postage stamp size> <start exposure> <end exposure> [<extra noise factor>]\n",
             argv[0]);
        exit(EXIT_FAILURE);
    }
    */
    ...
    if (cameraenv != NULL)
      {
	kids = megacam = megaprime = single = suprime = 0;
	if (strncmp(cameraenv,"kids",1)==0 || strncmp(cameraenv,"KIDS",1)==0) {
	  ...
	  if (strncmp(cameraenv,"megacam",7)==0 || strncmp(cameraenv,"MEGACAM",7)==0) {
	    megacam = 1;
	    printf(" setting up for CFHT (36-CCD MegaCam) camera\n");
	  } else {
        if (strncmp(cameraenv,"megapri",7)==0 || strncmp(cameraenv,"MEGAPRI",7)==0) {
	    megaprime = 1;
	    printf(" setting up for CFHT (40-CCD MegaPrime) camera\n");
	  } else {
    ...
    for(nt = 0; nt < NUM_THREAD; nt++) {
        ...
        wtest[nt] = (double *) calloc(2, sizeof(double));
...
/* get input environment variables */
    ...
    // get the environment variable for the prior parameters file and read the prior values
    prior_parameters_file = getenv("PRIOR_PARAMETERS");
    if (prior_parameters_file != NULL) {
      read_prior_parameters(prior_parameters_file);
    } else {
      fflush(stdout);
      fprintf(stderr,
	      " ERROR no priors parameter file specified - use environment variable PRIOR_PARAMETERS \n");
      exit(EXIT_FAILURE);
    }

    // get the environment variable for the PSF type (not enabled at present because handling of
    // Fourier tranforms of convolved models is not correct - we need to store a complex array
    // and not just the real part
    oversampling = 1;
    psf_type = getenv("PSF_OVERSAMPLING");
    if (psf_type != NULL) {
      oversampling = atoi(psf_type);
      printf(" specified PSF oversampling %d\n",oversampling);
      if ((oversampling > 0 && 1+2*(oversampling/2) != oversampling) || oversampling > 5 || oversampling <= 0)
	{
	  fflush(stdout);
	  fprintf(stderr," error specifying PSF oversampling value (1,3,5) in environment variable\n");
	  exit(EXIT_FAILURE);
	}
    } else {
      fflush(stdout);
      fprintf(stderr,
	      " ERROR PSF oversampling not specified - use environment variable PSF_OVERSAMPLING \n");
      exit(EXIT_FAILURE);
    }
    ...
    // try to open the output ascii file
    asciifilename = (char *) calloc(500, sizeof(char));
    strcpy(asciifilename, argv[2]);
    strcat(asciifilename, ".asc");
	...
	fprintf(asciifile,"# lensfit catalogue %s \n",argv[1]);
	fprintf(asciifile,"#  1  wcsx\n");
	fprintf(asciifile,"#  2  wcsy\n");
	fprintf(asciifile,"#  3  bias-corrected <e1>\n");
	fprintf(asciifile,"#  4  bias-corrected <e2>\n");
	fprintf(asciifile,"#  5  weight \n");
	fprintf(asciifile,"#  6  fitclass \n");
	fprintf(asciifile,"#  7  bias-corrected scalelength /pixels \n");
	fprintf(asciifile,"#  8  bulge-fraction \n");
	fprintf(asciifile,"#  9  model-flux \n");
	fprintf(asciifile,"# 10  pixel SNratio \n");
	fprintf(asciifile,"# 11  model SNratio \n");
	fprintf(asciifile,"# 12  contamination radius \n");
	fprintf(asciifile,"# 13  <PSF>-e1 \n");
	fprintf(asciifile,"# 14  <PSF>-e2 \n");
	fprintf(asciifile,"# 15  <PSF>-Strehl-ratio \n");
	fprintf(asciifile,"# 16  <PSF>-Q11 \n");
	fprintf(asciifile,"# 17  <PSF>-Q22 \n");
	fprintf(asciifile,"# 18  <PSF>-Q12 \n");
	fprintf(asciifile,"# 19  star-galaxy f-probability\n");
	fprintf(asciifile,"# 20  r correction \n"); 
	fprintf(asciifile,"# 21  2D measurement variance \n");
	fprintf(asciifile,"# 22  mean-likelihood |e| \n");
	fprintf(asciifile,"# 23  e1 correction \n");
	fprintf(asciifile,"# 24  e2 correction \n");
	fprintf(asciifile,"# 25  neighbour mag \n");
	fprintf(asciifile,"# 26  neighbour distance \n");
	fprintf(asciifile,"# 27  catmag \n");
	fprintf(asciifile,"# 28  n-exposures-used \n");
	fprintf(asciifile,"# 29  cat-ID \n");
    fprintf(asciifile,"# 30  sensitivity \n");
    fprintf(asciifile,"# 31  sensitivity c1 \n");
    fprintf(asciifile,"# 32  sensitivity c2 \n");
    fprintf(asciifile,"# 33  delta-SNR \n");
    for (i=0; i<nmaximages; i++)
      {
        fprintf(asciifile,"# %2d  PSF e1 \n",34+i*2);
        fprintf(asciifile,"# %2d  PSF e2 \n",35+i*2);
      }
    ...
    uniqexpname = (char **) calloc(nimages, sizeof(char *));
    ...
    for(j = 0; j < nimages; j++) {
        ...
        uniqexpname[j] = (char *) calloc(32, sizeof(char));
        bzero(uniqexpname[j], 32);
        meme += (float) 2 * 32 *sizeof(char);
        bzero(tempname, 500);
	...
	*/
	...
	if (foundexposure==0) 
      {
        strcpy(uniqexpname[nfoundexposures],exposurename[i]);
        nfoundexposures++;
      }
    }
    ...
    if (oversampling > 1)
      {
	pindex = (int **) calloc(nimages, sizeof(int *));
	soln = (double **) calloc(nimages, sizeof(double *));
      }
    ...
    // create chip location identifiers
    xchip = (int*)calloc(1+nimages, sizeof(int));
    ychip = (int*)calloc(1+nimages, sizeof(int));
	...
	*/
	    ...
	    if (oversampling <= 1)
	      ...
	      strcat(psfname, ".ospsfcoeffs.fits");
	     ...
	     * NB scalefactor is a large number - units of pixels per radian
             */

	    if (oversampling <= 1) {
	      getglobalpsfsizes(psfname, &correctd, &originalpwidth, &originalpheight,
			      &order[j], &chipvariation, &chiporder,
                  &ncoeffs, numstars[j], &npsfchip, xchip, ychip,
	      ...
	      int psfoversampling;
	      getospsfsizes(psfname, &correctd, &fwidth, &fheight, &psfoversampling,
			    &order[j], &chipvariation, &chiporder,
                &ncoeffs, &nfit, numstars[j], &npsfchip, xchip, ychip,
			    &xchipsampling, &ychipsampling, &big_gap,
			    &hxsize[j], &hysize[j], &psfposerror[j], &scalefactor[j],
			    &sorder, &schipvariation, &schiporder,
			    &sncoeffs);
		...
		{
		  fflush(stdout);
		  fprintf(stderr," mismatch in user-requested PSF oversampling and oversampling of PSF files\n");
		  exit(EXIT_FAILURE);
		}
	      //printf(" oversampling %d\n",oversampling);
	      //	printf(" mosaic %d x %d \n",nxchip,nychip);
	      // printf(" sampling %d x %d \n",xchipsampling,ychipsampling);
	      originalpwidth = fwidth/oversampling;
	      originalpheight = fheight/oversampling;
	      // padded model sizes
	      mwidth = 2*fwidth;
	      mheight = 2*fheight;
	      // find next highest power of two for speed
	      int npower = 1 + (int)(log(mwidth-1)/log(2.));
	      mwidth = mheight = pow(2, npower);
	      printf(" creating oversampled x%d PSFs of padded size %d x %d \n", oversampling, mwidth, mheight);
	    ...
	    if (CORRECT_DISTORTION == 0)
	    ...
	    else 
	      {
            ...
            if (chipnumber[j] > 0) {
                ...
                if (PSF_STAR_LIMIT > 0 && numstars[j][chipnumber[j] - 1] < PSF_STAR_LIMIT) {
                    printf
	...
	if (oversampling > 1 && postage_stamp_size != pwidth)
	  {
	    fflush(stdout);
	    fprintf(stderr,"error in specifying postage stamp size, must be %d\n",pwidth);
	    exit(EXIT_FAILURE);
	  }
	...
	if (postage_stamp_size > pwidth)
	...
	else
	  {
	    ...
	    if (psfwarning == 0)
	      {
		printf(" postage stamp size %d will be used \n",pwidth);
		psfwarning = 1;
	      }
	  }
	...
	if (oversampling <= 1)
	...
	else
	  {
	    if (nfit<=0 || ncoeffs<=0)
	      {
		fflush(stdout);
		fprintf(stderr," error in specified number of oversampled coefficients, %d %d\n",nfit,ncoeffs);
		exit(EXIT_FAILURE);
	      }
	    if (fwidth*fheight>0)
	      {
		soln[j] = ml_calloc(nfit*ncoeffs, sizeof(double), &meme, "soln");
		pindex[j] = ml_calloc(fwidth*fheight, sizeof(int), &meme, "pindex");
		readoscoeffs(psfname, pindex[j], soln[j], nfit, ncoeffs,
			     sncoeffs, xcoeffs[j], ycoeffs[j]);
	      }
	    else
	      {
		fflush(stdout);
		fprintf(stderr," error in oversampled psf size \n");
		exit(EXIT_FAILURE);
	      }
	  }
            /*
    ...
    for(nt = 0; nt < NUM_THREAD; nt++) {
      ...
      if (oversampling > 1)
	{
	  npix[nt] = ml_calloc((pwidth*pheight), sizeof(int), &meme, "mixing matrix number of elements");    
	  spix[nt] = ml_calloc((pwidth*pheight), sizeof(int*), &meme, "mixing matrix image element pointer");    
	  sval[nt] = ml_calloc((pwidth*pheight), sizeof(double*), &meme, "mixing matrix image element value pointer");    
	  for (i=0; i<pwidth*pheight; i++)
	    {
	      spix[nt][i] = ml_calloc(nfit, sizeof(int), &meme, "mixing matrix nfit elements");    
	      sval[nt][i] = ml_calloc(nfit, sizeof(double), &meme, "mixing matrix image element values");    
	    }
	  psfindex[nt] = (double*)calloc(nfit, sizeof(double));
	}
  ...
  /* set the sampling of the size-ellipticity parameter space
    ...
    bfsigma = 0.08;  // doesn't seem a good fit for G3 but we have to assume a gaussian in this code

#ifdef GREAT_PRIORS
    /* fraction of pure bulge-only galaxies */
    bulgepopfrac = 0.025; // G3 fraction in deep training set at S/N > 150
    fluxpriorslope = 0.0; // adopt flat flux prior for G3
#else
    /* fraction of pure bulge-only galaxies */
    bulgepopfrac = 0.1;
    ...
    minsamplepoints = 30;  /* target number of points in (e1, e2) in final measurement */
    /************************************************************/
    ...
    // test for environment variables holding threshold cuts, or default otherwise 
    Pecut = 0.02;
    Pecutenv = getenv("PECUT");
    if (Pecutenv != NULL) { Pecut = atof(Pecutenv); }

    Prcut = 0.02;
    Prcutenv = getenv("PRCUT");
    if (Prcutenv != NULL) { Prcut = atof(Prcutenv); }

    Lcut = 0.05;
    Lcutenv = getenv("LCUT");
    if (Lcutenv != NULL) { Lcut = atof(Lcutenv); }

    // test for whether we should skip model-making and fitting
    skip_fitting = skip_models = 0;
    skipenv = getenv("SKIP");
    if (skipenv != NULL) {
      skip_fitting = skip_models = atoi(skipenv); 
      if (skip_fitting==1) printf(" *** skipping making models and fitting \n");
    }

    printf(" Ellipticity posterior cut %4.2f, size cut %4.2f, likelihood threshold cut %4.2f\n", Pecut, Prcut, Lcut);

    if ((int)GREAT_SIZE_CONVENTION > 0) {
      if ((int)GREAT_SIZE_CONVENTION == 3) {
	printf(" using GREAT3 size convention \n");
      } else {
	printf(" using GREAT10 size convention \n");
      }
    } else {
	printf(" using lensfit size convention \n");
    }

    if ((int)ALLOW_STAR_FIT == 0) printf(" star fitting switched off\n");
...
#ifdef GREAT_PRIORS
    printf(" using GREAT3 lognormal distribution for scalelength prior\n");
#else
    printf(" using CFHTLenS scalelength prior functional form\n");
#endif
    ...
    // with M_g < -19 (LENSFIT STANDARD DEFAULT)
    //efunc_a = 0.0256;    // intrinsic ellipticity term
    //efunc_emax = 0.804;  // cut-off ellipticity 
    //efunc_sigma = 0.2539; // cut-off sigma term

    // basic ellipticity and size priors are now read from file
    // see read_prior_parameters()
    ...
    // calculate prior moment using fine sampling of 0.001
    double priornorm;
    double emod, eeprior, pstep = 0.001;
    int pdim = 1 + (int)(1./pstep);
    priornorm = priormoment = 0.;
    for (j=0; j<pdim; j++)
      {
	emod = j*pstep;
	eeprior = 
	...
	// calculate moments
	priornorm += eeprior*emod;
	priormoment += eeprior*pow(emod,3);
      }
    priormoment /= priornorm;
    printf(" ellipticity prior sigma = %f \n",sqrt(priormoment/2.));
    ...
    // add a guard region to help with poor sampling near edge
    ...
    if (oversampling <= 1)
      {
	// msf = 1 + 2*((int)(10.*max_aob*Rcutoff/pwidth)/2);  // must be an odd integer multiplier
	...
	printf(" model array expansion factor %d \n", msf);
      ...
      {
	printf(" model array oversampling %d \n", oversampling);
      }
    ...
    // set limiting magnitudes (both bright and faint limits must be specified)
    blim = 0.;
    flim = 1000.;
    if (argc == 8)
      {
    ...
    if (argc == 9)
      {
	// get noise factor from command line
	extranoisefactor = atof(argv[8]);
	if (extranoisefactor > 1.)
	  {
	    printf(" adding extra noise to postage stamps with factor %f \n",extranoisefactor);
	    // initialise GSL random number generator, one for each thread
	    T = gsl_rng_ranlux;
	    oldseedval = 0;
	    for (nt=0; nt<NUM_THREAD; nt++)
	      {
		gauss[nt] = gsl_rng_alloc(T);
		seedval = random_seed();
		// make sure seed is different in each thread
		while (seedval == oldseedval)
		  seedval = random_seed();
		gsl_rng_set(gauss[nt],seedval);
		oldseedval = seedval;
	      }
	  }
      }
...
/* read the input catalogue */
    ...
    output_string = (char**) calloc(nobj, sizeof(char*));
    for (i=0; i<nobj; i++) output_string[i] = (char*) calloc((290+nmaximages*16), sizeof(char));

    wcsx = (double *) calloc(nobj, sizeof(double));
    wcsy = (double *) calloc(nobj, sizeof(double));
    catmag = (float *) calloc(nobj, sizeof(float));
    neighbourmag = (float *) calloc(nobj, sizeof(float));
    neighbourdistance = (float *) calloc(nobj, sizeof(float));
    ...
    rcontam = (float *) calloc(nobj, sizeof(float));
    ...
    for (j=0; j<nobj2; j++)
      {
	if (catmag[j]<5. || catmag[j]>100. || catid[j]<0 )
	  {
	    fflush(stdout);
    ...
    // find nearest neighbour magnitude and distances above flim
    for (i=0; i<nobj; i++)
      {
        neighbourdistance[i] = 1000.; // closest separation in arcsec
        neighbourmag[i] = 99.;
        for (j=0; j<nobj; j++)
          {
            if (i !=j && (catmag[j] < catmag[i] || catmag[j] < flim) )
              {
                sepdec = 3600.*(wcsy[i]-wcsy[j]);
                if (fabs(sepdec) < neighbourdistance[i])
                  {
                    sepra = 3600.*(wcsx[i]-wcsx[j])*cos(wcsy[i]*pi/180.);
                      if (fabs(sepra) < neighbourdistance[i])
                        {
                          // separation in arcsec assuming wcsx is RA and dec in degrees
                          sep = sqrt(pow( sepra, 2 ) + pow( sepdec, 2 ) );
                          if (sep < neighbourdistance[i]) 
                            {
                              neighbourdistance[i] = sep;
                              neighbourmag[i] = catmag[j]-catmag[i];
                            }
                        }
                  }
              }
    ...
    if (catminmag < blim) { catminmag = blim; }
    ...
    pmax = rfunc(rval,catminmag,0.);
    while (rfunc(rval,catminmag,0.) > pmax-1.609) {rval *= 1.05;}
    ...
    */
    ...
    Rmin = rcut0;  // value read from prior parameters file
    ...
    */
    ...
    if (nim == NULL) {
      fflush(stdout);
      fprintf(stderr,"memory allocation error for number of images, nim \n");
      exit(EXIT_FAILURE);
    }
    ...
    for(nt = 0; nt < NUM_THREAD; nt++) {
        ...
        if (dpix[nt] == NULL) {
	  fflush(stdout);
	  fprintf(stderr,"Memory allocation error for dpix\n");
	  exit(EXIT_FAILURE);
        }
        meme += (float) pwidth *pheight * sizeof(double);
    }
	...
	expandedft[nt] = (complex double**)calloc(2, sizeof(complex double*));
	meme += 2.*sizeof(complex double*);
	if (CORRECT_DISTORTION==2)
	  {
	    for (mt=0; mt<2; mt++)
	      {
		expandedft[nt][mt] = (complex double*)calloc(expandedheight*expandedwidth, sizeof(complex double));
		...
		meme += (float)expandedheight*expandedwidth*sizeof(complex double);
	      }
	  }
	// interpolated model array inside likelihood function
	interpft[nt] = (complex double***)calloc(nmaximages, sizeof(complex double**));
	meme += (float)nmaximages*sizeof(complex double**);
	for (i=0; i<nmaximages; i++)
	  {
	    interpft[nt][i] = (complex double**)calloc(2, sizeof(complex double*));
	    meme += 2.*sizeof(complex double*);
	    for (mt=0; mt<2; mt++)
	      {
		interpft[nt][i][mt] = (complex double*)calloc(pheight*halfpwidth, sizeof(complex double));
		...
		meme += (float)pheight*halfpwidth*sizeof(complex double);
	      }
	  }
    ...
    nnobjlim = 20;
    pnobj = ml_calloc(nobj, sizeof(int**), &datamemory, "pnobj");
    nnobj = ml_calloc(nobj, sizeof(int*), &datamemory, "nnobj");
    for(i = 0; i < nobj; i++) {
	...
	// allocate memory for contaminating object positions
	pnobj[i] = ml_calloc(nmaximages, sizeof(int*), &datamemory, "pnobj[]");
	nnobj[i] = ml_calloc(nmaximages, sizeof(int), &datamemory, "nnobj[]");
	for (jj=0; jj<nmaximages; jj++)
	  {
	    pnobj[i][jj] = ml_calloc(nnobjlim, sizeof(int), &datamemory, "pnobj[][]");
	  }
    }

    // printf(" pnobj allocated\n"); fflush(stdout);


    datamemory1 = datamemory;

    //    printf (" successfully allocated memory for data postage stamps \n");
    // fflush(stdout);
...
/*  figure out how many ellipticity values there will be in the grid and allocate
   ...
   */
	...
	for (i=0; i<nstringcat; i++)
	  ...
	  fits_report_error(stderr, status); } /*  print error message  */
        if (fits_update_key(ofptr[ii], TDOUBLE, "PSCALE", &defaultmeanscalefactor,
                            "r prior parameter", &status)) {
	  fits_report_error(stderr, status); } /*  print error message  */
	if (fits_update_key(ofptr[ii], TFLOAT, "PCUT", &Pecut,
			    "posterior e threshold", &status)) {
	  fits_report_error(stderr, status); } /*  print error message  */
	if (fits_update_key(ofptr[ii], TFLOAT, "PRCUT", &Prcut,
			    "posterior r threshold", &status)) {
	  fits_report_error(stderr, status); } /*  print error message  */
     ...
     */
    modelmemory =
        (2. * (float) (numR * nume) * (float) modelsize * sizeof(complex float)
         + numR * nume * sizeof(complex float **)
         + numR * nume * 2. * sizeof(complex float *));
    if (VERBOSE == 1)
            ...
            if (meanmoments[0] <= 0.) 
              {
                ...
                for(j = 0; j < nimages; j++) 
                  {
                    if (no_psf[j] == 0 && badccd[j] == 0) 
                      {
                        // identify the chip
                        if (chipnumber[j] <= 0 || npsfchip <= 0) 
                          {
                            fflush(stdout);
                            fprintf(stderr,
                        ...
                        */
                        if (GLOBAL_PSF == 0) {
                          ...
                          for(pixel = 0; pixel < (pwidth * pheight); pixel++) {
                            ...
                            xval =
                                ((xchip[ichip] + 0.5) * xchipsampling) / hxsize[j] - 1.;
                            yval = (ychip[ichip] + 0.5) * ychipsampling;
                            /*
                            ...
                            if (oversampling <= 1)
                              ...
                              {
                                // load the oversampled model into array star
                                reconstructosPSF(0, xval, yval, forder, j, ichip, (int)NCHIPS, star[0]); 
                                // and also created psf at native sampling 
                                ospos[0] = ospos[1] = 0.;
                                smatrix(0, ospos, pindex[j]);
                                downsample(0, psf[0]);
                              }
                        }
                        ...
                        if (VERBOSE == 1)
                          printf(" moments %f %f %f \n",meanmoments[0],meanmoments[1],meanmoments[2]);
                ...
                // if constant psf, create its fourier transform now
                // the transform is stored in array starA[0][0]
                if (oversampling > 1)
                  {
                    fftw_execute(moffatpsf[0]);
                  }
                ...
                if (minmoment > 0.) {
                  ...
                  fprintf(stderr,
                          ...
                          minmoment);
                  fprintf(stderr," probably caused by error in handling PSF file for this chip\n");
                  exit(EXIT_FAILURE);
                }
              }
...
/* table to identify each object */
		...
		for (i=0; i<testsize; i++) otestout[i]=0.;
             ...
             */
	    if (skip_models == 1) {
	      printf(" not making models \n");
	    } else {
		if (modelft == NULL) {
		  modelft = (complex float***)ml_calloc((numR*nume), sizeof(complex float**), &memm, "modelft");
		  for(i = 0; i < numR * nume; i++) {
		    modelft[i] = (complex float**)ml_calloc(2, sizeof(complex float*), &memm, "modelft[]");
                    for(j = 0; j < 2; j++) {
		      modelft[i][j] = (complex float*)ml_calloc(modelsize, sizeof(complex float), &memm, "modelft[][]");
                    }
		  }
		  printf
	    ...
	    if (skip_fitting != 1) {
	      printf
                (" measuring shapes for galaxies, catalogue numbers %d <= n <= %d \n",
                 nmin + 1, nmax); } else {
	      printf
                (" generating outputs for galaxies, catalogue numbers %d <= n <= %d \n",
                 nmin + 1, nmax); }
            fflush(stdout);
    ...
    for(i = 0; i < nobj; i++) {
        if (fitclass[i] == 0) {
            ...
            if (fthresh > 0.95)
                fthresh = 0.95;
            if (fprob[i] < fthresh && rmar[i]<=0.5 && (int)ALLOW_STAR_FIT == 1) {
              fitclass[i] = 2;
                row = i + 1;
                // update FITS table column
                fits_write_col(ofptr[0], TINT, 11, row, 1, 1, &fitclass[i],
                               &status);
                // overwrite relevant parts of output string in ascii table
                // making sure that remainder of string is retained
                strncpy(output_string[i]+46," 2 ",3);
                tgoodfit--;
                tstarfit++;
                tswap++;
            }
          ...
          if (maxboxno[i] <= 0 || numpsfstars >= PSF_STAR_LIMIT) {
              ...
              if (USE_SWARP == 0)
                {
                    ...
                    edistvals[0]=edistvals[3]=180.*3600./M_PI/defaultmeanscalefactor;
			...
			// set the scale and distortion parameters
                  ...
                  else
                    {
                      ...
                      /*
                        if (ngoodpix==0 && VERBOSE==1) 
                        {
                        printf(" object %d image %d pos %f %f %f %f \n",
                        ii,i,wcsx[ii],wcsy[ii],objx[i][ii],objy[i][ii]);
                        }
                      */
                      if (CORRECT_DISTORTION == 0)
                        {
            ...
            if (ngoodpix > 0) {
                ...
                if (mval >= snlimit || nim[ii] > 0
                    ...
                    thisnim = nim[ii];
                    nim[ii]++;
                    if (nim[ii] > nmaximages)
                      {
                        fflush(stdout);
                        fprintf(stderr," error making data stack for galaxy %d\n",ii+1);
                        fprintf(stderr," galaxy found on %d images when maximum %d expected\n",nim[ii],nmaximages);
                        for (j=0; j<nimages; j++)
                          {
                            if (objx[j][ii] > 0. && objy[j][ii] > 0.)
                              {
                                fprintf(stderr,"%s at %f %f\n",image_file[j],objx[j][ii],objy[j][ii]);
                              }
                          }
                        exit(EXIT_FAILURE);
                      }
		    ...
		    for (j=0; j<4; j++)
		      edist[ii][thisnim][j] = edistvals[j];
                    ...
                    //printf(" image %d extracted %d \n",i,extractedobject[i][ii]); fflush(stdout);
		    ...
		    // add noise - because the data have been normalised by
		    // noiserefval, the amplitude of noise can be specified
		    // just as the command-line argument
		    if (extranoisefactor > 1.)
		      {
			if (noise[i] > 0.)
			  {
			    addnoise(nt, dpix[nt], pwidth, pheight, extranoisefactor, noise[i]);
			  }
			else
			  {
			    fflush(stdout);
			    fprintf(stderr," image noise not specified at addnoise \n");
			    exit(EXIT_FAILURE);
			  }
		      }
		    ...
		    // find relative positions of other objects on these postage stamps
		    //printf("object %d nobj %d \n",ii,nobj); fflush(stdout);
		    for (kobj =0; kobj<nobj; kobj++) {
		      // check for valid object locations on this image excluding the current object
		      if (kobj != ii && objx[i][kobj] > -999. && objy[i][kobj] > -999.)
			{
			  dx = objx[i][kobj]-objx[i][ii];
			  dy = objy[i][kobj]-objy[i][ii];
			  //printf(" object %d %d image %d dpos %f %f \n",ii,kobj,i,dx,dy); fflush(stdout);
			  // check if it's on the postage stamp
			  if (fabs(dx)<pwidth/2 && fabs(dy)<pheight/2) {
			    idx = (int)(dx+pwidth+0.5) - pwidth/2;
			    idy = (int)(dy+pheight+0.5) - pheight/2;
			    pixel = idx + idy*pwidth;
			    // printf ("pixel %d %d %d \n",idx,idy,pixel); fflush(stdout);
			    // double check that it's not in the centre (should be target object)
			    if (pixel != (pwidth/2) + (pheight/2)*pwidth) {
			      // store pixel and number of such objects up to some limit
			      pnobj[ii][thisnim][nnobj[ii][thisnim]] = pixel;
			      if (nnobj[ii][thisnim] < nnobjlim-1) {
				nnobj[ii][thisnim]++;
			      }
			    }
			  }
	...
	for(j = 0; j < nimages; j++) {
	  ...
	  if (jj >= 0) {
        ...
        if (nimcheck > 0) {
          ...
          dstatus = detectobject(nt, i, Dreal[i], pnobj[i], nnobj[i], &rcontam[i]);
...
{
    int i, nt, more, nfound; 
    int ii, j, jj, jim, x, y, xx, yy, xs, maxchisqpos;
    int nRo;
    int close, pixel, modelnum, ichip, nchips, nret;
    ...
    double maxrfitvals, fval;
    double distsq;
    double bulgeflux, diskflux, bulgefraction, totalflux;
    double mbulgeflux, mdiskflux, mtotalflux, bflocal;
    double profile_sn, profile_sn_num, profile_sn_denom;
    double *sn_num, *sn_denom, maxsnr, meansnr, meansnr_num, diffsnr;
    double rprob = 0.;
    double maxlikel, maxposterior, zerolikel, mzerolikel, maxchisq;
    double Lmaxlikel;

    double dist, xval, yval, normvalue, meane1, meane2;
    double chisquared, reducedchisq, bestsize, delt, deltmin;
    double psfe[2], centroid[2], pmoments[3], meanpsfe[2], meanstrehl, meanpsfm[3], psfemod;
    double de[2], dshift[2];  
    double starflux, bestpos[2], starbestpos[2];
    double Lthreshold, Pthreshold, val, likelval;
    double emod, sump, sumprior, beta, mval, pval, pvalmax, posterior;
    double sensitivity[3], rsens[3];
    double eeprior;
    double minimumvalue, maximumvalue;
    double lchisq, rndf, rchisq, imchisq, imndf;
    ...
    complex double ecorr, eshift, oldeshift, oldecorr, einput, edelta;
    complex double edata, emeasured;
    double rmeasured, rdata, rshift, oldrshift, rdelta;
    double gweight, gvariance, mweight, mvariance;
    double ospos[2];
     ...
     */
    bestne = bestnRo = bestsize = 0;
    nimcheck = 0;
    starbestpos[0] = starbestpos[1] = 0.;
    bestpos[0] = bestpos[1] = 0.;

    // allocate memory for individual SNR estimates
    sn_num = (double*)calloc(nmaximages, sizeof(double));
    sn_denom = (double*)calloc(nmaximages, sizeof(double));
	...
	pthread_mutex_lock(&threadlock);
      printf(" objectloop thread %d object %d \n",nt,i); fflush(stdout);
      */
	...
	iter = 0;
            ...
            meanpsfe[0] = meanpsfe[1] = 0.;
	    meanpsfm[0] = meanpsfm[1] = meanpsfm[2] = 0.;
            meanstrehl = 0.;
	    meanscalefactor=0.;
	    sumw=0.;
                ...
                for(j = 0; j < nimages; j++) {
                    ...
                    if (jim >= 0) {
                        if (GLOBAL_PSF == 0) {
                          ...
                          //ychip = ichip / nxchip;
                          //xchip = ichip - ychip * nxchip;
                          xval = (objx[j][i] + xchip[ichip] * xchipsampling) / hxsize[j] - 1.;
                          yval = objy[j][i] + ychip[ichip] * ychipsampling;
                          /*
                          ...
                          if (oversampling <= 1)
                            ...
                            {
                              reconstructosPSF(nt, xval, yval, forder, j, ichip, nchips, star[nt]); 
                              ospos[0] = ospos[1] = 0.;
                              smatrix(nt, ospos, pindex[j]);
                              downsample(nt, psf[nt]);
                            }
                        ...
                        // remove check on peak position (done for G3 but probably best left like this)
                        int peaktol = pwidth;

                        if (
                            // normvalue < 0.97 || normvalue > 1.03 || 
                            psfemod > psfelimit
                            || x > peaktol || x < -peaktol || y > peaktol || y < -peaktol 
                          ...
                          // printf(" bad psf %f %f %d %d %f %f \n",normvalue,psfemod,x,y,minimumvalue,maximumvalue);
                            ...
                            */
                            ...
                            meanpsfe[1] += psfe[1];
                            meanstrehl += psf[nt][0];
                            meanpsfm[0] += pmoments[0];
                            meanpsfm[1] += pmoments[1];
                            meanpsfm[2] += pmoments[2];
                            sumw += weight[j];
                            ...
                            if (oversampling <= 1)
                              ...
                              {
                                // in the case of the constant PSF, the PSF is already
                                // convolved with the models, so B is set to unity
                                for (ii=0; ii<halfpwidth*pheight; ii++)
                                  {
                                    B[nt][nimcheck][ii] = 1. + 0.*I;
                                  }
                              }
		...
		// if skip_fitting has been set, empty the data postage stamps to prevent fitting
		if (skip_fitting==1)
		  {
		    // exit the loop early for testing
            // normalise PSF averages
            if (nimcheck>0) 
              {       
                meanpsfe[0] /= (double)nimcheck;
                meanpsfe[1] /= (double)nimcheck;
                meanstrehl /= (double)nimcheck;
                meanpsfm[0] /= (double)nimcheck;
                meanpsfm[1] /= (double)nimcheck;
                meanpsfm[2] /= (double)nimcheck;
              }
            // fill up the output buffer string
		    sprintf(output_string[i],
                    "%10.6lf %10.6lf %7.4f %7.4f %7.4f %2d %7.4f %6.4f %8.1f %8.1f %8.2f %6.2f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %5.2f %5.2f %5.2f %3d %8d %7.4f %8.6f %8.6f %8.2f", 
			    wcsx[i], wcsy[i], 0., 0., 0.,
			    fitclass[i], 0., 0., 0., snratio[i], profile_sn,
			    rcontam[i],
			    meanpsfe[0], meanpsfe[1], meanstrehl, 
			    meanpsfm[0], meanpsfm[1], meanpsfm[2],
			    0., 0.,
			    0., 0., 0., 0.,
			    neighbourmag[i], neighbourdistance[i],
                    catmag[i], nimcheck, catid[i], 0., 0., 0., 0. );
            // add PSF ellipticities of individual exposures onto end of output string
            // go through the unique exposures
            for (jj=0; jj<nfoundexposures; jj++)
              {
                nfound = 0;
                // select only the images that match this exposure
                for(j = 0; j < nimages; j++) 
                  {
                    if (strcmp(exposurename[j],uniqexpname[jj])==0)
                      {
                        // find which image has been used for this object
                        jim = extractedobject[j][i];
                        if (jim >= 0) 
                          {
                            len = strlen(output_string[i]);
                            sprintf(output_string[i]+len,
                                    " %7.4f %7.4f",psfevals[nt][jim][0],psfevals[nt][jim][1]);
                            nfound++;
                          }
                      }
                  }
                if (nfound > 1)
                  {
                    fprintf(stderr,
                            " error, %d exposures named %s the same for object %d\n",
                            nfound,uniqexpname[jj],i+1);
                    exit(EXIT_FAILURE);
                  }
                if (nfound==0)
                  {
                    len = strlen(output_string[i]);
                    sprintf(output_string[i]+len,
                            " %7.4f %7.4f",99.0000,99.0000);
                  }
              }
            // empty the data and reset data flags
		    for (j=0; j<nimages; j++)
		      {
                jj = extractedobject[j][i];
                if (jj>=0)
		...
		*/
		...
		for(nRo = 1; nRo < numR; nRo++) {
		  ...
		  rprior[nt][nRo] = rfunc(Rarcsec, catmag[i], meanscalefactor);
		}
		 ...
		 * and if it has enough exposures
			...
			bestnRo++;
		      */
		      // set starting scale length from maximum of prior distribution
		      double rfuncmax = -1.e10;
		      for (nRo=1; nRo<numR; nRo++)
			{
			  Rarcsec = Ro[nRo]*defaultmeanscalefactor;
			  if (rfunc(Rarcsec, catmag[i], defaultmeanscalefactor) > rfuncmax)
			    {
			      bestnRo = initnRo = nRo;
			      rfuncmax = rfunc(Rarcsec, catmag[i], defaultmeanscalefactor);
			    }
			}
				...
				if (fittedrstep[nt][ne] > -1)
				  {
				    ...
				    if (nret >= 2)
				      {
                        fflush(stdout);
                        fprintf(stderr," error from recall (line 7546) of findmaxr, nret = %d object %d\n",nret,i+1);
					//exit(EXIT_FAILURE);
				      }
				    /*
				    if (VERBOSE ==1 && (i==0 || (i==0 && nmax==1))) {
					...
					if (nret<2)
					  {
					    ...
					    if (nret==0) {
					      if (rfitvals[nt][ne][0] > 0. && rfitvals[nt][ne][2] > 0.) {
						newrstep = 1 + 
		      ...
		      while (more==1 && nmore < nmorelimit && maxrfitvals > NOT_MEASURED)
			{
			  ...
			  // measure marginalised likelihood			  
			  exception += 
			    measure_marginalised_likelihood 
			    (nt, i, nimcheck,
			     sampling, initsampling, 
			     initnRo, initrstep, 
			     zerolikel,
			     &maxlikel, &maxposterior,
			     printlikel);
			  ...
			  //printf("thread %d object %d new points tried\n",nt,i+1);
			  //fflush(stdout);

			  // set likelihood thresholds at Lcut, Pecut
			  // (as set here, select only on likelihood)
			  Lthreshold = maxlikel + log(Lcut);
			  Pthreshold = maxposterior; // + log(Pecut);
			  ...
			  // iterations if not
			  if (VERBOSE==1) printf(" thread %d object %d iteration %d maxlikel %g\n",nt, i+1, nmore, maxlikel);
			  for (ne=0; ne<nume; ne++)
			    {
			      if (old_wanted[nt][ne]==0 && wanted[nt][ne]==1 )
				{
				  ...
				  if (VERBOSE==1) 
				    {
				      printf(" thread %d object %d iteration %d added %f %f %g\n",nt,i+1,nmore,e1[ne],e2[ne],marP[nt][ne]);
				    }
				  */
			  ...
			  if (more==0)
			      ...
			      if (numwanted < minsamplepoints && sampling > 1)
				{
				  ...
				  if (VERBOSE==1) 
				    {
				      printf(" thread %d object %d iteration %d changed sampling to %d\n",nt,i+1,nmore,sampling);
				    }
				  */
				}
			    }
		      ...
		      if (nmore >= nmorelimit)
			{
			  ...
			  // this limit was exceeded for bright stars while the algorithm was 
			  // exploring the galaxy likelihood surface.  I think if we just let it 
			  // quietly continue it should be OK, but let's output a warning to be
			  // cross-checked later.
			  
			  //exit(EXIT_FAILURE);
			  // Ian Fenech Conti : Removed the exit caluse and set the error to 
			  // FitClass of -13.
			  //fitclass[i] = -13;
			}
		      ...
		      if (VERBOSE==1)
			{
              ...
              if (Lmaxlikel + 0.5 * sumdata[nt] > 7.035 ||
                  zerolikel + 0.5 * sumdata[nt] > 7.035
                  ) {
			/*
			...
			// measure ellipticity.
			// In this version replace the marginalised size with the 
			// likelihood-corrected maximum-likelihood size, integrated
			// over the size prior but not the ellipticity prior (so
			// that the returned size is consistent with the returned
			// ellipticity)

			// printf(" object %d maxlikel %g \n",i+1,maxlikel);

            measure_ellipticity(nt, sampling, maxlikel, &ecorr, &rmar[i], &gweight, &gvariance, sensitivity);

			//printf(" object %d first bias result %f %f \n", i+1, creal(ecorr), cimag(ecorr));
			
			// remember these data values
			edata = ecorr;
			rdata = rmar[i];
			...
			*/
			...
			// normalise the likelihood surface 
			beta = 0.;
			for(ne = 0; ne < nume; ne++)
			  {
			    if (marP[nt][ne] > NOT_MEASURED) {
			...
			if (beta > 0.)
			...
			else
			  {
			    if (fitclass[i] >= -4)
			      {
                    printf(" warning, object %d , fitclass %d, error normalising marP, beta = %g \n",
                           i+1,fitclass[i],beta);
                    if (fitclass[i]==0)
                      {
                        printf(" fitclass reset to -3\n");
                        fitclass[i] = -3;
                      }
                    fflush(stdout);
			      }
	    ...
	    // remember the component fluxes for later
	    mdiskflux = diskflux;
	    mbulgeflux = bulgeflux;
	    mtotalflux = mdiskflux+mbulgeflux;
	     ...
	     */ 
            if (fitclass[i] >= 0 || ( fitclass[i] < -3 && fitclass[i] != -8) ) {
	      // printf("making model, object %d thread %d fitclass %d\n",i+1,nt,fitclass[i]); fflush(stdout);
              /*  
              if (fitclass[i] == 1) {
		  ...
		  // otherwise use corrected galaxy parameters
          */
		  // - find closest ready-made model
		  deltmin = 1.e10;
		  bestne = 0;
		  for (ne=0; ne<nume; ne++)
		    {
		      delt = cabs(e1[ne]+I*e2[ne] - ecorr);
		      if (delt < deltmin)
                {
                  deltmin = delt;
                  bestne = ne;
		  ...
		  for (nRo = 1; nRo<numR; nRo++)
		    {
		      delt = fabs(Ro[nRo] - rmar[i]);
		      if (delt < deltmin && Ro[nRo] > G3sizecut(bestne))
                {
                  deltmin = delt;
                  bestnRo = nRo;
                  //printf("object %d reset bestnRo to %d %f \n", i+1, bestnRo, Ro[bestnRo]);
                }
		    }
		  //printf("best values %d %f %f %d %f %f \n",bestne,e1[bestne],e2[bestne],bestnRo,Ro[bestnRo],G3sizecut(bestne));
		  // set the bias-corrected galaxy model 
		  modelnum = bestnRo * nume + bestne;
          //}
                ...
                for(j = 0; j < nimages; j++) {
                    ...
                    if (jj >= 0) {
                        ...
                        complexconvolvemodel(pinv[nt], B[nt][thisnim],
                ...
                if (mcross2 > 0.) {
                    ...
                    if (mcross0 <= 0.) {
                        ...
                        if (mcross1 <= 0.) {
                            fflush(stdout);
                            fprintf(stderr,
                                    " remake model error at object %d, all cross-components zero or negative, fitclass %d \n",i+1,fitclass[i]);
                            exit(EXIT_FAILURE);
                        }
                    }
                    // otherwise model error
                        ...
                        fflush(stdout);
                        fprintf(stderr,
                                " remake model error at object %d, zero cross-component, fitclass %d \n",i+1,fitclass[i]);
                        exit(EXIT_FAILURE);
                    }
                }
                if (fabs(denom) > 0.01) {
		...
		// iterate the bias correction to accuracy of 0.001 in ellipticity
		// or up to maximum number of iterations set below
		int bias_iter = 0;
		eshift = 1.;
		rshift = 1.;
		oldecorr = ecorr + 1.;
		int bias_iter_limit = 1;  // maximum one iteration on bias correction
		// loop while...
		while (cabs(ecorr-oldecorr) > 0.0001 &&  // corrected ellipticity has changed by > .0001
		       cabs(eshift) > 0.001              // difference between data and model ellipticity > .001
		       && bias_iter < bias_iter_limit    // less than iterations limit
               )
          // && fitclass[i] <= 0)              // excluding anything classified as star
		  {
		    // use galaxy parameters
		    // - find closest ready-made model
		    deltmin = 1.e10;
		    bestne = 0;
		    for (ne=0; ne<nume; ne++)
		    ...
		    // having identified the best model ellipticity, now find the maximum likelihood size
		    // using the same function calls as in the original model fitting
		    if (fittednRo[nt][ne] > -1)
		      {
                bestnRo = fittednRo[nt][ne];
		      }
		    ...
		    // as before - it won't take much time as the likelihood function
		    // values will be remembered inside findmaxr
		    Ploop = 1;  // find maximum likelihood, not maximum posterior
		    if (fittedrstep[nt][ne] > -1)
		      {
                rstep = fittedrstep[nt][ne];
                nret = findmaxr (Ploop, nt, nimcheck, bestnRo, ne, rstep, rfitvals[nt][ne]);
                if (nret >= 2)
                  {
                    fflush(stdout);
                    fprintf(stderr," error from recall (line 8496) of findmaxr, nret = %d object %d\n",nret,i+1);
                    //exit(EXIT_FAILURE);
                  }
                else
                  {
            ...
            for(j = 0; j < nimages; j++) {
              ...
              if (jj >= 0) {
                ...
                imchisq = 0.;
                imndf = 0;
                sn_num[thisnim] = sn_denom[thisnim] = 0.;
                for(y = 0; y < pheight; y++) {
                  for(x = 0; x < pwidth; x++) {
                    pixel = x + y * pwidth;
                    // make galaxy model 
                    ...
                    // also calculate SNR in each exposure to be used for asteroid detection
                    sn_num[thisnim] += weight[j]*mval*Dreal[i][jj][pixel];
                    sn_denom[thisnim] += weight[j]*mval*mval;
                    /*
                ...
                // assemble the model into the data postage stamps 
                // so that they can be remeasured as if they were data
                // set the bulge fraction to be fixed
                bflocal = mbulgeflux/mtotalflux;
                /*
                  if (bflocal > 0.9)
                  {
                  // high measured bulge fraction, set to pure de Vauc
                  mbulgeflux = mtotalflux;
                  mdiskflux = 0.;
                  }
                  else
                  {
                  // otherwise set to the prior-expected bulge fraction
                  // (avoids concern that the bf is very noisy and may be biased)
                  mdiskflux = (1.-bfsigma)*mtotalflux;
                  mbulgeflux = mtotalflux - mdiskflux;
                  }
                */
                lchisq = loadmodel(nt, i, jj, thisnim, mdiskflux, mbulgeflux);
            ...
            // calculate SNR in each exposure
            // and then the difference between the SNR in the highest-SNR exposure and
            // the mean SNR of the other exposures, as a measure of asteroid detection
            maxsnr = 0.;
            for (ii=0; ii<thisnim; ii++)
              {
                if (sn_denom[ii]>0.)
                  {
                    sn_num[ii] /= sqrt(sn_denom[ii]);
                    if (sn_num[ii] > maxsnr)
                      {
                        maxsnr = sn_num[ii];
                      }
            ...
            for (ii=0; ii<thisnim; ii++)
              {
                if (sn_num[ii]>0. && sn_num[ii]<maxsnr)
                  {
                    meansnr += sn_num[ii];
                    meansnr_num++;
                  }
            ...
            if (meansnr_num > 0)
              {
                diffsnr = maxsnr - meansnr/meansnr_num;
              }
                    
            // calculate overall profile-weighted S/N ratio (all exposures)
            ...
            // reinitialise fitting routines
            maxlikel = maxposterior = maxrfitvals = NOT_MEASURED;
            for (ne=0; ne<nume; ne++)
              {
                fittednRo[nt][ne] = fittedrstep[nt][ne] = -1;
                ...
                for (nRo=0; nRo<numR; nRo++)
                  {
                    modelnum = nRo*nume + ne;
                    likel[nt][modelnum] = NOT_MEASURED;
                  }
              }
            maxrposterior[nt] = NOT_MEASURED;
            /* reset distortion parameters to no distortion
               because the simulated galaxy has been made distortion-free.  The PSF
               does have distortion included, so this is not quite right, but should
               not significantly affect the bias estimation */
            for (jj=0; jj<nimcheck; jj++)
              {
                edist[i][jj][0]=edist[i][jj][3]=1.;
                edist[i][jj][1]=edist[i][jj][2]=0.;
              }

            nRo = ne = modelnum = 0;
            // printf(" starting bias test object %d thread %d \n",i+1,nt); fflush(stdout);
            mzerolikel = likelihood(nt, nimcheck, nRo, ne, modelft[modelnum][0], modelft[modelnum][1]);
            // printf(" star model fit OK %d %lf thread %d \n",i+1,mzerolikel,nt); fflush(stdout);

            // now rerun the fitting on this model using the same sampling 
            ...
            measure_marginalised_likelihood 
              (nt, i, nimcheck,
               ...
               printlikel);
            /*
              printf(" model object %d maxlikel %g calls %d thread %d \n",i+1,maxlikel,ncall[nt]-n0,nt); 
              fflush(stdout);
            */

            // remeasure
            measure_ellipticity(nt, sampling, maxlikel, &emeasured, &rmeasured, &mweight, &mvariance, rsens);
            ...
            einput = e1[bestne] + I*e2[bestne]; 
            
            // evaluate the model bias as the shift in parameters from the data
            eshift  = edata - emeasured;
            rshift = rdata - rmeasured;

            // alternatively consider the model bias as the shift between input and output
            edelta = einput - emeasured;
            rdelta = Ro[bestnRo] - rmeasured;

            // limit how large the shift can be
            if (cabs(edelta) > (2*sampling*e_interval)) edelta *= (2*sampling*e_interval)/cabs(edelta);
            if (fabs(rdelta) > 1.0) rdelta *= 1.0/fabs(rdelta);

            // apply the bias correction 
            // check that the remeasured position is approaching the original measurement
            // after the first iteration
            if (bias_iter==0 || 
                (cabs(eshift) <= cabs(oldeshift) && fabs(rshift) <= fabs(oldrshift))
                )
              {
                ecorr = edata + edelta;
                rmar[i] = rdata + rdelta;
              ...
              {
                /*
                  printf("bias stop %d %d %f %f %f %f \n",bias_iter,i+1,
                  creal(oldeshift),cimag(oldeshift),
                  creal(eshift),cimag(eshift),
                  oldrshift, rshift);
                */
                bias_iter = bias_iter_limit; // stop the loop
              }
		    
            // force loop stop if at extreme values after first correction applied
            if (cabs(ecorr) > efunc_emax || rmar[i] <= Ro[1] || rmar[i] >= Ro[numR-1] ) bias_iter = bias_iter_limit;

            if (VERBOSE == 1)
              {
                printf("bias results: iter %d object %d , %f %f , %f %f , %f %f , %f %f , %f %f , %f %f\n",
                       bias_iter, i+1, 
                       creal(edata), cimag(edata), 
                       creal(ecorr), cimag(ecorr), 
                       creal(oldeshift), cimag(oldeshift), 
                       creal(eshift), cimag(eshift), 
                       rdata, rmar[i], oldrshift, rshift);
              }
		...
		// make sure illegal corrected values are downsized
		if (cabs(ecorr) >= 0.99) ecorr *= efunc_emax/cabs(ecorr);

		// free the postage stamp memory
		for(j = 0; j < nimages; j++) {
          jj = extractedobject[j][i];
          if (jj >= 0) {
            // free this image now it's no longer needed
	    ...
	    // apply a cut on objects with high weight but large measurement variance
	    if (profile_sn > 0. && pow(profile_sn,2)*gvariance < 1. ) 
	      {
            gweight = 0.;
            if (fitclass[i]==0)
              {
	    ...
	    // fill up the ascii output string
	    emod = cabs(ecorr);

	    if (skip_fitting != 1) {
	      sprintf(output_string[i],
		    "%10.6lf %10.6lf %7.4f %7.4f %7.4f %2d %7.4f %6.4f %8.1f %8.1f %8.2f %6.2f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %5.2f %5.2f %5.2f %3d %8d %7.4f %8.6f %8.6f %8.2f", 
                  wcsx[i], wcsy[i], creal(ecorr), cimag(ecorr), gweight,
                  fitclass[i], rmar[i], bulgefraction, totalflux, snratio[i], profile_sn, rcontam[i],
                  meanpsfe[0], meanpsfe[1], meanstrehl, 
                  meanpsfm[0], meanpsfm[1], meanpsfm[2],
                  fprob[i], rdelta,
                  gvariance, emod, creal(edelta), cimag(edelta),
                  neighbourmag[i], neighbourdistance[i],
                  catmag[i], nimcheck, catid[i],
                  sensitivity[0], sensitivity[1], sensitivity[2], diffsnr
                  );
          // add PSF ellipticities of individual exposures onto end of output string
          // go through the unique exposures
          for (jj=0; jj<nfoundexposures; jj++)
	    ...
	    /*
	    printf(" objectloop thread %d object %d finished \n",nt,i); fflush(stdout);
	    if (i>13000)
  ...
  for (nRo=nt; nRo<numR; nRo+=NUM_THREAD)
      ...
      for (ne=0; ne<nume; ne++)
	{
	  ...
	  if (MODELS_3D == 1)
	    ...
	    {
	      if (nRo == 5 && ne == nume/10) printout[nt]=1;
	      if (oversampling > 1)
		{
		  make2Dgalaxymodelconv(nt, modelft[modelnum], e1[ne], e2[ne], Ro[nRo]);
		}
	      else
		{
		  make2Dgalaxymodel(nt, modelft[modelnum], e1[ne], e2[ne], Ro[nRo]);
		  //printf(" models %f %f %f \n",e1[ne],e2[ne],Ro[nRo]);
		  //fflush(stdout);
		}
	    }
...
double
likelihood(int nt, int nim, int nRo, int ne, complex float *ftd, complex float *ftb)
{
    complex double model, diskmodel, bulgemodel;
    double maxval, area;
    double sumC, likel, bulgelikel;
    ...
    /*
    if (currentobject[nt]==printlikel) 
      {printf(" start likelihood %d %d %f %f \n",nRo,ne,e1[ne],e2[ne]);fflush(stdout);}
    */
		...
		for (x=0; x<halfpwidth; x++)
		  {
		    ...
		    expandedft[nt][mt][opixel] = (complex double)ft[mt][ipixel];
		  ...
		  for (x=0; x<halfpwidth; x++)
		    {
		      ...
		      interpft[nt][i][mt][opixel] = (complex double)ft[mt][opixel];
		    }
		}
	    }
	}
	...
	if (nRo > 0)
	  {
	    for(y = 0; y < pheight; y++) {
	      x = 0;
	      yx = y * halfpwidth + x;
	      diskmodel = interpft[nt][i][0][yx] * B[nt][i][yx] / fsize;
	      bulgemodel = interpft[nt][i][1][yx] * B[nt][i][yx] / fsize;
	      ...
	      //{
	      crossproduct[2] +=
                nimweight[nt][i] * creal(diskmodel*conj(bulgemodel));
	      //}
	      ...
	      diskmodel = interpft[nt][i][0][yx] * B[nt][i][yx] / fsize;
	      bulgemodel = interpft[nt][i][1][yx] * B[nt][i][yx] / fsize;
	      crossproduct[2] += nimweight[nt][i] * creal(diskmodel*conj(bulgemodel));
	      
	      for(x = 1; x < (halfpwidth - 1); x++) {
                yx = y * halfpwidth + x;
		diskmodel = interpft[nt][i][0][yx] * B[nt][i][yx] / fsize;
		bulgemodel = interpft[nt][i][1][yx] * B[nt][i][yx] / fsize;
		crossproduct[2] += 2. * nimweight[nt][i] * creal(diskmodel*conj(bulgemodel));
	      }
	    }
	  }
      }
    ...
    if (nRo > 0 && crossproduct[2] <= 0.)
      {
	fflush (stdout);
	fprintf (stderr, " crossproduct model error thread %d \n",nt);
	fprintf (stderr, " crossproduct %g \n", crossproduct[2]);
	for (i=0;  i<nim; i++)
	  {
	    fprintf( stderr, " image %d nimweight %f fsize %g\n",i,nimweight[nt][i],fsize);
	    fprintf (stderr, " central real %g %g %g \n central imag %g %g %g \n",
		     creal(interpft[nt][i][0][0]),creal(interpft[nt][i][1][0]),creal(B[nt][i][0]),
		     cimag(interpft[nt][i][0][0]),cimag(interpft[nt][i][1][0]),cimag(B[nt][i][0])
		     ); 
	    yx=0;
	    diskmodel = interpft[nt][i][0][yx] * B[nt][i][yx] / fsize;
	    bulgemodel = interpft[nt][i][1][yx] * B[nt][i][yx] / fsize;
	    fprintf (stderr, " central cross %g + I %g , %g + I %g , %g \n",
		     creal(diskmodel),cimag(diskmodel),creal(bulgemodel),cimag(bulgemodel),
		     creal(diskmodel*conj(bulgemodel)));
	  }
	exit (EXIT_FAILURE);
      }
    ...
    for(mt = mtstart; mt < 2; mt++) {
        ...
        for(y = 0; y < pheight; y++) {
            x = 0;
            yx = y * halfpwidth + x;
	    model = interpft[nt][i][mt][yx] * B[nt][i][yx] / fsize;
            sumC += nimweight[nt][i] * pow(cabs(model),2);
            C[nt][mt][yx] = nimweight[nt][i] * Dp[nt][i][yx] * conj(model);
	    ...
	    model = interpft[nt][i][mt][yx] * B[nt][i][yx] / fsize;
            ...
            for(x = 1; x < (halfpwidth - 1); x++) {
                yx = y * halfpwidth + x;
		model = interpft[nt][i][mt][yx] * B[nt][i][yx] / fsize;
		sumC += 2. * nimweight[nt][i] * pow(cabs(model),2);
		C[nt][mt][yx] = nimweight[nt][i] * Dp[nt][i][yx] * conj(model);
            }
        }
    ...
    if (nRo > 0 && crossproduct[2] > 0.) {
        ...
        if (nRo == 0) {
            ...
            if (crossproduct[1] <= 0.) {
                fflush(stdout);
                fprintf(stderr,
                        " model error, all cross-components zero or negative, thread %d \n",nt);
                exit(EXIT_FAILURE);
            }
        }
        // otherwise model error
            ...
            fflush(stdout);
            fprintf(stderr, " model error, zero cross-component, thread %d \n", nt);
	    fprintf (stderr, " vals %f %f %f %f %f ",
		     crossproduct[0],crossproduct[1],crossproduct[2],
		     c[nt][0][0],c[nt][1][0]); 
            exit(EXIT_FAILURE);
        }
	...
	*/
    ...
    if (currentobject[nt]==printlikel) 
      {printf(" end likelihood %d %d %f %f %g \n",nRo,ne,e1[ne],e2[ne],likel);fflush(stdout);}
...
// GREAT3 size prior
double rfunc(double r, float mag, double meanscalefactor)
{
  // G3 size prior - returns log(prior)
  // lognormal size distribution
  // currently magnitude-invariant (magnitude retained as function argument for code consistency)

  double y, tmag, rpriorval;

  // magnitude minus reference value - not currently used
  tmag = mag-refmag;

  // convert r in arcsec back to r in pixels
  r /= meanscalefactor;

  if (r>0. && rgamma>0. && r0>0.)
    {
      y = log(r/r0);
      rpriorval = -0.5*y*y/rgamma/rgamma - log(r) - 0.5*log(2.*M_PI) - log(rgamma);
    }
  else
    {
      rpriorval = log(1.e-20);
    }
...
// original lensfit size prior
// meanscalefactor is not used, as function works in arcsec, but argument is included for
// code consistency
  ...
  if (eval < efunc_emax)
    {
      ...
      prior = (1.-exp((eval-efunc_emax)/efunc_sigma))/pow((1.+eval),efunc_power);
...
read_images()
{
    ...
    wcstesttol = pow((1./60.),2);  // set the WCS test tolerance to a very generous value, 1 arcmin (squared)
                ...
                for(ii = nmin; ii < nmax; ii++) {
                    ...
                    if (wcs_to_raw(wcs_raw[i], wcspos[nt], rawpos[nt]) ==
                      ...
                      if (catmag[ii] >= blim && catmag[ii] <= flim) {
                        // test if this x,y coord lies within the current image
                        if (rawpos[nt][0] > 0.&& rawpos[nt][0] < (double) rawfield[i]->width) {
                          if (rawpos[nt][1] > 0. && rawpos[nt][1] < (double) rawfield[i]->height) {
                            // convert back to wcs to check the lookup was correct
                            // (can fail for multivalued astrometric solutions)
                            raw_to_wcs(wcs_raw[i], rawpos[nt], wtest[nt]);
                            if (*wtest[nt] == WCS_NOCOORD) {
                              fflush(stdout);
                              fprintf(stderr," error in inverse position lookup \n");
                              fprintf(stderr, " image pixel position %lf %lf \n", rawpos[nt][0],rawpos[nt][1]);
                              exit(EXIT_FAILURE);
                            }
                            wcstest =
                              pow(((wtest[nt][0]-wcspos[nt][0])/cos(wcspos[nt][1]*pi/180.)),2) +
                              pow((wtest[nt][1]-wcspos[nt][1]),2);
                            if (wcstest < wcstesttol)
                            ...
                            else
                              {
                                ...
                                printf(" WARNING input object rejected by WCS test %lf %lf -> %lf %lf -> %lf %lf\n",
                                       wcspos[nt][0],wcspos[nt][1],
                                       rawpos[nt][0],rawpos[nt][1],
                                       wtest[nt][0],wtest[nt][1]);
                                fflush(stdout);
                              }
                          }
                        }
...
detectobject(int nt,            /* thread number */
	     ...
	     int **pixn,        /* pixel position of contaminating objects */
	     int *nn,           /* number of contaminating objects */
	     float *rc          /* radius from nominal centre to nearest contaminating object pixels */
    )
{
    ...
    /* status return values are:
       ...
       6 - more than one catalogue object found in the target segmented region (added 20/8/2015, LM)
    ...
    // test if contaminating catalogue objects are found in the target segmented region
    if (target >= 0) {
      for(j = 0; j < nimages; j++) {
        // choose valid images from the stack 
        jj = extractedobject[j][obj];
        if (jj >= 0) {
          // test contaminating position against segmentation map
          for (kobj=0; kobj<nn[jj]; kobj++) {
            if (colour[nt][pixn[jj][kobj]] == target) {
              status = 6; // set status = 6 if object found in target-coloured pixel
            }
          }
        }
      }
    ...
    for(i = 1; i <= n; i++) {
      for(j = 0; j < size; j++) {
        if (colour[nt][j] == i) {
          ...
          // set the distance to the nearest contaminating pixels
          if (radius < *rc && i != target) {
            *rc = radius;
          }
                       ...
                       )
{
    ...
    double rdisk, rbulge, diskRo, xval, yval; 
    ...
    double rval, pval, wbulge, C, Cdisk;
    //double dflux, wdisk;
    double cosi, sini;
    ...
    */
    ...
    // make models with either the GREAT convention on how size relates to ellipticity, or the
    // more physical standard lensfit way

    if ((int)GREAT_SIZE_CONVENTION > 0) {

      halfmwidth = 1 + mwidth / 2;

    // G10 definition of scalelength:
    ScaledRo = Ro * msf;
    bScaledRo = Ro * msf;
    ...
    /* set factors that control the galaxy size following the shear transformation that converts
       circular isophotes into elliptical ones.  In Great10, this is done by equation A8 of Bridle
       et al (2010) (although that eqn appears to have an error), which makes the area of a galaxy
       invariant under the shear transformation.  Hence we need an appropriate determinant here */
    factor = bfactor = 1./(1.-e1*e1-e2*e2);

    // However, the GREAT3 convention differs from this, so if we wish to use it for G3 we must also
    // include the following correction (which switches the defintion back to semi-major axis as
    // in the original lensfit definition):
    if ((int)GREAT_SIZE_CONVENTION == 3) {
      factor *= 1./beta;
      bfactor *= 1./betab;
    }

    // set the constant inside the exponent for bulges 
    // according to the bulge Sersic index, using the same approximation
    // that is used in G10 (see Bridle et al 2010, equation A2), noting Sindex = 1/n.
    C = 2./Sindex - 0.331;

    // set also the scaling for the disk scalelength according to Bridle et al eqn A2
    // this choice means that diskRo is defined as halflight radius
    //Cdisk = 2.-0.331;
    // but for consitency with earlier versions of lensfit, esp CFHTLenS, here we now
    // set Cdisk=1 and the diskRo then becomes the exponential disk scalelength
    Cdisk = 1;

    eta = etab = sqrt(e1*e1 + e2*e2);
	...
	for (y=0; y<mh; y++)
	  {
		...
		xval = (1.-e1)*xx - e2*yy;
		yval = (1.+e1)*yy - e2*xx;	      
		r = sqrt(bfactor*(xval*xval + yval*yval));
		if (r <= brmax) {
		  rbulge = r / bulgeRo / Cdisk; // make sure that bulge halflight radius equals disk exponential scalelength
		  if (rbulge <= Rcutoff) {
		    expfactor = C * (pow(rbulge, Sindex) - 1.);
		    sb = exp(-expfactor);
		    bulge[nt][yp*mw+xp] = sb;
		    bflux += sb;
		  }
		}
	      }
	...
	for (y=0; y<mh; y++)
	  {
		...
		xval = (1.-e1)*xx - e2*yy;
		yval = (1.+e1)*yy - e2*xx;	      
		r = sqrt(factor*(xval*xval + yval*yval));
		if (r <= rmax) {
		  rdisk = r / diskRo;
		  if (rdisk <= Rcutoff) {
		    disk[nt][yp*mw+xp] = exp(-Cdisk*rdisk);
		  }
		}
	      }
	  }
    ...
    */    
	...
	if (Sindex==0.25)
	  {
	    ...
	    wbulge = ((6. + pval*C*(6.+pval*C*(3.+C*pval)))/6.
		      ...
		      5040.)*exp(-C*pval);
	    wbulge = 1. - wbulge;
	    mflux = 2. * pi * beta * bulgeRo * bulgeRo * exp(C) * wbulge * 20160. / pow(C, 8.);
	  }
	else if (Sindex==0.5)
	  {
	    /* Sersic index=2 */
	    ...
	    mflux = 2. * pi * beta * bulgeRo * bulgeRo * exp(C) * wbulge * 12. / pow(C, 4.);
	  }
	else
	  {
	    fflush(stdout);
	...
	// prevent hole appearing in centre
	for (y=-1; y<=1; y++)
	  {
	    yp = y>=0 ? y : y+mheight;
	    for (x=-1; x<=1; x++)
	      {
		xp = x>=0 ? x : x+mwidth;
		if (bulge[nt][0] < bulge[nt][yp*mwidth + xp])
		  {
		    bulge[nt][0] = bulge[nt][yp*mwidth + xp];
		  }
	      }
	  }

        //psfellipticitycentroid(bulge,mheight,mwidth,psfe,centroid);
        // printf("bulge %f %f %f %f %f %f %f %f \n",bulgeRo,e1,e2,beta,wbulge,mflux,bflux,bulge[nt][0]);
		...
		for(mt = 0; mt < 2; mt++) 
		  {
		    ...
		    if (Ro>0. && cabs(A[nt][0])<=0.)
		      {
			fflush(stdout);
			fprintf(stderr," error making FT of model convolved with pseudo-star \n");
			fprintf(stderr," %f %f %f %f\n",cabs(AA[nt][mt][0]),cabs(A[nt][0]),cabs(starA[nt][0]),mfsize);
			fprintf(stderr," %f %f %f \n",Ro,e1,e2);
			fprintf(stderr," %d %d %d \n",mw,mh,msf);
			fprintf(stderr," %f %f \n",disk[nt][0],bulge[nt][0]);
			exit(EXIT_FAILURE);
		      }
		    ...
		    // renormalise and write into modelft
		    nfactor = 1.;
		    if (cabs(A[nt][0]) > 0. && cabs(dstarA[nt][0])>0.)
		      nfactor = cabs(A[nt][0]/dstarA[nt][0]);
			...
			for (x=0; x<halfpwidth; x++)
			  {
			    ...
			    if (cabs(dstarA[nt][i]) != 0.)
			      {
				modelft[mt][i] += A[nt][i]/dstarA[nt][i]/nfactor;
			      }
			    else
			      {
				fflush(stdout);
			...
			fprintf(stderr," mt %d, A %f %f , dstarA %f %f , modelft %f \n", mt,
				creal(A[nt][0]),cimag(A[nt][0]),creal(dstarA[nt][0]),cimag(dstarA[nt][0]),cabs(modelft[mt][0]));
			exit(EXIT_FAILURE);
		      }
		    for (y=0; y<pheight; y++)
		      {
	...
	for(mt = 0; mt < 2; mt++) 
	  {
            ...
            // renormalise and write into modelft
            nfactor = 1.;
            if (cabs(AA[nt][mt][0]) > 0. && Ro > 0.)
	      nfactor = cabs(AA[nt][mt][0]);
	    for (y=0; y<pheight; y++)
	      {
		for (x=0; x<halfpwidth; x++)
		  {
		    i = y*halfpwidth + x;
		    modelft[mt][i] = AA[nt][mt][i]/nfactor;
		  }
	      }
	  }
      }
...
void
make2Dgalaxymodelconv (int nt,
                       complex float **modelft, 
                       double e1, double e2, double Ro
                       )
{
  // makes convolved galaxy model with invariant PSF suitable for constant PSF G3 images
    int i, mt, halfmwidth, x, y, yp, xp, xx, yx;
    int ix, iy, yy, pheight, opixel, ipixel;
    int mw, mh, msf, dx, dy, ds;
    double r, sb, etasq, factor, expfactor, ScaledRo, bScaledRo, bulgeRo;
    double rdisk, rbulge, diskRo, xval, yval, xv, yv; 
    double eta, etat, beta, e1t, e2t;
    double etab, etabt, etaobs, betab, e1bt, e2bt, etabsq, bfactor, brmax;
    double mflux, bflux, rmax;
    double nfactor;
    ...
    if ((int)GREAT_SIZE_CONVENTION > 0) {
      ...
      ScaledRo = Ro * oversampling;
      bScaledRo = Ro * oversampling;
	
	if (ScaledRo > 0.) {
	
	// additional oversampling factor when making surface brightness distributions
	// so that minor axis is oversampled 
	// must be an odd integer
	msf = 1 + 2*(int)(4./(ScaledRo*beta)/2.);
	ds = msf/2;
	...
	// However, the GREAT3 convention differs from this, so if we wish to use it for G3 we must also
	// include the following correction (which switches the definition back to semi-major axis as
	// in the original lensfit definition):
	if ((int)GREAT_SIZE_CONVENTION == 3) {
	  factor *= 1./beta;
	  bfactor *= 1./betab;
	...
	for (y=0; y<mh; y++)
	  {
	    // recentre array
	    yy = y - mh/2;
	    if (yy >= -fheight/2 && yy <= fheight/2)
	      {
		// find quadrant swapped output pixel
		yp = yy>=0 ? yy : yy+mh;
		for (x=0; x<mw; x++)
		  {
		    xx = x - mw/2;
		    if (xx >= -fwidth/2 && xx <= fwidth/2)
		      {
			xp = xx>=0 ? xx : xx+mw;
			xval = (1.-e1)*xx - e2*yy;
			yval = (1.+e1)*yy - e2*xx;
			// average pixels in isotropic space
			for (dy=-ds; dy<=ds; dy++)
			  {
			    yv = yval + (double)dy/(double)msf;
			    for (dx=-ds; dx<=ds; dx++)
			      {
				xv = xval + (double)dx/(double)msf;
				r = sqrt(factor*(xv*xv + yv*yv));
				// bulge
				if (r <= brmax) {
				  rbulge = r / bulgeRo / Cdisk; // make sure that bulge halflight radius equals disk exponential scalelength
				  if (rbulge <= Rcutoff) {
				    expfactor = C * (pow(rbulge, Sindex) - 1.);
				    sb = exp(-expfactor)/(msf*msf);
				    bulge[nt][yp*mw+xp] += sb;
				    bflux += sb;
				  }
				}
				...
				if (r <= rmax) {
				  rdisk = r / diskRo;
				  if (rdisk <= Rcutoff) {
				    disk[nt][yp*mw+xp] += exp(-Cdisk*rdisk)/(msf*msf);
				  }
				}
			      }
			  }
	...
	// printf(" bulge %f %f %f %f %f %f %f \n",e1,e2,Ro,mflux,bflux,bulge[nt][0],bulge[nt][1]);
...
/* FT the disk and bulge components*/
    ...
    // convolve each of disk and bulge models
    for(mt = 0; mt < 2; mt++) 
      {
	if (Ro>0. && cabs(AA[nt][mt][0])<=0.)
	  {
	...
	if (cabs(starA[0][0])<=0.)
	  {
	    fflush(stdout);
	    fprintf(stderr," error, FT of constant PSF is zero \n");
	    exit(EXIT_FAILURE);
	  }
	for (y = 0; y < mheight; y++) 
	  {
	    for(x = 0; x < halfmwidth; x++) 
	      {
		yx = x + y * halfmwidth;
		A[nt][yx] = AA[nt][mt][yx]*starA[0][yx]/mfsize;
	      }
	  }
	if (Ro>0. && cabs(A[nt][0])<=0.)
	  {
	    fflush(stdout);
	    fprintf(stderr," error making FT of model convolved with pseudo-star \n");
	    fprintf(stderr," %f %f %f %f\n",cabs(AA[nt][mt][0]),cabs(A[nt][0]),cabs(starA[0][0]),mfsize);
	    fprintf(stderr," %f %f %f \n",Ro,e1,e2);
	    fprintf(stderr," %d %d %d \n",mw,mh,oversampling);
	    fprintf(stderr," %f %f \n",disk[nt][0],bulge[nt][0]);
	    exit(EXIT_FAILURE);
	...
	for(y = 0; y < mheight; y++) 
	  {
	    ...
	    if (y<mheight/2)
	      {
		iy = (y + oversampling/2)/oversampling;
	      }
	    else
	      {
		iy = (mheight - y + oversampling/2)/oversampling;
		iy = -iy;
	      }
	    // check the new array element lies inside pheight
	    yy = iy + pheight/2;
		...
		for(x = 0; x < mwidth; x++) 
		  {
		    if (x<mwidth/2)
		      {
			ix = (x + oversampling/2)/oversampling;
		      }
		    else
		      {
			ix = (mwidth - x +oversampling/2)/oversampling;
			ix = -ix;
		      }
		    xx = ix + pwidth/2;
		    if (xx >=0 && xx < pwidth)
			...
			opixel = ix + iy * pwidth;
			ipixel = x + y * mwidth;
			dmodel[nt][opixel] += disk[nt][ipixel]/(oversampling*oversampling);
		      }
		  }
	      }
	  }
	...
	// next model, bulge or disk
      ...
      fprintf(stderr," this version requires G3 convention\n");
      exit(EXIT_FAILURE);
    }
...
void make3Dgalaxymodel (int nt, complex float **ft, double e1, double e2, double Ro)
{
       ...
       for (mt=0; mt<2; mt++)
	 {
	   ...
	   for (iy=0; iy<pheight; iy++)
		 ...
		 fftw_execute(starsmodel[nt]);
		  ...
		  and write into modelft */
	       nfactor = 1.;
	       if (cabs(A[nt][0]) > 0. && cabs(dstarA[nt][0]) > 0.) 
		 nfactor = cabs(A[nt][0]/dstarA[nt][0]);
	       else
		 if (Ro>0. || mt==1)
		   printf(" normalisation error: %f %f \n",cabs(A[nt][0]),cabs(dstarA[nt][0]));
	       for (y=0; y<pheight; y++)
		 {
		   for (x=0; x<halfpwidth; x++)
		     {
		       ...
		       if (cabs(dstarA[nt][i]) != 0.)
			 {
			   ft[mt][i] = A[nt][i]/nfactor/dstarA[nt][i];               
			 }
		       else
			 {
			   fflush(stdout);
	   ...
	   else
	     {
	       ...
	       // renormalise and write into modelft
	       nfactor = 1.;
	       if (cabs(A[nt][0])>0. && Ro>0.) nfactor = cabs(A[nt][0]);
	       for (y=0; y<pheight; y++)
		 {
		   for (x=0; x<halfpwidth; x++)
		     {
		       i = y*halfpwidth + x;
		       ft[mt][i] = A[nt][i]/nfactor;               
		     }
		 }
	     }
...
int findmaxr (int Pselect, int nt, int nimcheck, int startnRo, int ne, int rstep, double *fvals)
{
  ...
  //if (currentobject[nt]==printlikel) printf(" findmaxr ne= %d rstep= %d\n",ne,rstep); fflush(stdout);
...
double G3sizecut(int ne)
{
  // calculates lower size bound in G3, in units of pixels
  // if GREAT_PRIORS is set to 1
  // The GREAT3 challenge had an ellipticity-dependent size cut
  // which is reproduced here after fitting to the G3 deep data

  double eval, rcut;

#ifdef GREAT_PRIORS
      eval = sqrt(e1[ne]*e1[ne]+e2[ne]*e2[ne]);
      // size limit in pixels
      rcut = rcut0*pow((1.+eval),0.75);
#else
      eval = 0.;
      rcut = 0.;
#endif
...
int marginaliser(int nt, int iobj, int nimcheck, int bestnRo, int rstep, int ne, double zerolikel)
{
  ...
  double xmin, xmax, xcut, logPrcut;
  ...
  // stop when posterior has dropped below threshold defined by log(Prcut)
  nRmax = bestnRo + rstep;
  while ( nRmax<numR && ( (nRmax-bestnRo)<2 || posterior>maxrposterior[nt]+logPrcut ) )
    {
      nRo = nRmax;
      modelnum = nRo*nume + ne;
      priorval = rprior[nt][nRo]; //+eprior[nt][ne];
  ...
  while (nRmin>0 && ( (bestnRo-nRmin)<2 || posterior>maxrposterior[nt]+logPrcut ) )
    {
      nRo = nRmin;
      modelnum = nRo*nume + ne;
      priorval = rprior[nt][nRo]; // + eprior[nt][ne];
  ...
  if (maxrposterior[nt] > NOT_MEASURED) {
    ...
    if (nRo < numR) {
      ...
      priorval = rprior[nt][nRo] - rfunc(Rarcsec, catmag[iobj], defaultmeanscalefactor);  // adjust for varying Ro
	...
	ymarvals[nt][num_marvals[nt]] = 
	  ymval + priorval + rfunc(Rarcsec, catmag[iobj], defaultmeanscalefactor) - maxrposterior[nt];
	num_marvals[nt]++;
	xmval += Ro[nRo] / 5.;
      }
      /*
      ...
      if (currentobject[nt]==printlikel)
	{
      ...
      */
      ...
      if (currentobject[nt]==printlikel)
	{
	  printf(" start marginaliser gsl spline functions\n"); fflush(stdout);
	}
      */
      fspline[nt] = gsl_spline_alloc(gsl_interp_cspline,num_marvals[nt]);
      if (fspline[nt] == NULL) {
	...
	exit(EXIT_FAILURE);
      ...
      if (currentobject[nt]==printlikel)
	{
	  printf(" marginaliser gsl spline functions OK\n"); fflush(stdout);
	}
      */
      ...
      for (nRo=1; nRo<numR; nRo++)
	{
      ...
      if (currentobject[nt]==printlikel)
	{
	  printf(" marginaliser spline evaluation ok\n"); fflush(stdout);
	}
      */
      // redefine the lower bound to the integration limit
      // measured in units of pixels
      xcut = G3sizecut(ne);
      if (xcut > xmin) xmin = xcut;
      if (xmin < xmax) {
      // if there is a valid range of size,
	...
	(&marF, xmin, xmax,0., 1.e-3, gsl_work_size,
	 GSL_INTEG_GAUSS41,gsl_work[nt], &sump, &error);
      ...
      if (currentobject[nt]==printlikel)
	{
	  printf(" marginaliser gsl spline status %s\n",gsl_strerror(gsl_status)); fflush(stdout);
	}
      */
      if (gsl_status) {
	...
	if (gsl_status) {
	  ...
	  */
      ...
      if (currentobject[nt]==printlikel)
	{
	  printf(" start marginaliser gsl integration\n"); fflush(stdout);
	}
      */
      if (sump > 0.) {
	// no_exception++;
...
void measure_ellipticity (int nt, int sampling, double maxlikel, 
                          complex double *ecorr, double *rcorr,
                          double *gweight, double *gvariance, double *sensitivity)
{

  // function to estimate a ellipticity.  Estimates also the corresponding
  // galaxy scalelength
  // Jul 2017 added updated calculation of sensitivity

  int ne, ix, iy, hns;
  double sump, ee1, ee2, likel, x, y, rmean;
  double edgef, Pthreshold, maxposterior;
  double moment, maxmoment, tinterval , sumpmoment, post;
  complex double eobs, sume, m1;

  // sensitivity variables
  int yg;
  int nsens=8;
  double gtestmod = 0.04;
  double gthetas, gsq, jacobian, sumesh[nsens];
  complex double gval, conjgval, esh, meanesh[nsens], meanx, meany, bs1, bs2, s1b[2];
  double s1matrix[4], det, bdet;
  
  if (sampling <=0)
    {
      fflush(stdout);
      fprintf(stderr," error in sampling specified for measure_ellipticity: %d \n",sampling);
      exit(EXIT_FAILURE);
    }
  ...
  *gweight = *gvariance = 0.;
  ...
  if (maxlikel > NOT_MEASURED)
    {
      hns = sampling/2;
            
      // measure maximum posterior and set posterior threshold
      // to cut out extreme unlikely likelihood samples
      maxposterior = -1.e10;
      for(ne = 0; ne < nume; ne++)
        {
          if (marP[nt][ne] > NOT_MEASURED)
            {
              // find maximum log posterior
              post = marP[nt][ne]-maxlikel+eprior[nt][ne];
              if (post > maxposterior) maxposterior = post;
            }
        }
      // apply cuts on the posterior to cut out high likelihoods at extreme ellipticity
      // (although these are generally given low weight anyway)
      Pthreshold = maxposterior + log(Pecut);

      // uncomment this to not apply cuts on the posterior
      // Pthreshold = -1.e10;
          ...
          if (marP[nt][ne] > NOT_MEASURED)
            {
              ee1 = e1[ne];
              ee2 = e2[ne];
              likel = exp(marP[nt][ne]-maxlikel);
              // finely sample the array down to e_interval and calculate summation
                  ...
                  for (ix=-hns; ix<=hns; ix++)
                    {
                      ...
                      x *= e_interval;
                      // calculate intrinsic ellipticity
                      eobs = x + I*y;
                      ...
                      if (post >= Pthreshold)
                        { 
                          ...
                          sume += edgef * eobs * likel;
                          sump += edgef * likel;
                          // find weighted scalelength
                          rmean += edgef * likel * rmarvals[nt][ne];
                        }
                      sumpmoment += edgef*likel; 
                      // accumulate means including excluded pixels
                      m1 += edgef * eobs * likel; 
                      // accumulate variance 
                      moment += edgef * pow(cabs(eobs),2) * likel;

                      // estimation of matrix sensitivity
                      for (yg=0; yg<nsens; yg++)
                        {
                          gthetas = 2.*yg*M_PI/nsens;
                          gval = gtestmod*(cos(gthetas)+I*sin(gthetas));
                          conjgval = conj(gval);
                          gsq = creal(conjgval*gval);
                          esh = (eobs-gval)/(1.-eobs*conjgval);
                          jacobian = pow((1.-gsq),2)/pow(cabs(1.-conjgval*eobs),4);
                          meanesh[yg] += esh*likel*edgef*jacobian;
                          sumesh[yg] += likel*edgef*jacobian;
                        }
                    }
                }
            }
      ...
      if (sump > 0. && sumpmoment > 0.) 
        {
          ...
          *rcorr = rmean;
          moment /= sumpmoment;
          m1 /= sumpmoment;
          moment -= 2.*(creal(m1)*creal(sume) + cimag(m1)*cimag(sume));
          moment += pow(cabs(sume),2); 
        }

      // work out lensfit weight from inverse variance of likelihood surface
      tinterval = sampling*e_interval;
      maxmoment = efunc_emax*efunc_emax/2.;
      if (moment < tinterval*tinterval) moment = tinterval*tinterval;
      if (moment >0.) {
        *gvariance = moment; // measurement variance only (2D)
        if (moment < maxmoment)
          {
            // correct for the finite boundary
            moment = moment*maxmoment/(maxmoment-moment);
            // add in the shape noise
            moment += priormoment;
            // convert to 1D moment
            moment /= 2.;
            // convert to lensfit weight
            *gweight = 1/moment;
          }
      }

      // calculate sensitivity and c terms
      for (yg=0; yg<4; yg++)
        {
          s1matrix[yg]=0.;
        }
      ...
      for (yg=0; yg<nsens; yg++)
        {
          ...
          if (sumesh[yg]>0.)
            {
              meanesh[yg] /= sumesh[yg];
              meanesh[yg] -= m1;
              esh = (m1-gval)/(1.-m1*conj(gval)) - m1;
              meany += meanesh[yg];
              meanx += esh;
            }
          else
          ...
          for (yg=0; yg<nsens; yg++)
            {
              ...
              meanesh[yg] -= meany;
              esh -= meanx;
              s1matrix[0] += pow(creal(esh),2);
              s1matrix[1] += creal(esh)*cimag(esh);
              s1matrix[2] += creal(esh)*cimag(esh);
              s1matrix[3] += pow(cimag(esh),2);
              s1b[0] += meanesh[yg]*creal(esh);
              s1b[1] += meanesh[yg]*cimag(esh);
            }
          det = s1matrix[0]*s1matrix[3]-s1matrix[1]*s1matrix[2];
          bdet = 1.;
          bs1 = 1.;
          bs2 = 0. + I;
          if (det>0.)
            {
              bs1 = (s1matrix[3]*s1b[0] - s1matrix[1]*s1b[1])/det;
              bs2 = (-s1matrix[2]*s1b[0] + s1matrix[0]*s1b[1])/det;
              bdet = creal(bs1)*cimag(bs2)-cimag(bs1)*creal(bs2);
            }
          /*
          printf(" determinants %f %f smatrix %f %f %f %f s1b %f %f %f %f \n",
                 det,bdet,s1matrix[0],s1matrix[1],s1matrix[2],s1matrix[3],
                 creal(s1b[0]),cimag(s1b[0]),creal(s1b[1]),cimag(s1b[1]));
          */
          if (det>0. && bdet>0.)
            {
              sensitivity[0] = sqrt(bdet);
            }
          else
            {
              // occasionally for nasty shaped likelihood surfaces, the response can
              // actually flip sign (positive g has a negative response) which causes
              // bdet to be negative.  We should not include such galaxies in the measurement
              // so set the sensitivity to zero
              sensitivity[0] = 0.0;
              /*
                fflush(stdout);
                printf(" zero or negative bias determinant %f with smatrix det %f\n",bdet,det);
                printf(" s1matrix %f %f %f %f\n",s1matrix[0],s1matrix[1],s1matrix[2],s1matrix[3]);
                printf(" s1b %f %f %f %f\n",creal(s1b[0]),cimag(s1b[0]),creal(s1b[1]),cimag(s1b[1]));
                printf(" ismatrix %f %f %f %f\n",ismatrix[0],ismatrix[1],ismatrix[2],ismatrix[3]);
                printf(" bs %f %f %f %f\n",creal(bs1),cimag(bs1),creal(bs2),cimag(bs2));
                printf(" ecomplex %f %f \n",creal(m1),cimag(m1));
                printf(" meanx %f %f \n",creal(meanx),cimag(meanx));
                printf(" meany %f %f \n",creal(meany),cimag(meany));                                                               printf(" esh, meanesh: \n");
                for (yg=0; yg<nsens; yg++)
                {
                gthetas = 2.*yg*M_PI/nsens;
                gval = gmod*(cos(gthetas)+I*sin(gthetas));
                esh = (m1-gval)/(1.-m1*conjgval) - m1;
                esh -= meanx;
                printf(" %f %f %f %f\n",creal(esh),cimag(esh),creal(meanesh[yg]),cimag(meanesh[yg]));
                }
                fflush(stdout);
              */
            }
          sensitivity[1] = creal(meany-meanx);
          sensitivity[2] = cimag(meany-meanx);
        }
    }
...
double loadmodel(int nt, int i, int jj, int thisnim, double diskflux, double bulgeflux)
{
  // load model components stored in array c back into the data postage stamp
  // suitable for refitting
  // check chisquared
  double val, chisq; // , sdata, smodel;
  int x, y, pixel, ii;

  chisq = 0.;
  // sdata = smodel = 0.;
  datasqp[nt][thisnim] = 0.;
  for(y = 0; y < pheight; y++) {
    for(x = 0; x < pwidth; x++) {
      pixel = x + y * pwidth;
      val = diskflux*c[nt][0][pixel] + bulgeflux*c[nt][1][pixel];
      chisq += pow((Dreal[i][jj][pixel] - val),2);
      // sdata += dpix[nt][pixel];
      dpix[nt][pixel] = val;
      // smodel += val;
      datasqp[nt][thisnim] += val * val / fsize;
    }
  }

  // printf("loadmodel %d %d %d %d %f %f %f \n",nt,i,jj,thisnim,sdata,smodel,chisq);
...
int measure_marginalised_likelihood (
				      int nt, int i, int nimcheck,
				      int sampling, int initsampling, 
				      int initnRo, int initrstep, 
				      double zerolikel,
				      double *maxlikel, double *maxposterior,
				      int printlikel
				      )
{
  int ne, new, bestnRo, rstep, nstep, Ploop, nret, newrstep, exception;
  double priorval, posterior, likelval;
  ...
  for (ne=0; ne<nume; ne++)
    {
      if (wanted[nt][ne] == 1) 
	{
	  //if (currentobject[nt]==printlikel) {printf(" loop %d %d %g \n",ne,wanted[nt][ne],marP[nt][ne]); fflush(stdout);}
	  ...
	  if (marP[nt][ne] == NOT_MEASURED)
	      ...
	      if (bestnRo < 0)
		{
	      ...
	      rstep = fittedrstep[nt][ne];
	      //if (currentobject[nt]==printlikel) {printf(" initial rstep %d \n",rstep); fflush(stdout);}
	      if (rstep<1)
		{
		  ...
		  while (newrstep != rstep && nstep < 8)
		    {
		      ...
		      nstep++;
		      rstep = newrstep;
		      //if (currentobject[nt]==printlikel) {printf(" loop rstep %d \n",rstep); fflush(stdout);}
		      nret = findmaxr (Ploop, nt, nimcheck, bestnRo, ne, rstep, rfitvals[nt][ne]);
		      // if successful, try to improve the rstep interval and update best position
		      if (nret<2)
			{
		  ...
		  if (currentobject[nt]==printlikel) 
		    {printf(" new %f %f nret %d bestnRo %d rstep %d \n",e1[ne],e2[ne],nret,bestnRo,rstep);}
		}
	      else
		{
		  if (currentobject[nt]==printlikel) 
		    {printf(" already %f %f bestnRo %d rstep %d \n",e1[ne],e2[ne],bestnRo,rstep);}
		}
	      if (rstep > 0)
		{
		  ...
		  //if (currentobject[nt]==printlikel) 
		  //{printf(" start marginaliser %d %d %d %f %f \n",ne,rstep,bestnRo,e1[ne],e2[ne]);fflush(stdout);}

		  exception += marginaliser(nt, i, nimcheck, bestnRo, rstep, ne, zerolikel);
		  new++;
		  //if (currentobject[nt]==printlikel) 
		  //{printf(" marginaliser ok %f %f marP %f \n",e1[ne],e2[ne],marP[nt][ne]);fflush(stdout);}
		}
	    }
	  ...
	  if (marP[nt][ne] > NOT_MEASURED)
	    {
	      ...
	      likelval = marP[nt][ne];
	      // define the largest likelihood value of interest as being the
	      // likelihood of the peak posterior - the idea here is to
	      // ensure that we sample to low enough likelihood values
	      // without being biased by the posterior (ie threshold is flat
	      // in L not in P) and without being biased by extreme high
	      // ellipticity points
	      if (posterior > *maxposterior) 
		{
		  *maxposterior = posterior;
...
int parse_string(char *string, double *value)
{
  // reads a single value from a string
...
void read_prior_parameters (char *prior_parameters_file)
{
  // reads a file containing prior parameter values
  ...
  pfile = fopen(prior_parameters_file,"r");
  if (pfile == NULL)
    {
      fflush(stdout);
      fprintf(stderr," error trying to open prior parameters file %s\n",
	      prior_parameters_file);
      exit(EXIT_FAILURE);
    }

  printf(" reading prior parameters from file %s\n",
	 prior_parameters_file);

  // read prior parameters
  status = i = 0;
  while (fgets(string, 200, pfile) != NULL && i < 10) {
    if (strncmp(string, "/", 1) != 0) {
      status += parse_string(string, &pvals[i]);
      i++;
    }
  }
  fclose(pfile);
  ...
  if (status != 0 || i != 7)
    {
      fflush(stdout);
      fprintf(stderr, " error reading value from input prior parameters file %s\n",
	      prior_parameters_file);
      exit(EXIT_FAILURE);
    }
  ...
  printf(" prior parameter r0          %f \n", r0);
  printf(" prior parameter rgamma      %f \n", rgamma);
  printf(" prior parameter rcut0       %f \n", rcut0);
  printf(" prior parameter efunc_emax  %f \n", efunc_emax);
  printf(" prior parameter efunc_sigma %f \n", efunc_sigma);
  printf(" prior parameter efunc_a     %f \n", efunc_a);
  printf(" prior parameter efunc_power %f \n", efunc_power);
...
void reconstructosPSF(int nt, double xval, double yval, int order, int image, int chip, int nchip, double *localpsf)
{
  // reconstruct the valid PSF region at this location on the mosaic
  // from the polynomial coefficients.

  int i, ichip, n, nq, qx, qy, pixel, opixel, x, y, yy;
  ...
  if (chip >= nchip)
    {
      fflush(stdout);
      fprintf(stderr," error in call to recontructosPSF, chip %d >= %d\n",chip,nchip);
      exit(EXIT_FAILURE);
    }

  // the psfindex[] array has elements which are matched to
  // the solution vector - i.e. elements are not yet ordered by location
  // on the image.  
  for (i=0; i<nfit; i++)
    {
      //printf(" %d %d\n",i,nfit);fflush(stdout);
      nq=0;
      psfindex[nt][i] = 0.;
		...
		{ // chip dependent
		  for (ichip=0; ichip<nchip; ichip++)
			...
			{ // select the appropriate coefficient for this chip
			  psfindex[nt][i] += (soln[image][n])*pow(xval,qx)*pow(yval,qy);
			}
		      nq++;
		    }
		}
  ...
  // now place into correct padded location
  for (y=0; y < fheight/2; y++)
      ...
      for (x=0; x < fwidth/2; x++)
	{
	  pixel = y*fwidth + x;
	  opixel = y*mwidth + x;
	  if (pindex[image][pixel]>=0)
	    localpsf[opixel] = psfindex[nt][pindex[image][pixel]];
	  else
	    localpsf[opixel] = 0.;
	}
      for (x=fwidth/2; x < fwidth; x++)
	{
	  pixel = y*fwidth + x;
	  opixel = y*mwidth + mwidth - fwidth + x;
	  if (pindex[image][pixel]>=0)
	    localpsf[opixel] = psfindex[nt][pindex[image][pixel]];
	  else
	    localpsf[opixel] = 0.;
  ...
  for (y=fheight/2; y < fheight; y++)
    {
      yy = y + mheight - fheight;
      for (x=0; x < fwidth/2; x++)
	{
	  pixel = y*fwidth + x;
	  opixel = yy*mwidth + x;
	...
	{
	  pixel = y*fwidth + x;
	  opixel = yy*mwidth + mwidth - fwidth + x;
	  if (pindex[image][pixel]>=0)
	    localpsf[opixel] = psfindex[nt][pindex[image][pixel]];
	  else
	    localpsf[opixel] = 0.;
...
int smatrix(int nt, double *pos, int *ppindex)
{
  ...
  int xmin,xmax,ymin,ymax,p,fp,nset,setppindex;
  ...
  for (y=0; y<pheight; y++)
    {
      rymin = oversampling*(y+pos[1]-0.5);
      rymax = oversampling*(y+pos[1]+0.5);
      ymin = (int)(rymin+fheight+0.5)-fheight;
      ymax = (int)(rymax+fheight+0.5)-fheight;
      for (x=0; x<pwidth; x++)
	{
	  rxmin = oversampling*(x+pos[0]-0.5);
	  rxmax = oversampling*(x+pos[0]+0.5);
		  ...
		  if (xxx>=0 && xxx<fwidth && yyy>=0 && yyy<fheight)
		    {
		      ...
		      if (ppindex[fp] >= 0)
			{
			  if (ppindex[fp]>=tfsize)
			    {
			      fprintf(stderr," smatrix ppindex error at %d, ppindex = %d\n",fp,ppindex[fp]);
			      fflush(stderr);
			      exit(EXIT_FAILURE);
			    }
			  ...
			  setppindex = 0;
			  for (jj=0; jj<npix[nt][p]; jj++)
			    {
			      if (spix[nt][p][jj]==ppindex[fp])
				{
				  sval[nt][p][jj] += fx1*fx2*fy1*fy2;
				  setppindex = 1;
				  break;
				}
			    }
			  ...
			  if (setppindex==0)
			    {
			      spix[nt][p][npix[nt][p]] = ppindex[fp];
			      sval[nt][p][npix[nt][p]] = fx1*fx2*fy1*fy2;
			      npix[nt][p]++;
			      nset++;
			    }
			}
...
void downsample(int nt, double *dpsf)
{
  int x, y, k, p;
  ...
  for (y=0; y<pheight; y++)
      ...
      for (x=0; x<pwidth; x++)
	{
	  ...
	  for (k=0; k<npix[nt][p]; k++)
	    {
	      if (spix[nt][p][k]<0 || spix[nt][p][k]>=fwidth*fheight)
		{
		  fprintf(stderr," spix error at %d %d, spix = %d\n",p,k,spix[nt][p][k]);
		  fflush(stderr);
		  exit(EXIT_FAILURE);
		}
	      dpsf[p] += sval[nt][p][k]*psfindex[nt][spix[nt][p][k]];
	    }
	}
    }
...
int open3Dfits(char *psfname, fitsfile* *afptr, int num, int pwidth, int pheight)
{
  int status = 0;
  int anaxis;
  long anaxes[3];
...
int write3Dfits(fitsfile *afptr, int num, double *f, int pwidth, int pheight)
{
  int status = 0;
  long fpixel[3]={1,1,1};
  int psfsize;
...
void complexconvolvemodel(fftw_plan * pinv,
              fftw_complex * B, complex float **modelft,
              int pheight, int pwidth, fftw_complex **C)
{
    ...
    for(mt = 0; mt < 2; mt++) {
        ...
        for(y = 0; y < pheight; y++) {
            for(x = 0; x < halfpwidth; x++) {
                yx = y * halfpwidth + x;
                C[mt][yx] = (fftw_complex)modelft[mt][yx] * B[yx] / fsize;
            }
        }
