...
int
updateoscoeffs(char *psfname, int ncoeffs, int order,
	       int chipvariation, int chiporder, double wsigma, double *xc, double *yc)
{
    fitsfile *afptr;
    int status = 0;
    int i;
    ...
    if (fits_update_key(afptr, TDOUBLE, "WSIGMA", &wsigma,
                        "sigma of astrometry weight function", &status)) {
        fits_report_error(stderr, status);      /* print error message */
        return (status);
    }
