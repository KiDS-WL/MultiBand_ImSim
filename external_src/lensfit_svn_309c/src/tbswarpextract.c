...
this version fits and subtracts local background
...
int
tbswarpextract(wcsstruct * wcs, float *apix, float *badpix, int *dim,
               double *dpix, double *dbadpix, int *objpix, float objx,
               float objy, float poserror, int pwidth, int pheight,
               ikernelstruct * ikernel, float noise, int *region,
               double *scalefactor, float maxlimit, double *rpos,
               double *wpos, double *wneg, double *wcentre, double **kernel,
               double *xfit, double *yfit, double *zfit, double *wfit,
               double **u, double **v)
{
    int ix, iy, x, y, xx, yy, sx, sy, spix, i, j, jj, n, pixel, dx, dy;
    int ngoodpix, objectcolour;
    int halfwidth, halfheight, ival[2];
    int allbad, nfound;
    int naxis = 2;
    int kwidth[2], xpix0, ypix0, xcen, ycen;
    int nfit, order, crossterm, ncoeffs;
    int n1, n2, nstart, nlist, more;
    float negative_sigma_limit, positive_sigma_limit;
    float dist, maxval;
    double cosdist, val, swarpshift[2];
    double hsize, xd, yd, avals[4], w[3];
    ...
    /*
     * improve the regions flagged as having object pixels 
     */

    // make linked list of bad pixels 
    nlist = 0;
    for(y = 0; y < pheight; y++) {
        for(x = 0; x < pwidth; x++) {
            j = y * pwidth + x;
    ...
    // work through and update list of bad pixels
    // iterate until no more pixels need to be dealt with
    more = 0;
    if (nlist > 0)
        more = 1;
    nstart = 0;
    while (more == 1) {
        ...
        n1 = nlist;
        n2 = nlist;
        // work through list of new bad pixels
        // check if any neighbours need to be flagged as bad
        for(i = nstart; i < nlist; i++) {
            y = objpix[i] / pwidth;
            x = objpix[i] - y * pwidth;
            for(dy = -1; dy <= 1; dy++) {
                yy = y + dy;
                ...
                for(dx = -1; dx <= 1; dx++) {
                    ...
                    jj = xx + yy * pwidth;
                    if (dbadpix[jj] > 0.) {
                        // flag any extra pixels that are above 2 sigma
                        if (dpix[jj] > positive_sigma_limit) {
                            if (n2 >= pwidth * pheight) {
                                fflush(stdout);
                                fprintf(stderr,
                                        " error counting pixels in tbswarpextract\n");
                                for(i = 0; i < n2; i++) {
                                    printf(" %d %d %d \n", i,
                                           objpix[i] / pwidth,
                                           objpix[i] -
                                           pwidth * (objpix[i] / pwidth));
                                }
                                printf(" %d %d %d \n", n2, jj / pwidth,
                                       jj - pwidth * (jj / pwidth));
                                fflush(stdout);
                            ...
                            objpix[n2] = jj;    // add to list
                            n2++;
                            ngoodpix--;
                            dbadpix[jj] = 0.;   // flag as bad
                            dpix[jj] = 0.;
                            more = 1;   // more iterations required
                        }
                    }
                }
            }
            ...
            for(x = 0; x < pwidth; x++) {
                ...
                if (xx >= 0 && xx < dim[0]) {
                    ...
                    if (dbadpix[jj] > 0. && region[j] == 0) {
                        ...
                        if (x >= pwidth / 2)
                            xd = (double) (x - pwidth);
                        if (xd * xd + yd * yd > 64)     // exclude central region from background region
                        {
                            xfit[nfit] = xd / hsize;
                            yfit[nfit] = yd / hsize;
                            wfit[nfit] = 1.;
