# -*- coding: utf-8 -*-
# @Author: lshuns
# @Date:   2022-09-02 16:27:20
# @Last Modified by:   lshuns
# @Last Modified time: 2022-09-28 16:15:12
...
# out info
# outpath = os.path.join(main_dir, f'skills_v07D7v_LF_321_shear_noSG_noWeiCut_newCut_const_withNeigIn.feather')
outpath = os.path.join(main_dir, f'skills_v07D7v_LF_321_shear_noSG_noWeiCut_newCut_var_withNeigIn.feather')
...
for part_tag in split_ran_tags:
    for i_shear, shear_tag in enumerate(unique_shear_tags):
        ...
        for file in file_list:
            ...
            cata0.loc[:, 'tile_label'] = tile_label
            ## rotation info
            ...
            # >>>>>>>>>>>>> get neighbour information from input catalogue
            cata_tmp = pd.read_feather(os.path.join(subdir, 'catalogues', 'input', f'gals_info_tile{tile_label}.feather'))
            ### discard duplicated rows
            cata_tmp.drop_duplicates(subset='index_input', inplace=True, ignore_index=True)
            # ++++++++++++++ build KDTree from input
            X_spac = np.empty((len(cata_tmp), 2), dtype=float)
            X_spac[:, 0] = cata_tmp['RA_input'].values * 3600.
            X_spac[:, 1] = cata_tmp['DEC_input'].values * 3600.
            kdt_sim = sst.cKDTree(X_spac, leafsize=100)
            # ++++++++++++++ find neighbours
            dist_list, index_list = kdt_sim.query(X_spac, k=2)
            cata_tmp.loc[:, 'nnb_dist'] = dist_list[:, 1]
            cata_tmp.loc[:, 'nnb_index'] = cata_tmp.loc[index_list[:, 1], 'index_input'].values
            cata_tmp.loc[:, 'nnb_redshift_input'] = cata_tmp.loc[index_list[:, 1], 'redshift_input'].values
            cata_tmp.loc[:, 'nnb_r_input'] = cata_tmp.loc[index_list[:, 1], 'r_input'].values
            if shear_type == 'variable':
                cata_tmp.loc[:, 'nnb_g1_in'] = cata_tmp.loc[index_list[:, 1], 'gamma1_input'].values
                cata_tmp.loc[:, 'nnb_g2_in'] = cata_tmp.loc[index_list[:, 1], 'gamma2_input'].values
            del dist_list, index_list
            # discard useless columns
            if shear_type == 'variable':
                cata_tmp.drop(columns=['RA_input', 'DEC_input', 'redshift_input', 'Re_input',
                                       'axis_ratio_input', 'position_angle_input', 'sersic_n_input', 'r_input',
                                       'gamma1_input', 'gamma2_input', 'e1_input_rot0', 'e2_input_rot0',
                                       'e1_input_rot90', 'e2_input_rot90'], inplace=True)
            else:
                cata_tmp.drop(columns=['RA_input', 'DEC_input', 'redshift_input', 'Re_input',
                                       'axis_ratio_input', 'position_angle_input', 'sersic_n_input', 'r_input',
            ...
            cata_tmp.rename(columns={"index_input": "id_input"}, inplace=True)
            # merge
            cata0 = cata0.merge(cata_tmp, on=['id_input'])
            del cata_tmp
...
# combined cata saved to /disks/shear16/ssli/ImSim/output/skills_v07D7v_extra/skills_v07D7v_LF_321_shear_noSG_noWeiCut_newCut_var_withNeigIn.feather
# Elapsed:39:30.88,User=2722.312,System=4929.113,CPU=322.7%.
...
# combined cata saved to /disks/shear16/ssli/ImSim/output/skills_v07D7v/skills_v07D7v_LF_321_shear_noSG_noWeiCut_newCut_const_withNeigIn.feather
# Elapsed:9:57.73,User=728.666,System=1195.845,CPU=321.9%.
